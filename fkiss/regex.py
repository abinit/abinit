...
class HasRegex:
    ...
    RE_OMP_SENTINEL = re.compile(r"\s*!\$OMP", re.IGNORECASE)

    RE_SEARCH_PROC = re.compile(r"(subroutine|function|program)", re.IGNORECASE)

    RE_IMPLICIT_NONE = re.compile(r"\s*implicit\s+none\s*", re.IGNORECASE)
    ...
    RE_PROC_END = re.compile(r"^end\s*(?P<proc_type>subroutine|function|program)\s+(?P<name>\w*)", re.IGNORECASE)
...
re.IGNORECASE | re.VERBOSE)
    ...
    RE_INTERFACE_START = re.compile(r"(abstract\s+)?interface\s*(?P<name>\w*)", re.IGNORECASE)
    RE_INTERFACE_END = re.compile(r"^end\s+interface\s*(?P<name>\w*)", re.IGNORECASE)

    # [if ()] call <name> [([<dummy-arg-list>])]
    #RE_SUBCALL = re.compile("^(?:if\s*\(.*\)\s*)?call\s+(?P<name>\w+)\s*(?:\(\s*(.*?)\s*\))?\Z", re.I)
    RE_SUBCALL = re.compile(r"^(?:if\s*\(.*\)\s*)?call\s+(?P<name>\w+)", re.IGNORECASE)
    ...
    RE_CONTAINS = re.compile(r"^(contains)\s*(\!+\s*\w*|\Z)", re.IGNORECASE)
    #RE_PUB_OR_PRIVATE_MODPROC = re.compile((r"^(?P<name>public|private)\s*(\!+\s*\w*|\Z)", re.I)

    # Continuation line:
    # To be improved with Lookahead/Lookbehind
    RE_CONTLINE_START = re.compile(r"^(?P<prefix>[^&\!]+)&(?P<postfix>\s*(\!+.*)?)", re.IGNORECASE)

    RE_CONTLINE_NEXT = re.compile(r"^&?(?P<value>.*?)&\s*(\!+.*)?", re.IGNORECASE)
    ...
    RE_CHARACTER_DEC = re.compile(r"^character\s*\(\s*len\s*=\s*(?P<len>(\*|\w+))\)", re.IGNORECASE)

    # To search for intent.
    RE_INTENT = re.compile(r",\s+intent\s*\(\s*(?P<value>in|out|inout)\s*\)", re.IGNORECASE)

    # For type(xxx)
    RE_TYPECLASS_DEC = re.compile(r"^(?P<ftype>(type|class))\s*\(\s*(?P<name>\w+)\s*\)", re.IGNORECASE)
...
re.IGNORECASE | re.VERBOSE)
    ...
    RE_INCLUDE = re.compile(r"#?include\s+(?P<path>.+)", re.IGNORECASE)
...
FORTRAN_INTRINSICS = {
    ...
    "unpack","use","value","verify","volatile","wait","where","write","xor","zabs",
}
