...
def fort2html(s: str, full: bool = True) -> str:
    ...
    Args:
        s: The Fortran code string.
        full: If True, generate a full HTML document.

    Returns:
        str: The HTML-formatted code, or the original string if pygments is unavailable.
    """
    try:
        from pygments import highlight
    except ImportError:
...
class FortranVariable(Node):
    """
    Representation of a Fortran variable with its type, shape, and attributes.

    Args:
        name: Name of the variable.
        ancestor: The parent `Node` (e.g., a `Procedure` or `Datatype`).
        ftype: Fortran type (e.g., "integer", "real").
        shape: Array shape (None if scalar).
        kind: Optional kind parameter.
        strlen: Optional string length for characters.
        attribs: List of attributes (e.g., "allocatable", "pointer").
        initial_value: Optional default value.
        doc: Optional documentation/comments associated with the variable.
    """
...
class Datatype(Node, HasRegex):
    """
    Representation of a Fortran derived type (TYPE).
    ...
    def analyze(self, verbose: int = 0) -> None:
        ...
        for i, line in enumerate(self.lines):
            ...
            if not line or line.startswith("#"):
                continue
            if i == 0 and not line.lower().startswith("type"):
                raise ValueError(line)
            if i == len(self.lines) - 1 and not line.lower().startswith("end"):
                raise ValueError(line)
            if i in (0, len(self.lines) - 1):
                continue
            ...
            if line.startswith("!"):
                ...
                if names is not None:
                    for i, name in enumerate(names):
                        ...
                        self.variables[name] = FortranVariable(
                            name,
                            self,
                            ftype,
                            shapes[i],
...
class Procedure(Node):
    """
    Base class for Fortran procedures (modules, programs, subroutines, functions).

    Attributes:
        contains: List of nested `Procedure` objects.
        local_uses: List of names of modules explicitly used by this procedure.
        includes: List of names of files included by this procedure.
        parents: List of `Procedure` objects that call or contain this one.
        children: List of names of subroutines/functions called by this procedure.
        path: Path to the source file containing this procedure.
    """
    ...
    def __init__(
        ...
        if self.basename.endswith(".F90.in"):
...
class Program(Procedure):
    """Representation of a Fortran PROGRAM."""
...
class Function(Procedure):
    """Representation of a Fortran FUNCTION."""
...
class Subroutine(Procedure):
    """Representation of a Fortran SUBROUTINE."""
...
class FortranKissParser(HasRegex):
    """
    Parser for Fortran source code based on regular expressions.

    This parser performs a "kiss" (Keep It Simple, Stupid) analysis to identify
    basic structural elements like modules, subroutines, and their dependencies,
    without full compiler-level semantic analysis.

    Args:
        macros: Dictionary of macro substitutions to apply before parsing.
        verbose: Verbosity level for debugging.
    ...
    def parse_file(self, path: str, include_files: bool = False) -> FortranKissParser:
        ...
        Args:
            path: Path to the .f90 or .F90 file.
            include_files: If True, recursively include content of `#include` directives.

        Returns:
            FortranKissParser: The parser instance populated with results.
        """
        with open(path, encoding="utf8") as fh:
            ...
            if string and string[0] == "#":
                raise ValueError(
                    "Found `#` as first character in file `%s`\n" % path
    ...
    def parse_string(self, string: str, path: str | None = None) -> FortranKissParser:
        ...
        Args:
            ...
            path: Optional path for error reporting and metadata.
        ...
        while self.lines:
            ...
            if not line:
                continue
            if self.handle_comment(line):
                continue
            # Convert to lower case here so that we don't have to deal with case.
            line = line.lower()

            if self.handle_use_statement(line):
                continue
            if self.handle_cpp_line(line):
                continue
            if self.handle_contains(line):
                continue
            if self.handle_call(line):
                continue
            # subroutine or function declaration.
            if self.handle_procedure(line):
                continue
            if self.consume_module_header(line):
                continue
            # Handle `end module`
        ...
        while self.stack:
            ...
            elif p.is_subroutine:
                self.subroutines.append(p)
            elif p.is_function:
                self.functions.append(p)
            else:
                raise ValueError("Don't know how to handle type `%s`" % type(p))
    ...
    def consume_module_header(self, line: str) -> bool:
        ...
        while self.lines:
            ...
            if self.consume_interface(line):
                continue

            # Datatype declaration.
            if self.consume_datatype(line):
                continue
    ...
    def consume_interface(self, line: str) -> bool:
        ...
        name = m.group("name")
        if self.verbose > 1:
            print("begin interface", name, "in line:", line)
        while self.lines:
            line = self.lines.popleft()
    ...
    def consume_datatype(self, line: str) -> bool:
        ...
        while self.lines:
            ...
            if end_match:
                ...
                if self.verbose > 1:
                    print("end datatype %s in %s" % (end_name, line))
                if name != end_name:
    ...
    def handle_procedure(self, line: str) -> bool:
        ...
        if proc_type == "subroutine":
            ...
            new_node = Program(
                name, self.ancestor, self.preamble, arg_names=arg_names, path=self.path
            )
        else:
    ...
    def handle_args(self, line: str) -> bool:
        ...
        for var in fvars:
            if var.name not in proc.args:
                ...
                self.warn(
                    "Cannot find `%s` in `%s`\nin file: %s"
                    % (var.name, list(proc.args.keys()), self.path)
                )
            else:
    ...
    def from_lines(cls, lines: list[str]) -> RobodocHeader:
        ...
        for line in lines[1:]:
            ...
            if not line.startswith("!!"):
                break
            s = line[2:]
            if active_key is None and not s:
                continue
            k = s.strip()
