...
# TYPE 2
    if (type == 2 and not temperature):
      if lifetime:
        ...
        if nb_cpus == 1: # Sequential version does not need multiprocessing.Pool
          for ii in N.arange(len(arguments)):
            total = dynamic_zpm(arguments[ii],ddw_save=ddw_save,ddw_save2=ddw_save2,type=type,smearing=smearing,eig0=eig0,degen=degen)
            if ii == 0:
              self.total_corr = total
            else:
              self.total_corr += total
        else:
    ...
    if (type == 2 and temperature):
      if lifetime:
        ...
        if nb_cpus == 1: # Sequential version does not need multiprocessing.Pool
          for ii in N.arange(len(arguments)):
            total = dynamic_zpm_temp(arguments[ii],ddw_save=ddw_save,ddw_save2=ddw_save2,type=type,temp_info=temp_info,\
