...
while IFS= read -r -d '' file; do
  ...
  if [ "${size}" -gt "${max_file_size_bytes}" ]; then
    printf 'File exceeds 50 MiB limit: %s (%s bytes)\n' "${file}" "${size}"
    oversized=1
  fi
done < <(git diff --cached --name-only -z --diff-filter=ACMRT)

if [ "${oversized}" -ne 0 ]; then
  printf 'Commit rejected: remove or reduce staged files larger than 50 MiB.\n'
  exit 1
fi

printf 'All staged files are within the 50 MiB limit.\n'
