...
!! SOURCE
 ...
 type, public :: gwr_t
  ...
  integer,allocatable :: nbcalc_ks(:,:)
   ...
   ! to mix it with low-level just to make memory for ugb scale better. The size of ugb is negligible
   ! when compared to G and Chi.
...
! *************************************************************************
 ...
 ! Keep a reference to other objects to simplify the API.
 gwr%dtset => dtset
 gwr%dtfil => dtfil
 gwr%cryst => cryst
 gwr%psps => psps
 ...
 if (gwr%dtset%useria == 2) gwr%use_supercell_for_tchi = .False.
 gwr%use_supercell_for_sigma = .True.
 if (gwr%dtset%userib == 2) gwr%use_supercell_for_sigma = .False.
 ...
 !call xmpi_min_ip(gwr%sc_batch_size, input_comm, ierr)
 !call xmpi_min_ip(gwr%uc_batch_size, input_comm, ierr)
...
subroutine gwr_gk_to_scbox(gwr, sc_ngfft, desc_mykbz, green_scgvec, my_ir, ndat, idat_list, &
                           gt_gpr, gt_scbox, gt_scbox_win)
...
#else
   ! This one is buggy but it's supposed to be the most efficient one.
   do ii=1,gwr%kpt_comm%nproc
     !IF (.not. MPI_ASYNC_PROTECTS_NONBLOCKING) CALL MPI_F_SYNC_REG(gt_scbox)
     !call xmpi_barrier(gwr%kpt_comm%value)
     call xmpi_win_fence(gt_scbox_win)
     idat = idat_list(ii)
     !write(std_err, *)" me, epoch, idat", gwr%kpt_comm%me, ii, idat
...
subroutine gwr_wcq_to_scbox(gwr, sc_ngfft, desc_myqbz, wc_scgvec, my_ir, ndat, idat_list, &
...
subroutine gwr_get_gkbz_rpr_pm(gwr, ik_bz, itau, spin, gk_rpr_pm, ipm_list)
...
!!
!! FUNCTION
!!  Helper function to FFT transform a two-point function: F_{r',r} --> F_{g,g'}
!!
!! INPUTS
!!
!! OUTPUT
...
subroutine gwr_rpr_to_ggp(gwr, desc, rpr, ggp)

!Arguments ------------------------------------
 class(gwr_t),intent(in) :: gwr
 type(desc_t),intent(in) :: desc
 class(__slkmat_t),intent(inout) :: rpr, ggp

!Local variables-------------------------------
 integer :: ig2, npwsp, col_bsize, ir2, ndat
 type(__slkmat_t) :: rgp, gpr
 character(len=500) :: msg
 type(uplan_t) :: uplan_k
...
! *************************************************************************
 ...
 ! Allocate intermediate gpr PBLAS matrix to store F(g',r)
 npwsp = desc%npw * gwr%nspinor
 ABI_CHECK(block_dist_1d(gwr%g_nfft * gwr%nspinor, gwr%g_comm%nproc, col_bsize, msg), msg)
 call gpr%init(npwsp, gwr%g_nfft * gwr%nspinor, gwr%g_slkproc, desc%istwfk, size_blocs=[-1, col_bsize])
 ...
 ! F(g',r) --> F(r,g')
 call gpr%ptrans("N", rgp, free=.True.)

 ! F(r',g) --> F(g',r), results in gpr%buffer
 do ig2=1, ggp%sizeb_local(2), gwr%uc_batch_size
   ndat = blocked_loop(ig2, ggp%sizeb_local(2), gwr%uc_batch_size)
   call uplan_k%execute_rg(ndat, rgp%buffer_cplx(:, ig2), ggp%buffer_cplx(:, ig2))
 end do
...
subroutine gwr_get_wc_rpr_qbz(gwr, iq_bz, itau, spin, wc_rpr)
...
!Local variables-------------------------------
!scalars
 integer :: ig2, npwsp, nrsp, col_bsize, ir1, ndat !, idat, g0_q(3), my_iqf,
 !logical :: q_is_gamma
 character(len=500) :: msg
...
!arrays
 ...
 integer :: idat_list(gwr%kpt_comm%nproc), kproc_list(gwr%kpt_comm%nproc)
...
! *************************************************************************
 ...
 if (gwr%use_supercell_for_tchi) then
   ...
   kproc_list = [(ii, ii=1,gwr%kpt_comm%nproc)]
   idat_list = cshift(kproc_list, shift=-gwr%kpt_comm%me)
   !write(std_err, *)" me, idat_list", gwr%kpt_comm%me, idat_list
...
if (.not. use_shmem_for_k) then
   ...
   call green_plan%free()
    ...
    ! ===================================================================
    call wrtout(std_out, " Building chi with convolutions in k-space:", pre_newlines=2)
    ...
    do my_is=1,gwr%my_nspins
      ...
      do my_it=1,gwr%my_ntau
        ...
        do my_ikf=1,gwr%my_nkbz
          ...
          call gwr%get_gkbz_rpr_pm(ik_bz, itau, spin, gk_rpr_pm, ipm_list=[1])
          ...
          do my_iqi=1,gwr%my_nqibz
            ...
            call gwr%get_gkbz_rpr_pm(ikq_bz, itau, spin, gkq_rpr_pm, ipm_list=[2])
   ...
   do my_it=1,gwr%my_ntau
     ...
     do ikcalc=1,gwr%nkcalc
       do band=gwr%bstart_ks(ikcalc, spin), gwr%bstop_ks(ikcalc, spin)
         ...
         ! This is just to avoid sending garbage to Pade
         sigc_pm(:) = exp(-gwr%tau_mesh(itau))
         ibc = band - gwr%bstart_ks(ikcalc, spin) + 1
         sigc_it_diag_kcalc(:, itau, ibc, ikcalc, spin) = sigc_pm(:)
        end do
     end do ! ikcalc
...
! *************************************************************************
 ...
 !  [free_ugb]: True if array with empty KS states should freed as soon as possible. Default: True
...
! *************************************************************************
 ...
 if (gwr%comm%me == 0 .and. gwr%dtset%prtvol >= 1) then
   ...
   associate (units => [std_out, ab_out])
