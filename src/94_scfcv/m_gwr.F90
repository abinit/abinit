   ...
   do spin=1,gwr%nsppol
     do ik_ibz=1,gwr%nkibz
       ...
       do bstart=1, nbsum, band_step
         ...
         ABI_MALLOC(cg_work, (2, npwsp, nb)) ! This is always dp
         if (gwr%comm%me == master) then
...
!arrays
 integer :: mask_kibz(gwr%nkibz), units(2)
 real(dp) :: tsec(2)
...
! *************************************************************************
 ...
 ! F(r',g) --> F(g',r) and store results in gpr%buffer
 do ig2=1, ggp%sizeb_local(2), gwr%uc_batch_size
   ndat = blocked_loop(ig2, ggp%sizeb_local(2), gwr%uc_batch_size)
...
! *************************************************************************
 ...
 ! FFT Wc(g,g') -> Wc(r,g'). Results stored in rgp
...
if (.not. use_shmem_for_k) then
         ...
         if (.not. use_mpi_for_k) then
           ...
           ! Reduce one G_k(tau) on the idat-1 proc, perform FFTs in parallel.
           ...
           do ipm=1,2
             call green_plan%execute(gt_scbox(:,idat,ipm), -1, ndat=gwr%nspinor)
         ...
         do ii=1,gwr%kpt_comm%nproc
           idat = idat_list(ii); if (idat > ndat) cycle
           do my_ikf=1,gwr%my_nkbz
             ! Compute k+g
             ik_bz = gwr%my_kbz_inds(my_ikf); gg = nint(gwr%kbz(:, ik_bz) * gwr%ngkpt)
             desc_k => desc_mykbz(my_ikf)
