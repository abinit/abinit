...
!!
!!   - in GWR, the k-mesh must be Gamma-centered when the supercell-method is used.
!!     Convolutions can use shifted k-meshes, although this part should be tests.
...
!!
!!   - For nspinor = 2, we use 4 Scalapack matrices to store G^k_ab(g,g') instead of a single matrix of shape (2*npw, 2*npw)
!!
!!   - Memory peaks:
!!
...
!!
!! COPYRIGHT
!! Copyright (C) 1999-2026 ABINIT group (MG)
...
module m_gwr
 ...
 use m_fft_mesh,      only : calc_ceikr, calc_ceigr
...
!! SOURCE
 ...
 type, public :: gwr_t
  ...
  integer,allocatable :: bstart_ks(:,:)
   ...
   ! Depends on spin because all degenerate states should be included when symsigma /= 0 is used.
 ...
 contains
   ...
   procedure :: rpr_to_ggp => gwr_rpr_to_ggp
   ! F_{r',r} --> F_{g,g'}
...
!! ke_ebands=KS energies
!! mpi_enreg=information about MPI parallelization (FAKE one as GWR has its own MPI algo)
!! input_comm=MPI communicator
!!
!! OUTPUT
!!
!! SOURCE
...
!Local variables-------------------------------
!scalars
 integer,parameter :: qptopt1 = 1, master = 0, ndims = 4
...
!arrays
 ...
 real(dp) :: my_shiftq(3,1), kk_ibz(3), kk_bz(3), qq_bz(3), qq_ibz(3), kk(3), tsec(2), qbz(3), sq(3)
...
! *************************************************************************
 ...
 gwr%ks_ebands => ks_ebands; gwr%kibz => ks_ebands%kptns; gwr%wtk => ks_ebands%wtk; gwr%mpi_enreg => mpi_enreg
 ...
 ! Note symrec convention here.
 kptopt = ks_ebands%kptopt
 call krank_ibz%from_kptrlatt(gwr%nkibz, kibz, ks_ebands%kptrlatt, compute_invrank=.False.)

 ABI_MALLOC(gwr%kbz2ibz, (6, gwr%nkbz))
 if (kpts_map("symrec", kptopt, cryst, krank_ibz, gwr%nkbz, gwr%kbz, gwr%kbz2ibz) /= 0) then
 ...
 if (kpts_map("symrel", kptopt, cryst, krank_ibz, gwr%nkbz, gwr%kbz, gwr%kbz2ibz_symrel) /= 0) then
   ABI_ERROR("Cannot map kBZ to IBZ!")
 end if

 ! Here we set up the q-mesh for chi and w and the corresponding symmetry tables.
 ! Supercell algos require Gamma-centered meshes with ngqpt so that we can easily insert q+G in the FFT box
 ! Convolution algos can handle multiple shifts in the k-mesh.
 ! After this block we should know IBZ, BZ, weights and qbz2ibz mapping
 if (.not. can_use_kshifts) then
   ...
   call qrank%from_kptrlatt(gwr%nqibz, gwr%qibz, qptrlatt, compute_invrank=.False.)
   ...
   ! Transfer data from qmesh to gwr.
 ...
 do ikcalc=1,gwr%nkcalc
   ...
   if (kpts_map("symrel", kptopt, cryst, krank_ibz, 1, kk, indkk_k) /= 0) then
      write(msg, '(5a)' ) &
 ...
 ! No augmentation of workspace arrays.
       ...
       do ip_t=1,gwr%ntau
         ...
         do ip_k=1,gwr%nkbz
           ...
           do ip_g=1,gwr%green_mpw
             ...
             !ABI_CHECK(block_dist_1d(gwr%green_mpw, ip_g, col_bsize, msg), msg)
 ...
 ! Store the rank of the MPI proc in tau_comm treating the itau index.
 ...
 if (my_rank == master .and. gwr%gwrnc_write) then
   ...
   ncerr = nctk_def_dims(ncid, [ &
     ...
     nctkdim_t("nqibz", gwr%nqibz), &
     nctkdim_t("nqbz", gwr%nqbz), &
     nctkdim_t("nkcalc", gwr%nkcalc), &
     nctkdim_t("max_nbcalc", gwr%max_nbcalc), &
     nctkdim_t("smat_bsize1", smat_bsize1), &
     nctkdim_t("smat_bsize2", smat_bsize2) &
     ], defmode=.True.)
   NCF_CHECK(ncerr)
...
!Local variables-------------------------------
 integer :: my_is, my_it, ipm, npw, col_bsize, itau, spin, ik_ibz, iq_ibz, iab
   ...
   do my_it=1,gwr%my_ntau
     ...
     case ("green")
       ...
       do ik_ibz=1,gwr%nkibz
         ...
         npw = gwr%green_desc_kibz(ik_ibz)%npw
         ABI_CHECK(block_dist_1d(npw, gwr%g_comm%nproc, col_bsize, msg), msg)
         do iab=1,gwr%nsig_ab
           associate (gt => gwr%gt_kibz(:, ik_ibz, itau, spin, iab))
           do ipm=1,2
             if (action == "malloc") call gt(ipm)%init(npw, npw, gwr%g_slkproc, istwfk1, size_blocs=[-1, col_bsize]) !, gpu_action=gpu_action)
     ...
     case ("tchi", "wc")
       ...
       do iq_ibz=1,gwr%nqibz
         ...
         npw = gwr%tchi_desc_qibz(iq_ibz)%npw
         ABI_CHECK(block_dist_1d(npw, gwr%g_comm%nproc, col_bsize, msg), msg)
         if (what == "tchi") mat => gwr%tchi_qibz(iq_ibz, itau, spin)
         if (what == "wc") mat => gwr%wc_qibz(iq_ibz, itau, spin)
         if (action == "malloc") call mat%init(npw, npw, gwr%g_slkproc, 1, size_blocs=[-1, col_bsize]) !, gpu_action=gpu_action)
     ...
     case ("sigma")
       ...
       do ik_ibz=1,gwr%nkibz
         ...
         do iab=1,gwr%nsig_ab
           ...
           do ipm=1,2
             if (action == "malloc") call sigc(ipm)%init(npw, npw, gwr%g_slkproc, 1, size_blocs=[-1, col_bsize]) !, gpu_action=gpu_action)
       ...
       if (need_block_ks) then
         ...
         do band=bstart, bstop
           ...
           if (.not. cut_ug) then
             ...
             ugb%buffer_cplx(:, il_b) = cmplx(cg_work(1,:,ib), cg_work(2,:,ib), kind=gwp)
             ...
             do spinor=1,gwr%nspinor
               ...
               do ig=1,npw_k_disk
                 ...
                 if (gf2wfd(ig) /= 0) then
                   ugb%buffer_cplx(igw, il_b) = cmplx(cg_work(1,icg,ib), cg_work(2,icg,ib), kind=gwp)
                 end if
               end do
             end do ! spinor
...
!Local variables-------------------------------
!scalars
 integer :: my_is, my_iki, spin, ik_ibz, band, itau, ipm, il_b, npw, isgn, my_it, nbsum !, ig_glob, ig_loc, jg_loc, nb_occ
 integer :: ii, icomp
...
! *************************************************************************
 ...
 do my_is=1,gwr%my_nspins
   ...
   do my_iki=1,gwr%my_nkibz
     ...
     call green%init(npw, npw, gwr%g_slkproc, istwfk1) ! size_blocs=[-1, col_bsize])
       ...
       do ipm=1,2
         do ispinor = 1, gwr%nspinor
           ...
           do il_b = 1, work_gb(ispinor)%size_local(2)
             ...
             call xscal(npw, real(sqrt(gt_rfact), kind=gwp), work_gb(ispinor)%buffer_cplx(:, il_b), 1)
           end do ! il_b
         end do !ispinor
         ! end do ! ipm
         ! end do ! itau
...
!!
!! INPUTS
!!  select_my_kbz: Mask used to select a subset of gwr%my_nkbz k-points.
!!
!! OUTPUT
!!
!! SOURCE

subroutine gwr_gk_to_scbox(gwr, sc_ngfft, select_my_kbz, desc_mykbz, green_scgvec, my_ir, ndat, gt_gpr, gt_scbox, gt_scbox_win)
...
!Arguments ------------------------------------
 ...
 complex(gwp), intent(out) :: gt_scbox(product(sc_ngfft(4:6)), gwr%sc_batch_size, 2)
...
! *************************************************************************
 ...
 if (.not. present(gt_scbox_win)) then
   ...
   do my_ikf=1,gwr%my_nkbz
     ...
     do ipm=1,2
       call desc_mykbz(my_ikf)%to_scbox(gwr%kbz(:, ik_bz), gwr%ngkpt, sc_ngfft, ndat, &
   ...
   do iepoch=1,gwr%kpt_comm%nproc
     ...
     do my_ikf=1,gwr%my_nkbz
       ...
       ik_bz = gwr%my_kbz_inds(my_ikf); gg = nint(gwr%kbz(:, ik_bz) * gwr%ngkpt)
       do ipm=1,2
         call desc_mykbz(my_ikf)%to_scbox(gwr%kbz(:, ik_bz), gwr%ngkpt, sc_ngfft, ndat1, &
...
!!
!! INPUTS
!!  select_my_qbz: Mask used to select a subset of gwr%my_qkbz q-points.
!!
!! OUTPUT
!!
!! SOURCE

subroutine gwr_wcq_to_scbox(gwr, sc_ngfft, select_my_qbz, desc_myqbz, wc_scgvec, my_ir, ndat, &
                            wc_gpr, wct_scbox, wct_scbox_win)

!Arguments ------------------------------------
 class(gwr_t),target,intent(in) :: gwr
 integer,intent(in) :: sc_ngfft(18)
 logical,intent(in) :: select_my_qbz(gwr%my_nqbz)
 ...
 integer,intent(in) :: my_ir, ndat
 complex(gwp),intent(out) :: wct_scbox(product(sc_ngfft(4:6)), gwr%sc_batch_size)
 !complex(gwp),intent(out) :: wct_scbox(:,:)
...
! *************************************************************************
 ...
 if (.not. present(wct_scbox_win)) then
   ...
   do my_iqf=1,gwr%my_nqbz
     ...
     call desc_myqbz(my_iqf)%to_scbox(gwr%qbz(:,iq_bz), gwr%ngqpt, sc_ngfft, ndat, &
   ...
   do iepoch=1,gwr%kpt_comm%nproc
     ...
     do my_iqf=1,gwr%my_nkbz
       if (.not. select_my_qbz(my_iqf)) continue
       iq_bz = gwr%my_qbz_inds(my_iqf)
       call desc_myqbz(my_iqf)%to_scbox(gwr%qbz(:,iq_bz), gwr%ngqpt, sc_ngfft, ndat1, &
...
subroutine gwr_get_myk_green_gpr(gwr, itau, spin, select_my_kbz, desc_mykbz, gt_gpr)
...
!Local variables-------------------------------
!scalars
 integer :: my_ikf, ik_bz, ig2, ipm, npw, col_bsize, ndat, gpu_option, gpu_mode, iab
 logical :: k_is_gamma
...
! *************************************************************************
 ...
 do my_ikf=1,gwr%my_nkbz
   ...
   if (.not. k_is_gamma) then
     call calc_ceikr(kk_bz, gwr%g_ngfft, gwr%g_nfft, 1, ceikr)
#ifdef HAVE_OPENMP_OFFLOAD
   ...
   call uplan_k%init(desc_k%npw, 1, gwr%uc_batch_size, gwr%g_ngfft, desc_k%istwfk, &
   ...
   do ipm=1,2
     do iab=1,gwr%nsig_ab
       ...
       npw = desc_k%npw
       ABI_CHECK(block_dist_1d(npw, gwr%g_comm%nproc, col_bsize, msg), msg)
       call rgp%init(gwr%g_nfft, npw, gwr%g_slkproc, desc_k%istwfk, size_blocs=[-1, col_bsize]) !, gpu_action=gpu_action)
...
subroutine gwr_get_gkbz_rpr_pm(gwr, ik_bz, itau, spin, gk_rpr_pm, &
                               g0, ipm_list) ! optional
...
!Local variables-------------------------------
!scalars
 integer :: ig2, ipm, npw, col_bsize, ir1, ndat, ii, num_pm, ipm_list__(2), gpu_option, gpu_mode, iab
 logical :: have_g0
 !real(dp) :: cpu, wall, gflops
...
! *************************************************************************
 ...
 if (present(g0)) then
   ...
   if (any(g0 /= 0)) then
     have_g0 = .True.
     ABI_MALLOC(ceig0r, (gwr%g_nfft))
     call calc_ceigr(-g0, gwr%g_nfft, 1, gwr%g_ngfft, ceig0r)
     ABI_MALLOC(conjg_ceig0r, (gwr%g_nfft))
 ...
 call uplan_k%init(desc_kbz%npw, 1, gwr%uc_batch_size, gwr%g_ngfft, desc_kbz%istwfk, &
 ...
 do ii=1,num_pm
   ...
   ABI_CHECK(block_dist_1d(npw, gwr%g_comm%nproc, col_bsize, msg), msg)
   do iab=1,gwr%nsig_ab
     call rgp%init(gwr%g_nfft, npw, gwr%g_slkproc, desc_kbz%istwfk, size_blocs=[-1, col_bsize]) !, gpu_action=gpu_action)
     associate (g_gp => gt_pm(ipm, iab))
     gpu_mode = 1
     do ig2=1, g_gp%size_local(2), gwr%uc_batch_size
...
!!  !
!!  !!Local variables-------------------------------
!!  ! integer :: ig2, npw, nrsp, col_bsize, ir1, ndat
...
!!  ! ABI_CHECK(block_dist_1d(npw, gwr%g_comm%nproc, col_bsize, msg), msg)
!!  ! call rgp%init(nrsp, npw, gwr%g_slkproc, desc%istwfk, size_blocs=[-1, col_bsize])
!!  !
...
!Local variables-------------------------------
 integer :: ig2, npw, nr, col_bsize, ir2, ndat, isign, gpu_option, gpu_mode
...
! *************************************************************************
 ...
 npw = desc%npw; nr = gwr%g_nfft
 ABI_CHECK(block_dist_1d(nr, gwr%g_comm%nproc, col_bsize, msg), msg)

 gpu_option = gwr%dtset%gpu_option
 gpu_action = "None"; if (gpu_option == ABI_GPU_OPENMP) gpu_action = "alloc"
 call gp_r%init(npw, nr, gwr%g_slkproc, desc%istwfk, size_blocs=[-1, col_bsize])

 call uplan_k%init(desc%npw, 1, gwr%uc_batch_size, gwr%g_ngfft, desc%istwfk, desc%gvec, gwp, &
                   0)
 ...
 do ir2=1, rp_r%size_local(2), gwr%uc_batch_size
   ...
   call uplan_k%execute_rg(ndat, rp_r%buffer_cplx(:,ir2), gp_r%buffer_cplx(:,ir2), &
                           isign=isign, iscale=0, gpu_mode=gpu_mode)
 end do

 ! F(g',r) --> F(r,g')
 !call gp_r%ptrans("N", r_gp, free=.True.)
 ! FIXME: I don't know why but C is needed here.
 ...
 do ig2=1, g_gp%size_local(2), gwr%uc_batch_size
   ndat = blocked_loop(ig2, g_gp%size_local(2), gwr%uc_batch_size)
   call uplan_k%execute_rg(ndat, r_gp%buffer_cplx(:,ig2), g_gp%buffer_cplx(:,ig2), &
                           isign=-isign, iscale=0, gpu_mode=gpu_mode)
 end do
...
subroutine gwr_get_myq_wc_gpr(gwr, itau, spin, select_my_qbz, desc_myqbz, wc_gpr)
...
!Local variables-------------------------------
!scalars
 integer :: my_iqf, iq_bz, ig2, npw, col_bsize, ndat, gpu_option, gpu_mode
 real(dp) :: cpu, wall, gflops, mem_mb, qq_bz(3)
 logical :: q_is_gamma
 character(len=500) :: msg, gpu_action
 type(__slkmat_t) :: rgp, wc_qbz
 type(uplan_t) :: uplan_q
 complex(gwp),allocatable :: ceiqr(:)
! *************************************************************************
 ...
 do my_iqf=1,gwr%my_nqbz
   ...
   if (.not. q_is_gamma) then
     call calc_ceikr(qq_bz, gwr%g_ngfft, gwr%g_nfft, 1, ceiqr)
#ifdef HAVE_OPENMP_OFFLOAD
   ...
   npw = desc_q%npw
   ABI_CHECK(block_dist_1d(npw, gwr%g_comm%nproc, col_bsize, msg), msg)
   call rgp%init(gwr%g_nfft, npw, gwr%g_slkproc, desc_q%istwfk, size_blocs=[-1, col_bsize])

   call uplan_q%init(desc_q%npw, 1, gwr%uc_batch_size, gwr%g_ngfft, desc_q%istwfk, &
                     desc_q%gvec, gwp, gpu_option)
   ...
   do ig2=1,wc_qbz%size_local(2), gwr%uc_batch_size
     ...
     if (q_is_gamma) then
       call uplan_q%execute_gr(ndat, wc_qbz%buffer_cplx(:, ig2), rgp%buffer_cplx(:, ig2), gpu_mode=gpu_mode)
     else
       call uplan_q%execute_gr(ndat, wc_qbz%buffer_cplx(:, ig2), rgp%buffer_cplx(:, ig2), phase_r=ceiqr, gpu_mode=gpu_mode)
     end if
   end do ! ig2
...
!Local variables-------------------------------
!scalars
 integer :: ig2, npw, nr, col_bsize, ir1, ndat, gpu_option, gpu_mode
 character(len=500) :: msg, gpu_action
...
! *************************************************************************
 ...
 if (any(g0_q /= 0)) then
   ABI_MALLOC(ceig0r, (gwr%g_nfft))
   ABI_MALLOC(conjg_ceig0r, (gwr%g_nfft))
   call calc_ceigr(-g0_q, gwr%g_nfft, 1, gwr%g_ngfft, ceig0r)
   conjg_ceig0r = conjg(ceig0r)
#ifdef HAVE_OPENMP_OFFLOAD
   !$OMP TARGET ENTER DATA MAP(alloc:ceig0r, conjg_ceig0r) IF (gpu_option == ABI_GPU_OPENMP)
   !$omp target update to(ceig0r, conjg_ceig0r) if (gpu_option == ABI_GPU_OPENMP)
 ...
 npw = desc_qbz%npw
 ABI_CHECK(block_dist_1d(npw, gwr%g_comm%nproc, col_bsize, msg), msg)
 call rgp%init(nr, npw, gwr%g_slkproc, desc_qbz%istwfk, size_blocs=[-1, col_bsize]) ! , gpu_action=gpu_action)

 call uplan_k%init(desc_qbz%npw, 1, gwr%uc_batch_size, gwr%g_ngfft, desc_qbz%istwfk, &
 ...
 do ig2=1,wc_ggp%size_local(2), gwr%uc_batch_size
   ...
   if (any(g0_q /= 0)) then
     call uplan_k%execute_gr(ndat, wc_ggp%buffer_cplx(:,ig2), rgp%buffer_cplx(:,ig2), &
                             phase_r=ceig0r, gpu_mode=gpu_mode)
   else
     call uplan_k%execute_gr(ndat, wc_ggp%buffer_cplx(:,ig2), rgp%buffer_cplx(:,ig2), &
                             gpu_mode=gpu_mode)
   end if
...
!! INPUTS
!!  what: "tchi" or "wc"
!!  mode: "iw2t" for omega to tau or "it2w" for tau to omega.
!!  [sum_spins]: True to sum the two spin-channgels when what == "tchi". Default: False.
!!
!! SOURCE
...
! *************************************************************************
 ...
 if (sum_spins_) then  ! gwr%nsppol == 2 .and.
...
!arrays
 complex(gwp),intent(in) :: cg(desc%npw, ndat)
...
subroutine gwr_print_mem(gwr, units)
...
! *********************************************************************
 ...
 if (allocated(gwr%gt_kibz)) then
   ...
   if (mem_mb > zero) then
     call wrtout(units, sjoin("- Local memory for G(g,g',kibz,itau): ", ftoa(mem_mb, fmt="f8.1"), ' [Mb] <<< MEM'))
   end if
 end if
 if (allocated(gwr%tchi_qibz)) then
   mem_mb = sum(slk_array_locmem_mb(gwr%tchi_qibz))
   if (mem_mb > zero) then
     call wrtout(units, sjoin("- Local memory for Chi(g,g',qibz,itau): ", ftoa(mem_mb, fmt="f8.1"), ' [Mb] <<< MEM'))
   end if
 end if
 if (allocated(gwr%wc_qibz)) then
   mem_mb = sum(slk_array_locmem_mb(gwr%wc_qibz))
   if (mem_mb > zero) then
     call wrtout(units, sjoin("- Local memory for Wc(g,g,qibz,itau): ", ftoa(mem_mb, fmt="f8.1"), ' [Mb] <<< MEM'))
   end if
 end if
 if (allocated(gwr%sigc_kibz)) then
   mem_mb = sum(slk_array_locmem_mb(gwr%sigc_kibz))
   if (mem_mb > zero) then
     call wrtout(units, sjoin("- Local memory for Sigma_c(g,g',kibz,itau): ", ftoa(mem_mb, fmt="f8.1"), ' [Mb] <<< MEM'))
   end if
 end if
 if (allocated(gwr%ugb)) then
   mem_mb = sum(slk_array_locmem_mb(gwr%ugb))
   if (mem_mb > zero) then
     call wrtout(units, sjoin('- Local memory for u_gb wavefunctions: ', ftoa(mem_mb, fmt="f8.1"), ' [Mb] <<< MEM'))
   end if
 end if
 call wrtout(units, " ")
...
!Local variables-------------------------------
!scalars
 integer :: my_is, my_it, my_ikf, ig, my_ir, my_nr, nr, npw, ncol_glob, col_bsize, my_iqi, gt_scbox_win, gpu_option, iab
 ...
 type(c_ptr) :: void_ptr
!arrays
 integer :: sc_ngfft(18), gg(3), g0_kq(3), mask_qibz(gwr%nqibz), need_kibz(gwr%nkibz), got_kibz(gwr%nkibz), units(2)
 integer,allocatable :: green_scgvec(:,:), chi_scgvec(:,:)
 logical :: select_my_kbz(gwr%my_nkbz)
...
! *************************************************************************
 ...
 if (gwr%use_supercell_for_tchi) then
   ...
   if (use_shmem_for_k) then
     ...
     call gwr%kpt_comm%allocate_shared_master(buf_count, gwp, xmpi_info_null, void_ptr, gt_scbox_win)
   ...
   mem_mb = (sc_nfftsp * max_ndat * 2 * gwp) * b2Mb
   ...
   call green_plan%from_ngfft(sc_ngfft, max_ndat*2, gpu_option)
   ...
   ABI_MALLOC(cemiqr, (gwr%g_nfft)) ! The phase e^{-iq.r} in the unit cell.
#ifdef HAVE_OPENMP_OFFLOAD
  !$OMP TARGET ENTER DATA MAP(alloc:cemiqr) IF (gpu_option == ABI_GPU_OPENMP)
   ...
   do my_iqi=1,gwr%my_nqibz
     iq_ibz = gwr%my_qibz_inds(my_iqi)
     npw = gwr%tchi_desc_qibz(iq_ibz)%npw
     ncol_glob = gwr%g_nfft
     ABI_CHECK(block_dist_1d(ncol_glob, gwr%g_comm%nproc, col_bsize, msg), msg)
     call chiq_gpr(my_iqi)%init(npw, gwr%g_nfft, gwr%g_slkproc, 1, size_blocs=[-1, col_bsize])
   ...
   do my_is=1,gwr%my_nspins
     ...
     do my_it=1,gwr%my_ntau
       ...
       do my_iqi=1,gwr%my_nqibz
         ...
         call calc_ceikr(-gwr%qibz(:,iq_ibz), gwr%g_ngfft, gwr%g_nfft, 1, cemiqr)
         ...
         call uplan_q%init(desc_q%npw, 1, gwr%uc_batch_size, gwr%g_ngfft, istwfk1, &
                           desc_q%gvec, gwp, gpu_option)
  ...
  !$OMP TARGET EXIT DATA MAP(delete:cemiqr) IF (gpu_option == ABI_GPU_OPENMP)
   ...
   nr = gwr%g_nfft
   col_bsize = nr / gwr%g_comm%nproc; if (mod(nr, gwr%g_comm%nproc) /= 0) col_bsize = col_bsize + 1
   ...
   do iq_ibz=1,gwr%nqibz
     call chiq_rpr(iq_ibz)%init(nr, nr, gwr%g_slkproc, 1, size_blocs=[-1, col_bsize], gpu_action=gpu_action)
   end do
   ...
   do ipm=1,2
     do iab=1,gwr%nsig_ab
       call gk_rpr_pm(ipm, iab)%init(nr, nr, gwr%g_slkproc, 1, size_blocs=[-1, col_bsize]) !, gpu_action=gpu_action)
       call gkq_rpr_pm(ipm, iab)%init(nr, nr, gwr%g_slkproc, 1, size_blocs=[-1, col_bsize]) !, gpu_action=gpu_action)
     end do
   end do
   ...
   do my_it=1,gwr%my_ntau
     ...
     do iq_ibz=1,gwr%nqibz
       ...
       ! Here we symmetrize chi0 to get the integral in the BZ.
 ...
 end if ! super cell or mixed space algorithm.
...
!Local variables-------------------------------
 ...
 complex(gwp),contiguous, pointer :: ck_ptr(:,:)
! *************************************************************************
...
!Local variables-------------------------------
 ...
 complex(gwp),contiguous, pointer :: cq_ptr(:,:)
! *************************************************************************
...
subroutine gwr_print_trace(gwr, units, what)
 ...
 case ("tchi_qibz", "wc_qibz")
   ...
   if (gwr%comm%me == master) then
     do spin=1,gwr%nsppol
       ...
       call print_arr(units, ctrace3(:,:,spin))
     end do
   end if
   ABI_FREE(ctrace3)
         ...
         do il_g1=1,wc%size_local(1)
           ...
           if (iglob1 == iglob2) then
             ...
             if (iglob1 == ig0 .and. iglob2 == ig0) then
               ! Store epsilon_{iw, iq_ibz}(0, 0). Rescale by np_qibz because we will MPI reduce this array.
...
!scalars
 ...
 integer :: isym_k, trev_k, g0_k(3), tsign_k !, b1gw, b2gw, ! my_iqi, sc_ir, ig, my_iqf
 ...
 real(dp) :: max_abs_imag_wct, max_abs_re_wct, sck_ucvol, scq_ucvol, beta_r
 real(gwp) :: wtqm, wtqp
...
if (gwr%use_supercell_for_sigma) then
 ...
 if (use_shmem_for_k) then
   ...
   call gwr%kpt_comm%allocate_shared_master(buf_count, gwp, xmpi_info_null, void_ptr, wct_scbox_win)
   call c_f_pointer(void_ptr, wct_scbox, shape=[sc_nfftsp, max_ndat])
 end if

 call wrtout(std_out, sjoin(" use_mpi_for_k:", yesno(use_mpi_for_k)))
 call wrtout(std_out, sjoin(" use_shmem_for_k:", yesno(use_shmem_for_k)))
 mem_mb = 3 * (sc_nfftsp * max_ndat * gwp) * b2Mb
 ...
 call green_plan%from_ngfft(sc_ngfft, max_ndat*2, gpu_option)
 call wt_plan%from_ngfft(sc_ngfft, max_ndat, gpu_option)
...
if (.not. use_shmem_for_k) then
 ...
 call wc_rpr%init(nr, nr, gwr%g_slkproc, 1, size_blocs=[-1, col_bsize])
 ...
 do my_is=1,gwr%my_nspins
   ...
   ! TODO: MPI distribute or use MPI shared memory
   bmin = minval(gwr%bstart_ks(:, spin)); bmax = maxval(gwr%bstop_ks(:, spin))
   ABI_MALLOC_OR_DIE(uc_psir_bk, (nr * gwr%nspinor, bmin:bmax, gwr%nkcalc), ierr)
#ifdef HAVE_OPENMP_OFFLOAD
   !$OMP TARGET ENTER DATA MAP(alloc:uc_psir_bk) IF (gpu_option == ABI_GPU_OPENMP)
#endif

   !! !$OMP PARALLEL DO COLLAPSE(2) PRIVATE(ikcalc_ibz)
   do ikcalc=1,gwr%nkcalc
     do band=gwr%bstart_ks(ikcalc, spin), gwr%bstop_ks(ikcalc, spin)
       ikcalc_ibz = gwr%kcalc2ibz(ikcalc, 1)  ! NB: Assuming wfs in IBZ
       call gwr%kcalc_wfd%get_ur(band, ikcalc_ibz, spin, uc_psir_bk(:, band, ikcalc))
     end do
   end do
#ifdef HAVE_OPENMP_OFFLOAD
   !$omp target update to(uc_psir_bk) if (gpu_option == ABI_GPU_OPENMP)
#endif
   ...
   do my_it=1,gwr%my_ntau
     ...
     do my_ikf=1,gwr%my_nkbz
       ...
       ! Skip this BZ k-point if it is not in the IBZ(ikcalc) of some ikcalc.
       ...
       do ikcalc=1,gwr%nkcalc
         ...
         wtqp = one / gwr%nkbz; wtqm = zero
         if (gwr%dtset%symsigma /= 0) then
           ! If symsigma, sum only q"s in IBZ_k, then symmetrize the matrix elements.
 ...
 if (gwr%comm%me == 0) then
   ...
   do spin=1,gwr%nsppol
     do ikcalc=1,gwr%nkcalc
       ...
       ! Compute gaps assuming KS band indices.
...
! *************************************************************************
 ...
 ! Use ugb wavefunctions and the Lehmann representation to compute head/wings.
 call gwr%build_chi0_head_and_wings()

 ! Now compute G(itau) from ugb and start the GWR algorithm for chi.
 free_ugb__ = .True.; if (present(free_ugb)) free_ugb__ = free_ugb
 call gwr%build_green(free_ugb=free_ugb__)
 call gwr%build_tchi()
...
!arrays
 ...
 complex(gwp),intent(in) :: cg(npw * ndat)
 complex(gwp),target,intent(inout) :: cfft(ngfft(4)*ngfft(5)*ngfft(6)*ndat)

!Local variables-------------------------------
 integer :: n1, n2, n3, n4, n5, n6, i1, i2, i3, idat, ipw
 complex(gwp),contiguous,pointer :: cfft_ptr(:,:,:,:)
! *************************************************************************
...
!!
!! INPUTS
!! op_type: OP_COPY to transfer data, OP_ACC to transfer and accumulate
!! ngfft:
!!   n1,n2,n3=physical dimension of the FFT box
!!   n4,n5,n6=memory dimension of cfft
!! npw=number of G vectors in basis at this k point
...
!!
!! SIZE
!! cg(npw*ndat)= contains values for npw G vectors in basis sphere. See also op_type
!!
!! SOURCE

subroutine box2gsph(op_type, ngfft, npw, ndat, kg_k, cfft, cg)

!Arguments ------------------------------------
!scalars
 integer,intent(in) :: op_type, ngfft(6), npw, ndat
!arrays
 integer,intent(in) :: kg_k(3, npw)
 complex(gwp),target,intent(in) :: cfft(ngfft(4)*ngfft(5)*ngfft(6)*ndat)
 complex(gwp),intent(inout) :: cg(npw*ndat)
...
! *************************************************************************
 ...
 case (OP_ACC)
   ! Extract cg from cfft, ignoring components outside range of cg sphere and accumulate
   !$OMP PARALLEL DO PRIVATE(i1, i2, i3, icg) IF (ndat > 1)
   do idat=1,ndat
     do ipw=1,npw
       i1 = modulo(kg_k(1, ipw), n1) + 1
       ...
       !end if
       icg = ipw + (idat - 1) * npw
       cg(icg) = cg(icg) + cfft_ptr(i1, i2, i3, idat)
     end do
   end do

 case default
   ABI_BUG(sjoin("Invalid op_type:" ,itoa(op_type)))
 end select
...
!arrays
 ...
 real(dp), allocatable :: ddk_ug1(:,:,:), cg2_dp(:,:)
 ...
 do my_is=1,gwr%my_nspins
   ...
   do my_ikf=1,gwr%my_nkbz
     ...
     do band1_start=1, gwr%ugb_nband, block_size
       ...
       ! Precompute DH_DK |psi_b1>.
       ABI_MALLOC(ddk_ug1, (2, npw_ki * nspinor, 3))
...
! *************************************************************************
 ...
 do ikcalc=1,gwr%nkcalc ! TODO: Should be spin dependent!
   ...
   !if (gwr%comm%me == 0) call ltg_k%print([std_out], prtvol=gwr%dtset%prtvol)
     ...
     do ikcalc=1,gwr%nkcalc
       ...
       do band=gwr%bstart_ks(ikcalc, spin), gwr%bstop_ks(ikcalc, spin)
         write(msg,'(i5, *(f9.3))') &
           ...
           real(gwr%sigx_mat(band, merge(1, band, gwr%sig_diago), ikcalc, spin), kind=dp) * Ha_eV, & ! SigX
           gwr%qp_ebands%occ(band, ik_ibz, spin)                          ! Occ(E)
         call ydoc%add_tabular_line(msg)
       end do
...
!!  kpts(3,nkpt)=K-points in reduced coordinates.
!!
!! OUTPUT
!!  ph1d(maxval(sc_shape), 3, nkpt))
!!
!! SOURCE
...
!Arguments ------------------------------------
 ...
 complex(gwp),allocatable,intent(out) :: ph1d(:,:,:)
...
! *************************************************************************
 ...
 do ikpt=1,nkpt
   ...
   do ix=0,sc_shape(1) - 1
     arg = fact * ix
     ph1d(ix + 1, 1, ikpt) = cmplx(cos(arg), sin(arg), kind=gwp)
   end do
   fact = two_pi * kk(2)
   do iy=0,sc_shape(2) - 1
     arg = fact * iy
     ph1d(iy + 1, 2, ikpt) = cmplx(cos(arg), sin(arg), kind=gwp)
   end do
   fact = two_pi * kk(3)
   do iz=0,sc_shape(3) - 1
     arg = fact * iz
     ph1d(iz + 1, 3, ikpt) = cmplx(cos(arg), sin(arg), kind=gwp)
   end do
 end do ! ikpt
...
!Arguments ------------------------------------
 ...
 complex(gwp),target,intent(in) :: &
    sc_data(uc_ngfft(1)*sc_shape(1)*uc_ngfft(2)*sc_shape(2)*uc_ngfft(3)*sc_shape(3))
...
! *************************************************************************
 ...
 call c_f_pointer(c_loc(uc_psi), uc_psi_ptr, shape=[uc_n1, uc_n2, uc_n3])
     ...
     do iz=1,uc_n3
       do il2=1,sc_shape(2)
         do iy=1,uc_n2
           do il1=1,sc_shape(1)
             do ix=1,uc_n1
               cout = cout + uc_psi_ptr(ix, iy, iz) * sc_data_ptr(ix, il1, iy, il2, iz, il3)
             end do
           end do
         end do
       end do
       ...
       do il2=1,sc_shape(2)
         ...
         do iy=1,uc_n2
           do il1=1,sc_shape(1)
             ...
             do ix=1,uc_n1
               cout = cout + cphase * uc_psi_ptr(ix, iy, iz) * sc_data_ptr(ix, il1, iy, il2, iz, il3)
             end do
           end do
         end do
       end do
