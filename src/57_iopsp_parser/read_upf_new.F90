...
CONTAINS
  ...
  SUBROUTINE read_pp_header_schema ( upf )
    ...
    call remove_nobreak_space(upf%dft)
...
subroutine remove_nobreak_space(in_str)
 ...
 cnt = 0
 !do ii=1,len_trim(in_str) - 1
 tmp_str = ""
 do ii=1,len(in_str)
   if ((iachar(in_str(ii:ii)) >= 127)) cycle
   !if ((iachar(in_str(ii:ii)) == 194 .and. iachar(in_str(ii+1:ii+1)) == 160) &
   !    .or. iachar(in_str(ii:ii)) == 160) cycle
   cnt = cnt + 1
   print *, "copying`", in_str(ii:ii), "` with iachar:", iachar(in_str(ii:ii))
...
end subroutine remove_nobreak_space
