...
module m_respfn_driver
 ...
 use m_drivexc,     only : check_kxc, xc_need_kden
...
!Local variables-------------------------------
 ...
 real(dp) :: rmet(3,3),rprimd(3,3),rprimd_for_kg(3,3),strn_dummy6(6),strv_dummy6(6),tsec(2)
...
!Structured debugging if dtset%prtvol==-level
 if(dtset%prtvol==-level)then
   write(msg,'(80a,a,a)')  ('=',ii=1,80),ch10,' respfn : enter , debug mode '
...
!Set up the Ylm for each k point
 ...
 if (rfstrs/=0.or.pawbec==1.or.pawpiezo==1.or.dtset%efmas>0) then
   ...
   ABI_MALLOC(ylmgr,(0,0,psps%useylm))
 end if
...
!  4-Allocate PAW specific arrays
   ...
   call paw_an_init(paw_an,dtset%natom,dtset%ntypat,nkxc1,0,dtset%nspden,&
        &   cplex,dtset%pawxcdev,dtset%typat,pawang,pawtab,has_vxc=1,has_vhartree=has_vhartree,has_vxctau=usevxctau_paw,&
...
!PAW: compute Dij quantities (psp strengths)
 if (psps%usepaw==1)then
   ...
   call pawdenpot(compch_sph,el_temp,epaw,epawdc,spaw,gprimd,ipert,dtset%ixc,my_natom,natom,dtset%nspden,&
...
!-----2. Frozen-wavefunctions and Ewald(q=0) parts of 2DTE
 ...
 ABI_MALLOC(dyfrwf,(dyfr_cplex,3,3,natom,1+(natom-1)*dyfr_nondiag))
 ABI_MALLOC(becfrnl,(3,natom,3*pawbec))
 ...
 ABI_MALLOC(eltcore,(6,6))
 ABI_MALLOC(elteew,(6+3*natom,6))
 ABI_MALLOC(eltfrhar,(6,6))
 ABI_MALLOC(eltfrnl,(6+3*natom,6))
 ABI_MALLOC(eltfrloc,(6+3*natom,6))
 ABI_MALLOC(eltfrkin,(6,6))
 ABI_MALLOC(eltfrxc,(6+3*natom,6))
 ABI_MALLOC(eltvdw,(6+3*natom,6*usevdw))
 eltcore(:,:)=zero
 elteew(:,:)=zero
 eltfrnl(:,:)=zero
 eltfrloc(:,:)=zero
...
!  Verify that k-point set has full space-group symmetry; otherwise exit
   timrev=1
   if (symkchk(dtset%kptns,dtset%nkpt,dtset%nsym,symrec,timrev,msg) /= 0) then
     ABI_ERROR(msg)
   end if
...
!-----3. Initialisation of 1st response, taking into account the q vector.
 ...
 write(msg,'(3a)')ch10,' ==>  initialize data related to q vector <== ',ch10
...
!    Temporarily, ...
     write(msg, '(a,a,a,3es16.6,a,a,5(a,i2),a,a,a)' )ch10,&
...
&     'Action: change qpt, or rfelfd, or rfstrs in the input file.'
   ...
   if(chneut/=0)then
     write(msg,'(2a)')ch10,' chneut/=0 not allowed with q/=0 => chneut reset to 0 locally.'
     ABI_WARNING(msg)
     chneut=0
   end if
   if(asr/=0)then
     write(msg,'(2a)')ch10,' asr/=0 not allowed with q/=0 => asr reset to 0 locally.'
     ABI_WARNING(msg)
     asr=0
   end if
   ...
   do idir=1,3
     if(rfpert(ipert)==1.and.rfdir(idir)==1)then
       if( pertsy(idir,ipert)==1 )then
         write(msg, '(i5,a,i2,a,i4)' )ii,')    idir=',idir,'    ipert=',ipert
         call wrtout([std_out, ab_out] ,msg)
         ii=ii+1
       end if
     end if
...
&   ' You may have forgotten to set the rfdir or rfatpol variables. Continuing normally.',ch10
 ...
 if (dtset%prepanl==1.and.(rf2_dkdk/=0 .or. rf2_dkde/=0)) then
   ABI_MALLOC(rfpert_nl,(3,natom+2,3,natom+2,3,natom+2))
   rfpert_nl = 0
...
&   ch10,' optical susceptibility tensors is:',ch10
   ...
   write(msg,'(12x,a)')'i1pert  i1dir   i2pert  i2dir   i3pert  i3dir'
   call wrtout(std_out,msg)
   n1 = 0
           ...
           do i3pert = 1, natom + 2
             do i3dir = 1,3
               if (rfpert_nl(i1dir,i1pert,i2dir,i2pert,i3dir,i3pert)==1) then
                 n1 = n1 + 1
                 write(msg,'(2x,i4,a,6(5x,i3))') n1,')', i1pert,i1dir,i2pert,i2dir,i3pert,i3dir
                 call wrtout(std_out,msg)
   ...
   do i1dir = 1, 3
     do i2dir = 1, 3
       write(msg,'(3(a,i1))') ' ',i1dir,' ',i2dir,' : ',rf2_dirs_from_rfpert_nl(i1dir,i2dir)
       call wrtout(std_out,msg)
     end do
   end do
 end if
...
!   call sylwtens(indsym,natom+8,natom,dtset%nsym,rfpert_lw,symrec,dtset%symrel,symrel_cart)
   ...
   write(msg,'(7a)') ch10, ' The following reducible perturbations will also be ', ch10, &
             ...
             do i1dir = 1, 3
               if (rfpert_lw(i1dir,i1pert,i2dir,i2pert,i3dir,i3pert)==1) then
                 if (pertsy(i1dir,i1pert)==-1) then
                   ...
                   write(msg,'(a,i2,a,i4)' )'    idir=',i1dir,'    ipert=',i1pert
                   call wrtout(ab_out,msg,'COLL')
                   call wrtout(std_out,msg,'COLL')
                 end if
                 if (pertsy(i2dir,i2pert)==-1) then
                   pertsy(i2dir,i2pert)=1
                   write(msg,'(a,i2,a,i4)' )'    idir=',i2dir,'    ipert=',i2pert
                   call wrtout(ab_out,msg,'COLL')
                   call wrtout(std_out,msg,'COLL')
                 end if
               end if
...
!Deallocate the arrays that were needed only for the frozen wavefunction part
 ...
 ABI_MALLOC(d2eig0,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2k0,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2lo,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2loc0,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2nfr,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2nl,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2nl0,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2nl1,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2vn,(2,3,mpert,3,mpert))
 ABI_MALLOC(d2ovl,(2,3,mpert,3,mpert*psps%usepaw))
 blkflg(:,:,:,:)=0
 ...
 prtbbb=dtset%prtbbb
 ABI_MALLOC(d2bbb,(2,3,3,mpert,dtset%mband,dtset%mband*prtbbb))
 ABI_MALLOC(d2cart_bbb,(2,3,3,mpert,dtset%mband,dtset%mband*prtbbb))
 if(prtbbb==1)then
   d2cart_bbb(:,:,:,:,:,:)=zero
   d2bbb(:,:,:,:,:,:)=zero
 end if
 ...
 ABI_MALLOC(eig2nkq,(2,dtset%mband*dtset%nsppol,dtset%nkpt,3,natom,3,natom*dim_eig2nkq))
 dim_eigbrd=0
...
!  #######################################################################
   ...
   ABI_MALLOC(clflg,(3,mpert))
   if(dtset%ieig2rf > 0) then
     ABI_MALLOC(eigen0_pert,(dtset%mband*dtset%nkpt*dtset%nsppol))
     ABI_MALLOC(eigenq_pert,(dtset%mband*dtset%nkpt*dtset%nsppol))
     ABI_MALLOC(occ_rbz_pert,(dtset%mband*dtset%nkpt*dtset%nsppol))
   end if
...
!  frequencies.
   ...
   write(msg,'(a,a)' )ch10,' ==> Compute Derivative Database <== '
   call wrtout([std_out, ab_out], msg)
...
!  In case of bbb decomposition
   if(prtbbb==1)then
     ABI_MALLOC(blkflg1,(3,mpert,3,mpert))
     ABI_MALLOC(blkflg2,(3,mpert,3,mpert))
     blkflg2(:,:,:,:) = blkflg(:,:,:,:)
     do ipert = 1, mpert
       do ipert2 = 1, mpert
     ...
     ABI_MALLOC(d2tmp,(2,3,mpert,3,mpert))
...
&             occ_rbz_pert,hdr0)
     ...
     if (dtset%ieig2rf > 0) then
       ABI_FREE(eigen0_pert)
       ABI_FREE(eigenq_pert)
       ABI_FREE(occ_rbz_pert)
       ABI_FREE(eigen1_pert)
       call hdr0%free()
       if ((dtset%getwfkfine /= 0 .and. dtset%irdwfkfine ==0) .or.&
&       (dtset%getwfkfine == 0 .and. dtset%irdwfkfine /=0) )  then
         call hdr_fine%free()
         ABI_FREE(eigenq_fine)
       end if
...
!      Compute and print the T=0 Fan, and possibly DDW contributions to the eigenenergies.
       if(dtset%ieig2rf > 0) then
         write(msg, '(80a,9a)' ) ('=',mu=1,80),ch10,ch10,&
...
&         ' Print first the electronic eigenvalues, then the q-dependent Fan shift of eigenvalues.'
         ...
         if(qeq0)then
           write(msg, '(a)' )' Phonons at gamma, also compute the Diagonal Debye-Waller shift of eigenvalues.'
           call wrtout([std_out, ab_out], msg)
         end if
...
!        Compute and print Fan contribution
         ...
         ABI_MALLOC(eigen_fan_mean,(dtset%mband*dtset%nkpt*dtset%nsppol))
...
!          Compute and print Diagonal Debye-Waller contribution
           ...
           ABI_MALLOC(eigen_ddw_mean,(dtset%mband*dtset%nkpt*dtset%nsppol))
           if(qeq0)then
...
&             dtset%mband,natom,dtset%nkpt,dtset%nsppol,2,phfrq,dtset%prtvol)
             if(results_respfn%gam_jdtset == -dtset%jdtset)then
               ...
               ABI_MALLOC(results_respfn%gam_eig2nkq,(2,sz1,dtset%nkpt,3,natom,3,sz2))
...
&           eigen0,zero,zero,dtset%wtk)
           ...
           ABI_FREE(eigen_fanddw)

         end if

         ABI_FREE(eigen_fan)
         ABI_FREE(eigen_fan_mean)
       end if
...
!This feature is disabled in the PAW case
 ...
 if(prtbbb==1)then
   ABI_MALLOC(loctenscart_bbb,(2,3,3,mband,mband*prtbbb))
...
!Transform the tensor to cartesian coordinates
 ...
 if (prtbbb == 1) then
   ...
   write(msg,'(a,a)')ch10, ' Band by band decomposition of the localisation tensor (bohr^2)'
   call wrtout([std_out, ab_out], msg)
...
! GA: As much as I can tell, the option outd2 is always set to 1. This variable should be removed
             ...
             do idir2=1,3
               if(rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 if(ipert1==ipert2)then
                   write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,dyfrlo(idir1,idir2,ipert2),zero
                 else
                   write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,zero,zero
                 end if
               end if
             end do
           end do
...
!    Now the nonlinear xc core correction(1) part
     write(iout,*)' '
     write(iout,*)' Frozen wf xc core (1) part',' of the dynamical matrix'
     write(iout,*)'    j1       j2             matrix element'
     write(iout,*)' dir pert dir pert     real part   imaginary part'
     do ipert1=1,natom
       do idir1=1,3
...
!    Now the nonlinear xc core correction(2) part
     write(iout,*)' '
     write(iout,*)' Frozen wf xc core (2) part',' of the dynamical matrix'
     write(iout,*)'    j1       j2             matrix element'
     write(iout,*)' dir pert dir pert     real part   imaginary part'
     do ipert1=1,natom
       do idir1=1,3
         if ( (rfpert(ipert1)==1.and.rfdir(idir1)==1) .or. outd2==1) then
           ...
           do ipert2=1,natom
             do idir2=1,3
               if(rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 if(ipert1==ipert2)then
                   write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,dyfrx2(idir1,idir2,ipert2),zero
                 else
                   write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,zero,zero
                 end if
               end if
   ...
   if (pawpiezo==1.and.(rfpert(natom+2)==1.or.rfstrs/=0).and.outd2==1)then
     ...
     do idir1=1,3
       ...
       do ipert2=natom+3,natom+4
         do idir2=1,3
           write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,piezofrnl(ii,idir1),zero
           ii=ii+1
         end do
       end do
     end do
   ...
   if (pawbec==1.and.(rfpert(natom+2)==1.or.rfphon==1).and.outd2==1)then
     ...
     do ipert2=1,natom
       do idir1=1,3
         write(iout,*)' '
         do idir2=1,3
           write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,becfrnl(idir2,ipert2,idir1),zero
         end do
       end do
     end do
   end if
           ...
           do ipert2=natom+3,natom+4
             do idir2=1,3
               if (rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 ...
                 write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,elteew(ii,jj),zero
               end if
             end do
           end do
         end if
         ...
         if ( (rfpert(ipert1)==1.and.rfdir(idir1)==1) .or. outd2==1) then
           ...
           do ipert2=natom+3,natom+4
             do idir2=1,3
               if (rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 jj=idir2+3*(ipert2-natom-3)
                 write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2, eltfrloc(ii,jj),zero
               end if
             end do
           end do
         end if
         ...
         if ( (rfpert(ipert1)==1.and.rfdir(idir1)==1) .or. outd2==1) then
           ...
           do ipert2=natom+3,natom+4
             do idir2=1,3
               if (rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 jj=idir2+3*(ipert2-natom-3)
                 write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2, eltfrnl(ii,jj),zero
               end if
             end do
           end do
         end if
         ...
         if ( (rfpert(ipert1)==1.and.rfdir(idir1)==1) .or. outd2==1) then
           ...
           do ipert2=natom+3,natom+4
             do idir2=1,3
               if (rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 jj=idir2+3*(ipert2-natom-3)
                 write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2, eltfrxc(ii,jj),zero
               end if
             end do
           end do
         end if
         ...
         if ( (rfpert(ipert1)==1.and.rfdir(idir1)==1) .or. outd2==1) then
           ...
           do ipert2=natom+3,natom+4
             do idir2=1,3
               if (rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 jj=idir2+3*(ipert2-natom-3)
                 write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,eltfrkin(ii,jj),zero
               end if
             end do
           end do
         end if
         ...
         if ( (rfpert(ipert1)==1.and.rfdir(idir1)==1) .or. outd2==1) then
           ...
           do ipert2=natom+3,natom+4
             do idir2=1,3
               if (rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 jj=idir2+3*(ipert2-natom-3)
                 write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2, eltfrhar(ii,jj),zero
               end if
             end do
           end do
         end if
         ...
         if ( (rfpert(ipert1)==1.and.rfdir(idir1)==1) .or. outd2==1) then
           ...
           do ipert2=natom+3,natom+4
             do idir2=1,3
               if (rfpert(ipert2)==1.and.rfdir(idir2)==1)then
                 jj=idir2+3*(ipert2-natom-3)
                 write(iout,'(2(i4,i5),2(1x,f20.10))')idir1,ipert1,idir2,ipert2,eltcore(ii,jj),zero
               end if
             end do
           end do
         end if
...
!  -----------------------------------------------------------------------
   option=4
   ABI_MALLOC(dyfrlo_tmp1,(3,3,natom))
...
!Symmetrize local part of the dynamical matrix dyfrlo:
 ABI_MALLOC(dyfrsym_tmp,(1,3,3,natom,1))
 ABI_MALLOC(dyfrlo_tmp2,(1,3,3,natom,1))
...
!Zero out the output arrays :
 ...
 ABI_MALLOC(vxc10,(cplex*nfft,nspden))
...
!    Compute the derivative of the core charge with respect to j1
     ABI_MALLOC(xcccwk1,(cplex*n3xccc))
...
!          Compute the derivative of the core charge with respect to j2
           ABI_MALLOC(xcccwk2,(cplex*n3xccc))
