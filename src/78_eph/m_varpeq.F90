...
!! SOURCE
 ...
 type, public :: polstate_t
  ...
  integer, allocatable :: my_states_mask(:,:)
   ! (gqk%nb, gqk%my_nk)
   ! Energy filtering mask for electronic states trated by this MPI proc
   ! Relevant if energy filtering is used in gstore

  integer, allocatable :: states_mask_glob(:,:)
   ! (gqk%nb, gqk%glob_nk)
...
!----------------------------------------------------------------------
 ...
 do my_is=1,gstore%my_nspins
   ...
   ABI_MALLOC(polstate%states_mask_glob, (gqk%nb, gqk%glob_nk))
   polstate%my_states_mask(:,:) = 1
...
!----------------------------------------------------------------------
 ...
 ! integer
 ABI_SFREE(self%my_states_mask)
 ABI_SFREE(self%states_mask_glob)
 ! real
...
!----------------------------------------------------------------------
 ...
 do my_ik=1,gqk%my_nk
   do ib=1,gqk%nb
     if (self%my_states_mask(ib, my_ik) == 0) then
...
!scalars
 ...
 integer :: my_rank, ib, irsp, ir, uc_idx, mpw, band, bstart, ir1, ir2, ir3, wp1, wp2, wp3, ds_nfft
...
!arrays
 ...
 complex(gwpc) :: ceikr_
 complex(gwpc),allocatable :: ur_k(:,:), ds_ur_k(:,:), pol_wf(:,:,:), sc_ceikr(:)
!----------------------------------------------------------------------
 ...
 ABI_MALLOC(sc_ceikr, (sc_nfft*nspinor))
 ...
 do spin=1,nsppol
   ...
   do ik=1, nk
     ...
     kk_sc = kk * vpq%ngkpt
     call calc_ceikr(kk_sc, sc_ngfft, sc_nfft, nspinor, sc_ceikr)
               ...
               do ir3=0,sc_ngfft(3)-1
                 do ir2=0,sc_ngfft(2)-1
                   do ir1=0,sc_ngfft(1)-1
                     ...
                     pol_wf(irsp, ip, spin) = pol_wf(irsp, ip, spin) + a_nk * ur_k(uc_idx,1) * sc_ceikr(irsp)
                     ...
                     !pol_wf(irsp, ip, spin) = pol_wf(irsp, ip, spin) + a_nk * ur_k(uc_idx,1) * ceikr_
                   end do
                 end do
               end do
            else
               ...
               do ir3=0,sc_ngfft(3)-1
                 do ir2=0,sc_ngfft(2)-1
                   do ir1=0,sc_ngfft(1)-1
                     ...
                     ! Note ds_ur_k here
                     pol_wf(irsp, ip, spin) = pol_wf(irsp, ip, spin) + a_nk * ds_ur_k(uc_idx,1) * sc_ceikr(irsp)
