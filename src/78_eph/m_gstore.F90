...
type, public :: gqk_t
  ...
  ! e-ph matrix elements g (local buffer)
 ...
 contains
  ...
  procedure :: mykpt => gqk_mykpt
  ! Return the k-point from my local index my_ik

  procedure :: my_kweight => gqk_my_kweight
  ! Return the k-point weight from my local index my_ik
  ...
  !procedure :: ncread_path => gqk_ncread_path
...
type, public :: gstore_t
  ...
  !integer :: kptrlatt(3, 3) = -1, qptrlatt(3, 3) = -1
   ! k-mesh and q-mesh
...
!----------------------------------------------------------------------
 ...
 call gstore%krank%get_mapping(gstore%nkbz, kbz, dksqmax, cryst%gmet, kbz2ibz, &
 ...
 case ("fs_tetra")
   ...
   call gstore%filter_fs_tetra__(qbz, qbz2ibz, qibz2bz, kbz, kibz, kbz2ibz, kibz2bz, &
                                 select_qbz_spin, select_kbz_spin)
 case default
 ...
 call gstore%malloc__(max_nq, qglob2bz, max_nk, kglob2bz, qbz2ibz, kbz2ibz, .False.)
 ...
 if (my_rank == master) then
   ...
   NCF_CHECK(nf90_put_var(ncid, vid("gstore_glob_nq_spin"), glob_nq_spin))
   NCF_CHECK(nf90_put_var(ncid, vid("gstore_glob_nk_spin"), glob_nk_spin))
   ...
   do spin=1,gstore%nsppol
     ...
     if (gstore%with_vk == 1) then
       ...
       ncerr = nctk_def_arrays(spin_ncid, [ &
         nctkarr_t("vkmat_cart", "dp", "two, three, nb, nb, glob_nk") &  ! Global matrix
       ])
       NCF_CHECK(ncerr)
     end if
...
subroutine gstore_malloc__(gstore, max_nq, qglob2bz, max_nk, kglob2bz, qbz2ibz, kbz2ibz, with_gvals)
...
!----------------------------------------------------------------------
 ...
 do my_is=1,gstore%my_nspins
   ...
   if (with_gvals) then
     ...
     ABI_MALLOC_OR_DIE(gqk%my_g, (gqk%cplex, gqk%my_npert, gqk%nb, gqk%my_nq, gqk%nb, gqk%my_nk), ierr)
     ! This initialization is important as not all the g are computed when we filter in k-space.
     ! Abinit postprocessing tools will operate of the full my_g array
     ! and we don't want to trigger floating point exceptions.
     gqk%my_g = zero

     !ABI_MALLOC_OR_DIE(gqk%my_g2, (gqk%cplex, gqk%my_npert, gqk%nb, gqk%my_nq, gqk%nb, gqk%my_nk), ierr)
     !gqk%my_g2 = zero
...
!!
!! FUNCTION
!!  Build array with the indices of the start of the ik_ibz wavevector in the BZ.
!!
!! INPUTS
!!
!! OUTPUT
...
!----------------------------------------------------------------------
 ...
 do my_is=1,gstore%my_nspins
   ...
   ABI_CALLOC(gqk%my_wnuq, (gqk%my_npert, gqk%my_nq))
...
!Local variables-------------------------------
 ...
 type(gqk_t), pointer :: gqk
!arrays
 real(dp),allocatable :: work_kbz(:,:,:), dbl_delta_q(:,:,:)

!----------------------------------------------------------------------

 if (gstore%qzone /= "bz") then
   ABI_ERROR("gstore_get_lambda_iso_iw assumes qzone == `ibz`")
 end if
 ...
 do my_is=1,gstore%my_nspins
   gqk => gstore%gqk(my_is)
   ABI_MALLOC(work_kbz, (gqk%nb, gqk%nb, gstore%nkbz))
   ABI_MALLOC(dbl_delta_q, (gqk%nb, gqk%nb, gqk%my_nk))
   dbl_delta_q = one

   do my_iq=1,gqk%my_nq
    ! FIXME Compute integration weights for the double delta according to different schema.
       ...
       do ib_k=1,gqk%nb
         do ib_kq=1,gqk%nb
           do my_ip=1,gqk%my_npert
             ...
             lambda(:) = lambda(:) + &
               two * w_nuq / (imag_w(:) ** 2 + w_nuq ** 2) * g2 * weight_k * dbl_delta_q(ib_kq, ib_k, my_ik)
           end do
         end do
       end do
     end do
...
!!****f* m_gstore/gqk_my_kweight
!! NAME
!! gqk_my_kweight
!!
!! FUNCTION
!!  Return the weight of the k-point from the local index my_ik
!!
!! INPUTS
!!
!! OUTPUT
...
pure real(dp) function gqk_my_kweight(gqk, my_ik, gstore) result (weight_k)
...
!************************************************************************
 ...
 ! NB: Write ph data here as we are not guaranteed to have all the IBZ q-points
 ...
 do my_is=1,gstore%my_nspins
   ...
   ABI_MALLOC(my_vkdone, (gqk%my_nk))
   my_vkdone = .False.
   ...
   do my_iq=1,gqk%my_nq
     ...
     do my_ik=1,gqk%my_nk
       ...
       call ephtk_gkknu_from_atm(nb, nb, 1, natom, gkk_atm, phfrq, displ_red, gkq_nu)

       ! Save e-ph matrix elements.
       my_gbuf(:,:,:,:, my_ik, iqbuf_cnt) = gkq_nu

       if (gstore%with_vk /= 0 .and. .not. my_vkdone(my_ik)) then
         ! Avoid computing the same k-point multiple times.
         my_vkdone(my_ik) = .True.
     ...
     if (my_iq <= 10 .or. mod(my_iq, 20) == 0) then
       ...
       if (my_iq == 10) then
         ...
         call wrtout(std_out, " >>> Will print next iteration when mod(my_iq, 20) == 0) ...", do_flush=.True.)
       end if
     end if
   end do ! my_iq
...
subroutine dump_data()
 ...
 ! If parallelism over pertubation is ON, only the procs treating the first perturbation
...
function gstore_from_ncpath(path, cplex, dtset, cryst, ebands, ifc, comm) result(gstore)
...
! *************************************************************************
 ...
 if (my_rank == master) then
   ...
   ! TODO Should compuare ebands_file with input ebands
   ...
   if (gstore_cplex == 1 .and. cplex == 2) then
 ...
 ! Set MPI grid
 call gstore%set_mpi_grid__(cplex, glob_nq_spin, glob_nk_spin, dtset%eph_np_pqbks, nproc_spin, comm_spin)

 ! At this point, we have the Cartesian grid (one per spin if any)
 ! and we can finally allocate and distribute other arrays.
 call gstore%malloc__(max_nq, qglob2bz, max_nk, kglob2bz, qbz2ibz, kbz2ibz, .True.)
 ...
 !
 !   In memory, we have: my_g(cplex, my_npert, nb, my_nq, nb, my_nk)
 !   On disk, we have:   nctkarr_t("gvals", "dp", "gstore_cplex, nb, nb, natom3, glob_nk, glob_nq")
 !
 call cwtime(cpu, wall, gflops, "start")
 do spin=1,gstore%nsppol
   ...
   if (my_is /= 0) then
     ...
     do my_iq=1,gqk%my_nq
        iq_glob = my_iq + gqk%my_qstart - 1
        ncerr = nf90_get_var(spin_ncid, spin_vid("gvals"), gwork_q, &
                             start=[1, 1, 1, 1, 1, iq_glob])  ! count=[])
        NCF_CHECK(ncerr)

        ! Put data in the right position.
        do my_ik=1,gqk%my_nk
          ik_glob = my_ik + gqk%my_kstart - 1
          do my_ip=1,gqk%my_npert
            ipert = gqk%my_iperts(my_ip)
            slice_bb = gwork_q(:,:,:, ipert, ik_glob)
            if (cplex == gstore_cplex) then
              gqk%my_g(:, my_ip, :, my_iq, :, my_ik) = slice_bb
            else
              if (cplex == 1 .and. gstore_cplex == 2) then
                !gqk%my_g2(my_ip, :, my_iq, :, my_ik) = slice_bb(1,:,:) ** 2 + slice_bb(2,:,:) ** 2
                !ABI_WARNING("This conversion is not implemented!")
              else
     ...
     ! TODO
     if (gstore%with_vk == 1) then
       !NCF_CHECK(nctk_set_collective(spin_ncid, spin_vid("vk_cart")))
       !NCF_CHECK(nf90_get_var(spin_ncid, vid("vk_cart"), ??))
     else if (gstore%with_vk == 2) then
       !NCF_CHECK(nctk_set_collective(spin_ncid, spin_vid("vkmat_cart")))
       !NCF_CHECK(nf90_get_var(spin_ncid, spin_vid("vkmat_cart"), ??))
     end if
 ...
 call cwtime_report(" Reading", cpu, wall, gflops)
