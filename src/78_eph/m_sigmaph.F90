...
!scalars
 ...
 integer :: ib1,ib2,band !band1,band2,
 integer :: ik_ibz,ik_bz,ikq_ibz,isym_k,isym_kq,trev_k,trev_kq,timerev_q,nqpt_max
 ...
 integer :: ahc_nb,ahc_bstart,ahc_nshiftq,ntemp
...
!************************************************************************
 ...
 kk = zero; ahc_bstart = 1; ahc_nb = 1
 ABI_CHECK(ahc_nb <= ebands%mband, "ahc_nb > ebands%mband")
 ...
 ! TODO: Load u0 states and symmetrize them

 ! Define q-mesh for integration.
 ahc_ngqpt = ifc%ngqpt; if (all(dtset%eph_ngqpt_fine /= 0)) ahc_ngqpt = dtset%eph_ngqpt_fine
 ahc_nshiftq = 1
 ABI_MALLOC(ahc_shiftq, (3, ahc_nshiftq))
 ahc_shiftq = 0

 qptrlatt = 0
 qptrlatt(1,1) = ahc_ngqpt(1)
 qptrlatt(2,2) = ahc_ngqpt(2)
 qptrlatt(3,3) = ahc_ngqpt(3)
 nqpt_max = (product(ahc_ngqpt) * ahc_nshiftq) ! / facbrv

 ABI_ALLOCATE(qbz, (3,nqpt_max))
 call smpbz(brav1,std_out,qptrlatt,nqpt_max,nqbz,ahc_nshiftq,option1,ahc_shiftq,qbz)
 ...
 if (do_ftv1q /= 0) then
   ...
   call wrtout(std_out, sjoin("From ngqpt", ltoa(ifc%ngqpt), "to", ltoa(ahc_ngqpt)))
 ...
 ! Ground-state wavefunctions for which corrections are wanted (available on each node)
 ! TODO: Be careful with npw, mpw here
 !ABI_MALLOC(ahc_ug, (2, ahc_npw*nspinor, ahc_nb))
 !ABI_FREE(ahc_ug)
...
&            comm_atom=mpi_enreg%comm_atom,mpi_atmtab=mpi_enreg%my_atmtab)
     ...
     ABI_MALLOC(bras, (2, mpw*nspinor, ahc_nb))
     ABI_MALLOC(kets, (2, mpw*nspinor, ahc_nb))
     ABI_MALLOC(h1_kets, (2, mpw*nspinor, ahc_nb))
     ABI_MALLOC(atmgkk, (2, ahc_nb, ahc_nb, natom3))
