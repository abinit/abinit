...
subroutine wfk_ncdef_dims_vars(ncid, hdr, fform, write_hdr)
...
!scalars
 ...
 type(hdr_type),intent(in) :: hdr
 logical,optional,intent(in) :: write_hdr

!Local variables-------------------------------
!scalars
#ifdef HAVE_NETCDF
 character(len=500) :: title,history
 logical :: do_write_hdr 
...
! *************************************************************************
 ...
 call hdr_init_lowlvl(hdr_kfull,ebands_full,psps,pawtab,dummy_wvl,abinit_version,&
   ...
   ihdr%intxc,ihdr%ixc,ihdr%stmbias,ihdr%usewvl,dtset%pawcpxocc,dtset%ngfft,dtset%ngfftdg,ihdr%so_psp,&
