...
!    Computation of "on-site" 2nd-order energy, first-order potentials, first-order densities
     ...
     call pawdenpot(dum,el_temp,ipert,dtset%ixc,my_natom,dtset%natom,&
...
!    PAW: has to compute here the "on-site" 2nd-order energy
     if (psps%usepaw==1) then
       ...
       call paw_an_reset_flags(paw_an1) ! Force the recomputation of on-site potentials
       option=2
       call pawdenpot(dum,el_temp,ipert,dtset%ixc,my_natom,dtset%natom,dtset%nspden,&
...
&       dtset%pawxcdev,dtset%spnorbscl,dtset%xclevel,dtset%xc_denpos,dtset%xc_taupos,&
