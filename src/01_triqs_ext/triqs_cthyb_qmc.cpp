...
#include <triqs/utility/real_or_complex.hpp>    //ADDED For TRIQS 1.4
...
#include <fstream>
#include <iomanip>
//#include <boost/mpi/communicator.hpp> //COMMENTED: Not relevant anymore for TRIQS 1.4
#include <triqs/mpi/boost.hpp>  //ADDED Instead
...
//COMMENTED: The namespace utility became operator in TRIQS 1.4, and the many_body_operator class changed name
...
void ctqmc_triqs_run( bool rot_inv, bool leg_measure, bool hist, bool wrt_files, bool tot_not,                                  /*boolean*/	      
    ...
    //  many_body_operator<double> H;   //COMMENTED: New class name in TRIQS 1.4
...
//    if( delta_init.is_open()) std::cout << "delta_init is open;"<< endl;    //ADDED
//    if(rank==0){
//        delta_init << "# w_index l   l'  Im(iw)  delta(iw)\n" << endl;
//    } 
    //std::cout <<" w_mesh.size() = "<< w_mesh.size() << endl;   //ADDED


    /*
     *  In TRIQS 1.3, the frequency mesh for imfreq green's function is by default poisitive only
     *  In TRIQS 1.4, the default becomes both positive and negative
     *  The CTHYB solver has to have the default mesh
     *  To keep the current structure of ABINIT, we use that for a system with time-reversal symmetry, delta(-iw) = [delta(iw)]^* (Property 1) and solver on a full mesh
    */
    //Old implementation
    ...
    for(std::size_t w1 = 0; w1 < w_mesh.size(); ++w1){
        ...
        for(int o = 0; o < num_orbitals; ++o){
	        ...
	        for(int oo = 0; oo < num_orbitals; ++oo){
                ...
                //std::cout <<"   o, oo "<< o <<"; "<< oo;   //ADDED
          
	            //cell(o,oo) = f_iw_ptr[o+oo*num_orbitals+w_index*num_orbitals*num_orbitals]; //COMMENTED, actual use of (Property 1)
                ...
                //  COMMENTED
	            //if(rank==0){
                    //std::cout <<"\tw_index, w, iw = "<< w_index <<"\t"<< w <<"\t"<< iw <<"\to\too = "<< o <<"\t"<< oo <<"\tf_iw = "<< f_iw << endl; //ADDED
                    //delta_init << w_index+1 <<"\t"<< o+1 <<"\t"<< oo+1 <<"\t"<< imag(iw) <<"\t"<< setprecision(17) << fixed << f_iw << endl;  //ADDED
                    //ADDED
                    //delta_init << w_index+1 <<"\t"<< o+1 <<"\t"<< oo+1 <<"\t"<< imag(iw) <<"\t"<< setprecision(17) << fixed << f_iw_ptr[o+oo*num_orbitals+w_index*num_orbitals*num_orbitals] << endl; //COMMENTED
                    //std::cout <<"; B; f_iw_ptr: "<< f_iw_ptr[o+oo*num_orbitals+w_index*num_orbitals*num_orbitals] << endl;    //ADDED
	        }
	    }
    }
    ...
    //std::cout <<"  PRE g_iw_ptr  "<< endl;   //ADDED      
    std::cout << endl <<" === solver.G0_iw === "<< endl;    //ADDED

    //g_iw << fixed << setprecision(17) << w_index << "\t";
    for(int oo = 0; oo < num_orbitals; ++oo){      
        for(int o = 0; o < num_orbitals; ++o){ 
        //for(std::size_t w_index = 0; w_index < w_mesh.size(); ++w_index){ //Pour une frequence donnee     //COMMENTED: The positive frequencies on the mesh start at w_mesh.size()/2, if we use .data()
    ...
    //if(rank == 0){  //ADDED: The information of the processors is shared within TRIQS so at this point, only rank0 needs to write
        for(int oo = 0; oo < num_orbitals; ++oo){ 
            ...
            for(int o = 0; o < num_orbitals; ++o){ 
	            ...
	            for(int tau = 0; tau < n_tau; ++tau){
	                ...
	                //g_tau[compteur]= solver.G_tau()[0].data()(tau,o,oo);    //COMMENTED
	                g_tau[compteur]= solver.G_tau()[0].data()(tau,o,oo).real();   //ADDED: In TRIQS 1.4, G_tau is now real. To match with the double in ABINIT, just write a real
	                compteur++;
	            }
            }  
        }
    ...
    //Report G(l)
    if( leg_measure){  //ADDED: Same as for G_tau
        ...
        for(int oo = 0; oo < num_orbitals; ++oo){ 
            ...
            for(int o = 0; o < num_orbitals; ++o){ 
                ...
                for(int l = 0; l < n_l; ++l){
        	        ...
        	        //gl[compteur]= solver.G_l()[0].data()(l,o,oo); //COMMENTED
        	        gl[compteur]= solver.G_l()[0].data()(l,o,oo).real();    //ADDED: Same as G_tau
        	        compteur++;
...
//many_body_operator<double> init_Hamiltonian( double *eps, int nflavor, double *U){    //COMMENTED: Change in class name in TRIQS 1.4
many_body_operator_generic<double> init_Hamiltonian( double *eps, int nflavor, double *U){   //ADDED
  
//  many_body_operator<double> H; double coeff;     //COMMENTED: Change in class name in TRIQS 1.4
    many_body_operator_generic<double> H; double coeff;   //ADDED
...
//many_body_operator<double> init_fullHamiltonian( double *eps, int nflavor, double *U){    //COMMENTED: Change in class name in TRIQS 1.4
many_body_operator_generic<double> init_fullHamiltonian( double *eps, int nflavor, double *U){  //ADDED
...
//many_body_operator<double> init_fullHamiltonianUpDown( double *eps, int nflavor, double *U ){   //COMMENTED: Change in class name in TRIQS 1.4
many_body_operator_generic<double> init_fullHamiltonianUpDown( double *eps, int nflavor, double *U ){   //ADDED
