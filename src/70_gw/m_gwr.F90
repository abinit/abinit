...
!! SOURCE
 ...
 contains
   ...
   procedure :: get_gk_rpr => gwr_get_gk_rpr
...
!scalars
 ...
 real(dp) :: ecut_eff, cpu, wall, gflops, mem_mb, te_min, te_max, wmax
...
! *************************************************************************
 ...
 if (dtset%nkptgw /= 0) then
   ...
   if (any(abs(dtset%sigma_erange) > zero)) then
     ...
     call sigtk_kcalc_from_erange(dtset, cryst, ks_ebands, ks_gaps, gwr%nkcalc, gwr%kcalc, gwr%bstart_ks, gwr%nbcalc_ks, comm)
 ...
 if (any(dtset%gwr_np_gtks /= 0)) then
   ...
   !   kbz               |  to be optimized!               | scales (depends on BZ -> IBZ mapping)
 ...
 if (my_rank == master) then
   ...
   !NCF_CHECK(nf90_put_var(ncid, vid("ph_ngqpt"), dtset%ph_ngqpt))
...
subroutine gwr_rotate_gt(gwr, my_ikf, my_it, my_is, desc_kbz, gt_pm)
...
! *************************************************************************
 ...
 do ig1=1,desc_kbz%npw
   desc_kbz%gvec(:,ig1) = matmul(gwr%cryst%symrec(:,:,isym_k), desc_kibz%gvec(:,ig1)) - g0_k  ! tsign_k
 end do
...
! *************************************************************************
 ...
 do my_ikf=1,gwr%my_nkbz
   ...
   do ipm=1,2
     associate (ggp => gt_pm(ipm), desc_k => desc_kbz(my_ikf))
     ...
     do ig2=1,ggp%sizeb_local(2)
       ...
       ! Multiply by e^{ik.r}
       if (.not. k_is_gamma) rgp%buffer_cplx(:, ig2) = ceikr * rgp%buffer_cplx(:, ig2)
     end do ! ig2
 ...
 if (gwr%timeit) call cwtime_report(" gwr_get_green_gpr:", cpu, wall, gflops)
...
!!****f* m_gwr/gwr_get_gk_rpr
!! NAME
!!  gwr_get_gk_rpr
!!
!! FUNCTION
!!
!!  Use FFTs to compute G_k(r',r) from G_k(g,g')
!!
!!  for k in the BZ and given spin and tau.
!!
!!  Note that output matrix is transposed.
!!
!! INPUTS
!!
!! OUTPUT
...
subroutine gwr_get_gk_rpr(gwr, my_ikf, my_it, my_is, gk_rpr)

!Arguments ------------------------------------
 class(gwr_t),intent(in) :: gwr
 integer,intent(in) :: my_ikf, my_it, my_is
 type(matrix_scalapack),intent(out) :: gk_rpr(2)

!Local variables-------------------------------
!scalars
 integer,parameter :: ndat1 = 1
 integer :: ik_bz, ig2, ipm, npwsp, col_bsize, ir1
 real(dp) :: cpu, wall, gflops
 !real(dp) :: kk_bz(3)
...
! *************************************************************************
 ...
 ! Get G_k(+/- itau) in the BZ.
 call gwr%rotate_gt(my_ikf, my_it, my_is, desc_k, gt_pm)
 ...
 do ipm=1,2
   ...
   do ir1=1,gwr%g_nfft
     ...
     call fft_ur(desc_k%npw, gwr%g_nfft, gwr%nspinor, ndat1, &
                 gwr%g_mgfft, gwr%g_ngfft, desc_k%istwfk, desc_k%gvec, desc_k%gbound, &
                 gpr%buffer_cplx(:, ir1), &          ! in
                 gk_rpr(ipm)%buffer_cplx(:, ir1))    ! out
   end do
   call gpr%free()
   ! TODO: Rescale
   ! gk_rpr(ipm)%buffer_cplx =
 end do ! ipm

 call slk_array_free(gt_pm)

 if (gwr%timeit) call cwtime_report(" gwr_get_gk_rpr:", cpu, wall, gflops)

end subroutine gwr_get_gk_rpr
!!***
...
! *************************************************************************
 ...
 ABI_CHECK(trev_q == 0, "wc_rotate: trev_q /= 0 not yet coded")

 ! rotate gvec, recompute gbound and rotate vc_sqrt.
 ! TODO: 1) Handle TR and routine to rotate tchi/W including vc_sqrt
 !       2) Make sure that FFT box is large enough to accomodate umklapps
 do ig1=1,desc_qbz%npw
   desc_qbz%gvec(:,ig1) = matmul(gwr%cryst%symrec(:,:,isym_q), desc_qibz%gvec(:,ig1)) - g0_q  ! tsign_q
 end do
 ...
 associate (wq_i => gwr%wc_qibz(iq_ibz, itau, spin), wq_f => wct_qbz)
 call wq_i%copy(wct_qbz)
 ...
 !call wq_f%print(header=sjoin("not isirr_q with gt_kibz:", itoa(iq_ibz)))
 end associate
...
!scalars
 integer,parameter :: ndat1 = 1
 integer :: my_iqf, iq_bz, ig2, npwsp, col_bsize
 real(dp) :: cpu, wall, gflops
 logical :: q_is_gamma
...
! *************************************************************************
 ...
 do my_iqf=1,gwr%my_nqbz
   ...
   do ig2=1,wct_qbz%sizeb_local(2)
     ...
     call fft_ug(desc_q%npw, gwr%g_nfft, gwr%nspinor, ndat1, &
                 ...
                 wct_qbz%buffer_cplx(:, ig2), &  ! in
                 rgp%buffer_cplx(:, ig2))        ! out

     ! Multiply by e^{iq.r}
     if (.not. q_is_gamma) rgp%buffer_cplx(:, ig2) = ceiqr * rgp%buffer_cplx(:, ig2)
   end do ! ig2
 ...
 if (gwr%timeit) call cwtime_report(" gwr_get_wc_gpr:", cpu, wall, gflops)
...
!scalars
 ...
 real(dp) :: cpu_tau, wall_tau, gflops_tau, cpu_all, wall_all, gflops_all, local_max, max_abs_imag_chit !, spin_fact, sc_ucvol,
 logical :: q_is_gamma
 character(len=5000) :: msg
...
!arrays
 ...
 type(matrix_scalapack) :: gt_gpr(2, gwr%my_nkbz), chiq_gpr(gwr%my_nqibz), gk_rpr(2)
...
! *************************************************************************
 ...
 if (gwr%use_supercell) then
   ...
   call wrtout(std_out, sjoin(" ngkpt:", ltoa(gwr%ngkpt)))
   ...
   do my_is=1,gwr%my_nspins
     ...
     do my_it=1,gwr%my_ntau
       ...
       !ABI_CHECK(my_nr == gwr%g_nfft, "ngfft!")
       do my_ir=1,my_nr
         ...
         do my_ikf=1,gwr%my_nkbz
           ...
           do ipm=1,2
             ABI_CHECK(size(gt_gpr(ipm, my_ikf)%buffer_cplx, dim=2) == my_nr, "my_nr!")
             ABI_CHECK(size(gt_gpr(ipm, my_ikf)%buffer_cplx, dim=1) == desc_k%npw, "desc_k!")
         ...
         do my_iqi=1,gwr%my_nqibz
           ...
           ABI_CHECK(size(chiq_gpr(my_iqi)%buffer_cplx, dim=1) == desc_q%npw, "desc_q!")
       ...
       do my_iqi=1,gwr%my_nqibz
         ...
         do ig2=1,chi_rgp%sizeb_local(2)
           ...
           chi_rgp%buffer_cplx(:, ig2) = cemiqr * chi_rgp%buffer_cplx(:, ig2)
           ...
           gwr%tchi_qibz(iq_ibz, itau, spin)%buffer_cplx(:, ig2)  & ! * cemiqr &
             / gwr%g_nfft / gwr%cryst%ucvol / (gwr%nkbz * gwr%nqbz)
         end do
    ...
    do my_is=1,gwr%my_nspins
      ...
      do my_it=1,gwr%my_ntau
        ...
        do my_ikf=1,gwr%my_nkbz
          ...
          ! G_k(g,g') --> G_k(r,r')
          call gwr%get_gk_rpr(my_ikf, my_it, my_is, gk_rpr)
          call slk_array_free(gk_rpr)
          ...
          !do my_iqi=1,gwr%my_nqibz
            ...
            ! G_kq(g,g') --> G_kq(r,r')
            !call gwr%get_gk_rpr(my_ikqf, my_it, my_is, gkq_rpr)
            !call slk_array_free(gkq_rpr)

            !tchi_rrp(my_iqi)%buffer_cplx = tchi(my_iqi)%buffer_cplx + gk_rpr%buffer_cplx * gkq_rpr%buffer_cplx
 ...
 print *, "max_abs_imag_chit", max_abs_imag_chit
...
!DEBUG
         ...
         !    !.and. what == "tchi_qibz") then
         !    .and. what == "tchi_qibz" .and. gwr%tchi_space == "iomega") then
         !  if (itau == 1) then
         !    if (gwr%tchi_space == "iomega") then
...
! *************************************************************************
 ...
 ! This is not true for ther other q-points in the full BZ.

 ABI_MALLOC(gwr%wc_qibz, (gwr%nqibz, gwr%ntau, gwr%nsppol))

 free_chi = .False.
 if (free_chi) gwr%tchi_space = "none"
 ...
 do my_iqi=1,gwr%my_nqibz
   ...
   do my_is=1,gwr%my_nspins
     ...
     do my_it=1,gwr%my_ntau
       ...
       if (free_chi) call gwr%tchi_qibz(iq_ibz, itau, spin)%free()
       ...
       do il_g2=1,wc%sizeb_local(2)
         ...
         do il_g1=1,wc%sizeb_local(1)
           ...
           ! Handle divergence in Wc
           if (q_is_gamma .and. (iglob1 == 1 .or. iglob2 == 1)) then
             if (iglob1 == 1 .and. iglob2 == 1) then
               wc%buffer_cplx(il_g1, il_g2) = real(wc%buffer_cplx(il_g1, il_g2))
               vcs_g1 = sqrt(i_sz)
               vcs_g2 = sqrt(i_sz)
             elseif (iglob1 == 1) then
               vcs_g1 = (four_pi) ** (three/two) * q0sph ** 2 / two
               !vcs_g1 = sqrt(i_sz)
             elseif (iglob2 == 1) then
               vcs_g2 = (four_pi) ** (three/two) * q0sph ** 2 / two
               !vcs_g2 = sqrt(i_sz)
             end if
             !print *, " Replacing W(0,0) with spherical average:", wc%buffer_cplx(il_g1, il_g2)
           end if
       ...
       if (q_is_gamma) then
         !if (q_is_gamma .and. gwr%nqbz > 1) wc%buffer_cplx = zero
         call wrtout(std_out, sjoin(" Wc for q:", ktoa(qq_ibz), "i omega:", ftoa(gwr%iw_mesh(itau) * Ha_eV), "eV"))
         call print_arr(wc%buffer_cplx, unit=std_out)
 ...
 if (gwr%dtset%prtsuscep > 0) then
   call gwr%ncwrite_tchi_wc("wc", trim(gwr%dtfil%filnam_ds(4))//'_WCIMW.nc')
 end if
 ...
 if (gwr%dtset%prtsuscep > 0) then
   call gwr%ncwrite_tchi_wc("wc", trim(gwr%dtfil%filnam_ds(4))//'_WCIMT.nc')
 end if
...
!scalars
 integer, parameter :: ndat1 = 1
 integer :: my_is, my_it, spin, ik_ibz, sc_nfft, sc_size, my_ir, my_nr, iw !my_iki,
...
! *************************************************************************
 ...
 do my_is=1,gwr%my_nspins
   ...
   do ikcalc=1,gwr%nkcalc
     ...
     ik_ibz = gwr%kcalc2ibz(ikcalc, 1)  ! TODO: Assuming wfs in IBZ

     ! Compute e^{ik.r} phases on the two real-space FFT meshes (super cell and unit cell)
     call calc_sc_ceikr(kk_bz, gwr%ngkpt, gwr%g_ngfft, gwr%g_nfft, gwr%nspinor, sc_ceikr)
     call calc_ceikr(kk_bz, gwr%g_ngfft, gwr%g_nfft, gwr%nspinor, uc_ceikr)

     do band=gwr%bstart_ks(ikcalc, spin), gwr%bstop_ks(ikcalc, spin)
       call gwr%kcalc_wfd%get_ur(band, ik_ibz, spin, uc_psi_bk(:, band, ikcalc))

       ! Multiply by the phases to get psi_nk(r) in the super cell and in the unit cell
       call ur_to_sc_psi(gwr%ngkpt, gwr%g_ngfft, gwr%g_nfft, gwr%nspinor, 1, &
                         uc_psi_bk(:, band, ikcalc), sc_ceikr, sc_psi_bk(:, band, ikcalc))
   ...
   do my_it=1,gwr%my_ntau
     ...
     do my_ir=1,my_nr
       ...
       ! Take the union of (q,g') for q in the BZ. Also, note the use ngqpt instead of ngkpt.
       wct_scbox = zero
       do my_iqf=1,gwr%my_nqbz
         iq_bz = gwr%my_qbz_inds(my_iqf)
         iq_ibz = gwr%my_qbz2ibz(1, my_iqf)
...
!END DEBUG
       ...
       do ikcalc=1,gwr%nkcalc
         do band=gwr%bstart_ks(ikcalc, spin), gwr%bstop_ks(ikcalc, spin)
           cpsi_r = conjg(uc_psi_bk(uc_ir, band, ikcalc))
           sigc_pm(1) = sum(cpsi_r * gt_scbox(:, 1) * sc_psi_bk(:, band, ikcalc))
           sigc_pm(2) = sum(cpsi_r * gt_scbox(:, 2) * sc_psi_bk(:, band, ikcalc))
 ...
 print *, "Maxval abs imag W:", max_abs_imag_wt
 ...
 !sigc_it_diag_kcalc = -sigc_it_diag_kcalc * gwr%cryst%ucvol**2 / (one * sc_nfft * gwr%g_nfft) / (gwr%nkbz * gwr%nqbz)
 ...
 do spin=1,gwr%nsppol
   do ikcalc=1,gwr%nkcalc
      ...
      do band=gwr%bstart_ks(ikcalc, spin), gwr%bstop_ks(ikcalc, spin)
        ...
        !write(std_out, *)"For band:", band, "kpt:", kk_bz
        !do ipm=1,2
        !  write(std_out, "(a,i0,2x,*(es12.5,2x))")" re sigc_it_diag, pm: ", &
        !      ipm, real(sigc_it_diag_kcalc(ipm, 1:3, ibc, ikcalc, spin))
        !  write(std_out, "(a,i0,*(es12.5,2x))")" im sigc_it_diag, pm: ", &
        !      ipm, aimag(sigc_it_diag_kcalc(ipm, 1:3, ibc, ikcalc, spin))
        !end do
        !write(std_out, "(a,2x,*(es12.5,2x))")" re sigc_iw_diag: ", &
        !    real(sigc_iw_diag_kcalc(1:3, ibc, ikcalc, spin))
        !write(std_out, "(a,2x,*(es12.5,2x))")" im sigc_iw_diag: ", &
        !    aimag(sigc_iw_diag_kcalc(1:3, ibc, ikcalc, spin))
   ...
   do my_iqi=1,gwr%my_nqibz
     ...
     do my_it=1,gwr%my_ntau
       ...
       if (my_it == 1) then
         ...
         npw_q = desc_q%npw
         ABI_CHECK(npw_q == tchi%sizeb_global(1), "npw_q")
...
pure subroutine ur_to_sc_psi(sc_shape, uc_ngfft, uc_nfft, nspinor, ndat, ur, sc_eikr, sc_psi)

!Arguments ------------------------------------
 integer,intent(in) :: sc_shape(3)
 integer,intent(in) :: uc_ngfft(18)
 integer,intent(in) :: uc_nfft, nspinor, ndat
 complex(gwpc),intent(in) :: ur(uc_nfft, nspinor, ndat)
 complex(gwpc),intent(out) :: &
     ...
     do iz=1,uc_ngfft(3) * sc_shape(3)
       ...
       do iy=1,uc_ngfft(2) * sc_shape(2)
         ...
         do ix=1,uc_ngfft(1) * sc_shape(1)
           ...
           sc_psi(ix, iy, iz, spinor, idat) =  sc_eikr(ix, iy, iz, spinor) * ur(uc_ir, spinor, idat)
         end do
       end do
     end do
 ...
 do iz=0,uc_ngfft(3) * sc_shape(3) - 1
   do iy=0,uc_ngfft(2) * sc_shape(2) - 1
     do ix=0,uc_ngfft(1) * sc_shape(1) - 1
       ...
       sc_ceikr(ix+1, iy+1, iz+1, 1) = DCMPLX(cos(kdotr), sin(kdotr))
     end do
   end do
 end do
...
subroutine symmetrize_wings(gwr, what)
...
!arrays
 ...
 integer,allocatable :: ginv(:)
 real(dp) :: qq_ibz(3)
 complex(dp) :: head_myt(gwr%my_ntau)
 complex(dp),allocatable :: low_wing_myt(:,:), up_wing_myt(:,:)
 type(matrix_scalapack), pointer :: mats(:)

! *************************************************************************

 call wrtout(std_out, sjoin(" Symmetrizing head and wings at q = 0 of :", what))
 ...
 do my_is=1,gwr%my_nspins
   ...
   do my_iqi=1,gwr%my_nqibz
     ...
     associate (desc_q => gwr%tchi_desc_qibz(iq_ibz))
     ABI_MALLOC(low_wing_myt, (desc_q%npw, gwr%my_ntau))
     ABI_MALLOC(up_wing_myt, (desc_q%npw, gwr%my_ntau))
     ...
     do my_it=1,gwr%my_ntau
       itau = gwr%my_itaus(my_it)
       call mats(itau)%get_head_and_wings(head_myt(my_it), low_wing_myt(:,my_it), up_wing_myt(:,my_it), call_mpi=.False.)

       !call mats(itau)%set_head_and_wings(head_myt(my_it), low_wing_myt(:,my_it), up_wing_myt(:,my_it), call_mpi=.False.)
     end do

     !call xmpi_sum(low_wing_myt, gwr%g_comm%value, ierr)
     !call xmpi_sum(up_wing_myt, gwr%g_comm%value, ierr)
     ...
     ABI_FREE(low_wing_myt)
     ABI_FREE(up_wing_myt)
     ABI_FREE(ginv)
     end associate
...
end subroutine symmetrize_wings
...
! *************************************************************************
 ...
 if (my_rank == master) then
   ...
   ABI_CHECK(sus_nomega == gwr%ntau + 1, sjoin("sus_nomega: ", itoa(sus_nomega), "should be equal to ntau + 1"))
   ...
   if (any(abs(gwr%iw_mesh - aimag(sus_freqs(2:))) > tol5)) then
     do iw=1,gwr%ntau
 ...
 do my_is=1,gwr%my_nspins
   ...
   do my_iqi=1,gwr%my_nqibz
     ...
     ABI_CHECK(nmiss == 0, "nmiss > 0, increase ecuteps in legacy GW code to have more G-vectors!")
