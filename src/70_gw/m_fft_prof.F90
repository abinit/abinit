...
!! SOURCE
 ...
 contains
   ...
   procedure :: time_fftbox
   procedure :: time_fourwf
   procedure :: time_rhotwg
   procedure :: time_fftu
 end type FFT_test_t

 public :: fft_tests_free

 interface fft_tests_free
   module procedure fft_test_free_0D
   module procedure fft_test_free_1D
 end interface fft_tests_free
!!***
...
!! SOURCE
 ...
 contains
   ...
   procedure :: free => fftprof_free_0D
 end type FFT_prof_t

 public :: fftprofs_free
 public :: fftprofs_print

 interface fftprofs_free
   module procedure fftprof_free_0D
   module procedure fftprof_free_1D
 end interface fftprofs_free
!!***
...
!scalars
 ...
 class(MPI_type),intent(in) :: MPI_enreg_in
!arrays
 integer,intent(in) :: fft_setup(6),symrel(3,3,nsym)
...
! *************************************************************************
 ...
 Ftest%use_gpu_flavor = fft_setup(6)
...
&  Ftest%MPI_enreg%paral_kgb,symrel,tnons, unit=dev_null, use_gpu_flavor=ftest%use_gpu_flavor)
...
! *********************************************************************
 ...
 if (ftest%use_gpu_flavor == 0) then
...
subroutine fftprof_init(Ftprof, test_name, nthreads, ncalls, ndat, use_gpu_flavor, cpu_time, wall_time, gflops, results)

!Arguments -----------------------------------
 class(FFT_prof_t),intent(out) :: Ftprof
 integer,intent(in) :: ncalls,nthreads,ndat, use_gpu_flavor
...
! *************************************************************************
 ...
 Ftprof%ndat           = ndat
 Ftprof%use_gpu_flavor = use_gpu_flavor
...
subroutine fftprofs_print(Fprof, header, unit, mode_paral, prtvol)

!Arguments -----------------------------------
 class(FFT_prof_t),intent(in) :: Fprof(:)
 integer,optional,intent(in) :: unit,prtvol
 character(len=4),optional,intent(in) :: mode_paral
 character(len=*),optional,intent(in) :: header
...
!Local variables-------------------------------
!scalars
 integer :: my_unt,my_prtvol,ncalls, field1_w,ii,ref_lib !ifft
...
! *********************************************************************
 ...
 if (isign==1) then
   ...
   if (cplex==2) then
     ...
     ABI_ERROR(sjoin("Wrong cplex:", itoa(cplex)))
   end if
 end if

 ABI_CALLOC(results, (Ftest%nfft))
 ...
 call Ftprof%init(test_name,Ftest%nthreads,NCALLS_FOR_TEST,Ftest%ndat,ftest%use_gpu_flavor, &
...
!Arguments -----------------------------------
 ...
 character(len=500),intent(inout) :: header
 class(FFT_prof_t),intent(out) :: Ftprof

!Local variables-------------------------------
!scalars
 integer,parameter :: nspinor1 = 1, fftcache0 = 0
...
! *********************************************************************
 ...
 call plan%init(ndat, Ftest%ngfft(1:3), Ftest%ngfft(1:3), Ftest%ngfft(7), fftcache0, ftest%use_gpu_flavor)
...
! *********************************************************************
 ...
 isbuggy = &
    (option_fourwf==3 .and. fftalga==FFT_SG2002 .and. fftalgc /= 0 .and. &
     any(Ftest%istwf_k == [3,4,5,6,7,8,9]))  ! see sg_fourwf
 !isbuggy = .False.
...
&                            +g0(3)*(i3/DBLE(n3)) )
 ...
 case default
   ABI_ERROR(sjoin("Wrong value for option_fourwf:", itoa(option_fourwf)))
 end select
 ...
 do icall=1,NCALLS_FOR_TEST
   ...
   call fourwf(cplex,denpot,fofg_in,fofg_out,fofr_4,gbound_in,gbound_out,Ftest%istwf_k,&
    ...
    tim0,weight_r,weight_i, use_gpu_flavor=ftest%use_gpu_flavor)
...
! *********************************************************************
 ...
 if (use_padfft == 1) call sphereboundary(gbound,istwfk1,osc_gvec,Ftest%mgfft,osc_npw)
 ...
 ABI_MALLOC(rhotwg,(osc_npw*dim_rtwg1*ndat))
 ABI_MALLOC(results,(osc_npw*dim_rtwg1*ndat))
...
&                        +g0(3)*(i3/DBLE(n3)) )
   ...
   ABI_ERROR(sjoin("Wrong isign:", itoa(isign)))
 end if
 ...
 do icall=1,NCALLS_FOR_TEST
   ...
   if (isign == +1) then
     ...
     ! Store results at the first call.
     if (icall == 1) call xcopy(nfft*ndat,ur,1,results,1)

   else
     call fft_ur(npw_k,nfft,nspinor1,ndat,Ftest%mgfft,Ftest%ngfft,istwf_k,kg_k,gbound,ur,ug)
     ! Store results at the first call.
     if (icall==1) call xcopy(npw_k*ndat,ug,1,results,1)
   end if
   !if (isign==-1) then
 ...
 call cwtime(cpu_time, wall_time, gflops, "stop")
 call Ftprof%init(test_name,Ftest%nthreads,NCALLS_FOR_TEST,ndat,ftest%use_gpu_flavor, &
                  cpu_time,wall_time,gflops,results=results)
...
! *********************************************************************
 ...
 do set=1,nsetups
   write(funt,'(a, 6(a,i0))') "#",&
    ...
    ", use_gpu_flavor = ",fft_setups(6,set)
 end do
 ...
 do set=1,nsetups
   do iec=1,necut
     call Ftest%init(fft_setups(:,set),k_gamma,ecut_list(iec),boxcutmin,rprimd,nsym,symrel,MPI_enreg_in)
     call Ftest%time_fourdp(isign, cplex, header, Ftprof)
...
subroutine prof_fourwf(fft_setups, cplex, option, kpoint, necut, ecut_arth, &
                       boxcutmin, rprimd, nsym, symrel, MPI_enreg_in)
...
! *********************************************************************
 ...
 do set=1,nsetups
   do iec=1,necut
     call Ftest%init(fft_setups(:,set),kpoint,ecut_list(iec),boxcutmin,rprimd,nsym,symrel,MPI_enreg_in)
     call Ftest%time_fourwf(cplex, option, header, Ftprof)
...
!  to map the box to the G-sphere!
 ...
 do set=1,nsetups
   do iec=1,necut
     ...
     call Ftest%time_rhotwg(map2sphere,use_padfft,osc_npw,osc_gvec,header,Ftprof)
...
integer function empty_cache(kbsize) result(fake)
...
! *********************************************************************
 ...
 fake = int(SUM(chunk)) ! Need a result, otherwise smart compilers may skip the call.
 ABI_FREE(chunk)
