...
!FB  end type Hist_type
 ...
 public :: Aknowledgments
 public :: ReadEcho

contains

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 subroutine Aknowledgments(InVar)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'Aknowledgments'
!End of the abilint section
  ...
  write(InVar%stdout,'(a)') ' ######################### CALCULATION COMPLETED #############################'
  write(InVar%stdout,'(a)') ' #############################################################################'
  write(InVar%stdout,'(a)') ' Suggested references for the acknowledgment of ABINIT usage.'
  write(InVar%stdout,'(a)') ' '
  write(InVar%stdout,'(a)') ' The users of ABINIT have little formal obligations with respect to the ABINIT group'
  write(InVar%stdout,'(a)') ' (those specified in the GNU General Public License, http://www.gnu.org/copyleft/gpl.txt).'
  write(InVar%stdout,'(a)') ' However, it is common practice in the scientific literature,'
  write(InVar%stdout,'(a)') ' to acknowledge the efforts of people that have made the research possible.'
  write(InVar%stdout,'(a)') ' In this spirit, please find below suggested citations of work written by ABINIT developers,'
  write(InVar%stdout,'(a)') ' corresponding to implementations inside of ABINIT that you have used in the present run.'
  write(InVar%stdout,'(a)') ' Note also that it will be of great value to readers of publications presenting these results,'
  write(InVar%stdout,'(a)') ' to read papers enabling them to understand the theoretical formalism and details'
  write(InVar%stdout,'(a)') ' of the ABINIT implementation.'
  write(InVar%stdout,'(a)') ' For information on why they are suggested, see also http://www.abinit.org/about/?text=acknowledgments.'
  write(InVar%stdout,'(a)') ' '
  write(InVar%stdout,'(a)') ' [1] Thermal evolution of vibrational properties of $\alpha$-U' 
  write(InVar%stdout,'(a)') ' J. Bouchet and F. Bottin, Phys. Rev. B 92, 174108 (2015).'
  write(InVar%stdout,'(a)') ' Strong suggestion to cite this paper in your publications.'
  write(InVar%stdout,'(a)') ' This paper is also available at http://www.arxiv.org/abs/xxxx'
  write(InVar%stdout,'(a)') ' '
  write(InVar%stdout,'(a)') ' [2] Lattice dynamics of anharmonic solids from first principles'
  write(InVar%stdout,'(a)') ' O. Hellman and I.A. Abrikosov and S.I. Simak, Phys. Rev. B 84, 180301(R) (2011).'
  write(InVar%stdout,'(a)') ' Strong suggestion to cite this paper in your publications.'
  write(InVar%stdout,'(a)') ' '
  write(InVar%stdout,'(a)') ' [3] Temperature dependent effective potential method for accurate free energy calculations of solids'
  write(InVar%stdout,'(a)') ' O. Hellman and P. Steneteg and I.A. Abrikosov and S.I. Simak, Phys. Rev. B 87, 104111 (2013).'
  write(InVar%stdout,'(a)') ' Strong suggestion to cite this paper in your publications.'

 end subroutine Aknowledgments 

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 subroutine ReadEcho(InVar)
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'ReadEcho'
!End of the abilint section

  implicit none 

  integer :: ii,jj,tmp,istep,iatom
  ...
  character (len=30):: ReadIFC,firstqptseg,D1NN,Ngqpt1,Ngqpt2,DosDeltae
  double precision :: version_value
...
! Define phonons computational details
  ...
  read(40,*) string,InVar%nstep
  write(InVar%stdout,'(x,a20,x,i5)') string,InVar%nstep
  read(40,*) string,InVar%nstep1
  write(InVar%stdout,'(x,a20,x,i5)') string,InVar%nstep1
  InVar%nstep_remain=InVar%nstep-InVar%nstep1+1
...
! Optional input variables  
  ...
  do ii=1,100
    ...
    else if (string.eq.D1NN) then  
      read(40,*) string,InVar%d1NN
      write(InVar%stdout,'(x,a20,x,i4)') string,InVar%d1NN
...
! Output very important informations 
  ...
  write(InVar%stdout,'(a)') ' WARNING: default changed. The average for positions is now only'
  write(InVar%stdout,'(a)') '          performed from nstep(min) to nstep(max)'
...
! Read xred.dat, fcart.dat and etot.dat ASCII files or extract them from the HIST.nc netcdf file.
  ...
  if (InVar%netcdf) then
    ...
    InVar%etot(:)     =Hist%etot(:)
    write(InVar%stdout,'(a)') ' The Xred, Fcart and Etot data are extracted from the NetCDF file: HIST.nc '
  else
    open(unit=60,file='fcart.dat')
    open(unit=50,file='xred.dat')
    open(unit=40,file='etot.dat')
    do istep=1,InVar%nstep
      read(40,*) InVar%etot(istep)
      do iatom=1,InVar%natom
        read(50,*) InVar%xred(1,iatom,istep),InVar%xred(2,iatom,istep),InVar%xred(3,iatom,istep)
        read(60,*) InVar%fcart(1,iatom,istep),InVar%fcart(2,iatom,istep),InVar%fcart(3,iatom,istep)
      end do
    end do  
    close(40)
    close(50)
