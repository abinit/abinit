...
module m_lobpcg2
 ...
 use m_nvtx_data
  ...
  type, public :: lobpcg_t
    ...
    integer :: use_gpu_flavor                ! Which GPU version is used (0=none)
  ...
  subroutine lobpcg_init(lobpcg, neigenpairs, spacedim, blockdim, tolerance, nline, space, spacecom, use_gpu_flavor)
    ...
    lobpcg%use_gpu_flavor = use_gpu_flavor
...
!    end if
  ...
  subroutine lobpcg_allocateAll(lobpcg,space)
    ...
    call xg_init(lobpcg%XWP,space,spacedim,3*blockdim,lobpcg%spacecom, use_gpu_flavor=lobpcg%use_gpu_flavor)
    ...
    call xg_init(lobpcg%AXWP,space,spacedim,3*blockdim,lobpcg%spacecom, use_gpu_flavor=lobpcg%use_gpu_flavor)
    ...
    call xg_init(lobpcg%BXWP,space,spacedim,3*blockdim,lobpcg%spacecom, use_gpu_flavor=lobpcg%use_gpu_flavor)
    ...
    if ( lobpcg%nblock /= 1 ) then
      call xg_init(lobpcg%AllBX0,space,spacedim,lobpcg%neigenpairs,lobpcg%spacecom, use_gpu_flavor=lobpcg%use_gpu_flavor)
      call xg_init(lobpcg%AllAX0,space,spacedim,lobpcg%neigenpairs,lobpcg%spacecom, use_gpu_flavor=lobpcg%use_gpu_flavor)
    else
  ...
  subroutine lobpcg_run(lobpcg, X0, getAX_BX, pcond, eigen, occ, residu, prtvol, isppol, ikpt, inonsc, istep, nbdbuf)
    ...
    interface
      subroutine pcond(W,use_gpu_flavor)
        use m_xg, only : xgBlock_t
    ...
    call xg_init(eigenvalues3N,SPACE_R,blockdim3,1, use_gpu_flavor=lobpcg%use_gpu_flavor)
    ...
    do iblock = 1, nblock
      ...
      do iline = 1, nline
        ...
        call pcond(lobpcg%W,lobpcg%use_gpu_flavor)
        call timab(tim_pcond,2,tsec)

        ! Compute residu norm here !
        call timab(tim_maxres,1,tsec)
        call xgBlock_colwiseNorm2(lobpcg%W,residuBlock,use_gpu_flavor=lobpcg%use_gpu_flavor)
        call timab(tim_maxres,2,tsec)

        if(lobpcg%use_gpu_flavor==ABI_GPU_OPENMP) call xgBlock_copy_from_gpu(residuBlock)
        if (nbdbuf>=0) then
        ...
        if ( iline == 1 .or. minResidu < 1e-27) then
          ...
          call lobpcg_Borthonormalize(lobpcg,VAR_XW,.true.,ierr) ! Do rotate AW
          RR_var = VAR_XW
          call xgBlock_zero(lobpcg%P, use_gpu_flavor=lobpcg%use_gpu_flavor)
          call xgBlock_zero(lobpcg%AP, use_gpu_flavor=lobpcg%use_gpu_flavor)
          call xgBlock_zero(lobpcg%BP, use_gpu_flavor=lobpcg%use_gpu_flavor)
      ...
      call xgBlock_copy(eigenvaluesN,eigenBlock,use_gpu_flavor=lobpcg%use_gpu_flavor)
  ...
  subroutine lobpcg_getX0(lobpcg,iblock)
    ...
    call xgBlock_copy(lobpcg%X0,lobpcg%X, use_gpu_flavor=lobpcg%use_gpu_flavor)
  ...
  subroutine lobpcg_Borthonormalize(lobpcg,var,BorthoA,info)
    ...
    call xg_init(buffer,space(X),cols(X),cols(X),lobpcg%spacecom, use_gpu_flavor=lobpcg%use_gpu_flavor)

    ! Compute X^TBX
    call xgBlock_gemm(X%trans,BX%normal,1.d0,X,BX,0.d0,buffer%self,use_gpu_flavor=lobpcg%use_gpu_flavor)

    ! Compute Cholesky decomposition (Upper part)
    call xgBlock_potrf(buffer%self,'u',info,use_gpu_flavor=lobpcg%use_gpu_flavor)
...
!!$omp task
    ! Solve YU=X
    call xgBlock_trsm('r','u',buffer%normal,'n',1.d0,buffer%self,X,use_gpu_flavor=lobpcg%use_gpu_flavor)
!!$omp end task

!!$omp task
    ! Solve BYU=BX
    call xgBlock_trsm('r','u',buffer%normal,'n',1.d0,buffer%self,BX,use_gpu_flavor=lobpcg%use_gpu_flavor)
!!$omp end task

!!$omp task
    if ( BorthoA .eqv. .true. ) then
      ! Solve AYU=AX
      call xgBlock_trsm('r','u',buffer%normal,'n',1.d0,buffer%self,AX,use_gpu_flavor=lobpcg%use_gpu_flavor)
    end if
!!$omp end task
...
!!$omp end parallel
  ...
  subroutine lobpcg_rayleighRitz(lobpcg,var,eigenvalues,info,tolerance)
    ...
    case(VAR_X)
      ...
      eigenSolver = EIGENEV
      if(lobpcg%use_gpu_flavor==ABI_GPU_OPENMP) eigenSolver = EIGENEVD
#ifdef HAVE_LINALG_MKL_THREADS
    ...
    case(VAR_XW)
      ...
      eigenSolver = EIGENVX
      if(lobpcg%use_gpu_flavor==ABI_GPU_OPENMP) eigenSolver = EIGENVD
#ifdef HAVE_LINALG_MKL_THREADS
...
#ifdef HAVE_LINALG_SCALAPACK
    call xgScalapack_init(scalapack,lobpcg%spacecom,subdim,lobpcg%prtvol-2,(lobpcg%use_gpu_flavor/=ABI_GPU_DISABLED),use_slk)
    if ( use_slk) then
    ...
    call xg_init(subA,space(X),subdim,subdim,lobpcg%spacecom, use_gpu_flavor=lobpcg%use_gpu_flavor)
    !call xgBlock_zero(subA%self)
    if ( var /= VAR_X ) then
      call xg_init(subB,space(X),subdim,subdim,lobpcg%spacecom, use_gpu_flavor=lobpcg%use_gpu_flavor)
      !call xgBlock_zero(subB%self)
      !call xgBlock_one(subB%self)
    end if

    if ( eigenSolver == EIGENVX .or. eigenSolver == EIGENPVX ) then
      call xg_init(vec,space(x),subdim,blockdim, use_gpu_flavor=lobpcg%use_gpu_flavor)
    else if ( EIGPACK(eigenSolver) ) then
      call xg_init(vec,space(x),subdim,subdim, use_gpu_flavor=lobpcg%use_gpu_flavor)
    else
    ...
    call xgBlock_gemm(X%trans,AX%normal,1.0d0,X,AX,0.d0,subsub,use_gpu_flavor=lobpcg%use_gpu_flavor)

    if ( var /= VAR_X ) then
      call xg_setBlock(subB,subsub,1,blockdim,blockdim)
      call xgBlock_gemm(X%trans,BX%normal,1.0d0,X,BX,0.d0,subsub,use_gpu_flavor=lobpcg%use_gpu_flavor)
    endif
    ...
    ! Solve Hermitian eigen problem
      ...
      case (EIGENEVD)
        ...
        call xgBlock_heevd('v','u',subA%self,eigenvalues,info,use_gpu_flavor=lobpcg%use_gpu_flavor)
      case (EIGENEV)
      ...
      case (EIGENSLK)
        ...
        call xgScalapack_heev(scalapack,subA%self,eigenvalues,use_gpu_flavor=lobpcg%use_gpu_flavor)
      ...
      case (EIGENVD)
        ...
        call xgBlock_hegvd(1,'v','u',subA%self,subB%self,eigenvalues,info,use_gpu_flavor=lobpcg%use_gpu_flavor)
      case (EIGENV)
      ...
      case (EIGENSLK)
        ...
        call xgScalapack_hegv(scalapack,subA%self,subB%self,eigenvalues,use_gpu_flavor=lobpcg%use_gpu_flavor)
        info = 0 ! No error code returned for the moment
      case default
...
!    end if
    ...
    !FIXME Avoid those transfers
    if ( info == 0 ) then
      call xg_init(subB,space(X),spacedim,blockdim, use_gpu_flavor=lobpcg%use_gpu_flavor)
      ...
      call xgBlock_gemm(lobpcg%X%normal,Cwp%normal,1.0d0,lobpcg%X,Cwp,0.d0,subB%self,&
          use_gpu_flavor=lobpcg%use_gpu_flavor)
      call xgBlock_copy(subB%self,lobpcg%X,use_gpu_flavor=lobpcg%use_gpu_flavor)
      ...
      call xgBlock_gemm(lobpcg%AX%normal,Cwp%normal,1.0d0,lobpcg%AX,Cwp,0.d0,subB%self,&
          use_gpu_flavor=lobpcg%use_gpu_flavor)
      call xgBlock_copy(subB%self,lobpcg%AX,use_gpu_flavor=lobpcg%use_gpu_flavor)
      ...
      call xgBlock_gemm(lobpcg%BX%normal,Cwp%normal,1.0d0,lobpcg%BX,Cwp,0.d0,subB%self,&
          use_gpu_flavor=lobpcg%use_gpu_flavor)
      call xgBlock_copy(subB%self,lobpcg%BX,use_gpu_flavor=lobpcg%use_gpu_flavor)

      if ( var /= VAR_X ) then
        ! Cost to pay to avoid temporary array in xgemm
        if(lobpcg%use_gpu_flavor==ABI_GPU_OPENMP) call xgBlock_copy_from_gpu(vec%self) !FIXME Avoid that transfer
        call xgBlock_cshift(vec%self,blockdim,1) ! Bottom 2*blockdim lines are now at the top
        if(lobpcg%use_gpu_flavor==ABI_GPU_OPENMP) call xgBlock_copy_to_gpu(vec%self) !FIXME Avoid that transfer
        ...
        call xgBlock_gemm(WP%normal,Cwp%normal,1.0d0,WP,Cwp,0.d0,subB%self,use_gpu_flavor=lobpcg%use_gpu_flavor)
        call xgBlock_copy(subB%self,lobpcg%P,use_gpu_flavor=lobpcg%use_gpu_flavor)

        !lobpcg%AXWP(:,P+1:P+blockdim) = matmul(lobpcg%AXWP(:,W+1:W+subdim-blockdim),vec(1:subdim-blockdim,1:blockdim))
        call xgBlock_gemm(AWP%normal,Cwp%normal,1.0d0,AWP,Cwp,0.d0,subB%self,use_gpu_flavor=lobpcg%use_gpu_flavor)
        call xgBlock_copy(subB%self,lobpcg%AP,use_gpu_flavor=lobpcg%use_gpu_flavor)

        !lobpcg%BXWP(:,P+1:P+blockdim) = matmul(lobpcg%BXWP(:,W+1:W+subdim-blockdim),vec(1:subdim-blockdim,1:blockdim))
        call xgBlock_gemm(BWP%normal,Cwp%normal,1.0d0,BWP,Cwp,0.d0,subB%self,use_gpu_flavor=lobpcg%use_gpu_flavor)
        call xgBlock_copy(subB%self,lobpcg%BP,use_gpu_flavor=lobpcg%use_gpu_flavor)

        !/* Maybe faster solution
        ! * Sum previous contribution plus P direction
        ! */
        call xgBlock_add(lobpcg%X,lobpcg%P,use_gpu_flavor=lobpcg%use_gpu_flavor)
        call xgBlock_add(lobpcg%AX,lobpcg%AP,use_gpu_flavor=lobpcg%use_gpu_flavor)
        call xgBlock_add(lobpcg%BX,lobpcg%BP,use_gpu_flavor=lobpcg%use_gpu_flavor)
      end if
    end if
  ...
  subroutine lobpcg_getResidu(lobpcg,eigenvalues)
    ...
    call xgBlock_colwiseCymax(lobpcg%W,eigenvalues,lobpcg%BX,lobpcg%AX, use_gpu_flavor=lobpcg%use_gpu_flavor)
    ABI_NVTX_END_RANGE()
    call timab(tim_maxres,2,tsec)
  ...
  subroutine lobpcg_setX0(lobpcg,iblock)
    ...
    call xgBlock_copy(lobpcg%X,Xtmp, use_gpu_flavor=lobpcg%use_gpu_flavor)
  ...
  subroutine lobpcg_transferAX_BX(lobpcg,jblock)
    ...
    call xgBlock_copy(lobpcg%BX,CXtmp, use_gpu_flavor=lobpcg%use_gpu_flavor)

    ! AX
    call xg_setBlock(lobpcg%AllAX0,CXtmp,firstcol,lobpcg%spacedim,lobpcg%blockdim)
    call xgBlock_copy(lobpcg%AX,CXtmp, use_gpu_flavor=lobpcg%use_gpu_flavor)
