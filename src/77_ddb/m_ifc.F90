...
!! SOURCE
 ...
 type,public :: ifc_type
   ...
   real(dp) :: eta = -one
    ! eta: parameter used to split R and G-space summation. See ewald9.
   ...
   real(dp),allocatable :: dynmat(:,:,:,:,:,:)
     ...
     ! dynamical matrices relative to the q points of the BZ. sampling
...
!Arguments ------------------------------------
 class(ifc_type),intent(inout) :: Ifc
 integer,intent(in) :: asr,dipdip,symdynmat,nqshft,rfmeth,nsphere,comm,sys_dim
...
! ************************************************************************
 ...
 call gtdyn9(Ifc%acell,Ifc%atmfrc,Ifc%dielt,Ifc%dipdip,Ifc%dyewq0,d2cart,Crystal%gmet,Ifc%gprim,Ifc%mpert,natom,&
   ...
   Ifc%qdrp_cart,Ifc%ewald_option,eta,comm_, Ifc%sys_dim,dipquad=Ifc%dipquad,quadquad=Ifc%quadquad,&
...
! ************************************************************************
 ...
 if (ifc%dipdip==1) ABI_WARNING("2D electrostatics + IFCs derivatives from real-space IFCs not fully tested")
 ...
 !if (dipdip==1 .and. (ifc%sys_dim==2 .or. ifc%sys_dim==3 .or. ifc%sys_dim==4)) then
    !do kk=1,3
      !if (kk /= ifc%sys_dim-1) then
        !do jj=-1,1,2
          !qtmp(:) = zero
          !qtmp(kk)=jj*0.01_dp
...
!arrays
 ...
 real(dp),allocatable :: ref_phfrq(:,:),cut_phfrq(:,:), save_wghatm(:,:,:),save_atmfrc(:,:,:,:,:)
! *********************************************************************
...
! echo to log file
 ...
 if (prt_ifc == 1) then
   ...
   if (open_file(trim(filename), msg, newunit=unit_ifc, status="replace") /= 0) then
     ABI_ERROR(msg)
   end if
   write(iout, '(a,a)' )ch10,&
   ...
   if (open_file(trim(filename), msg, newunit=unit_tdep, status="replace") /= 0) then
     ABI_ERROR(msg)
   end if
...
!scalars
 ...
 real(dp) :: ew1,rsq,scprod,trace1,trace2,trace3, yy,dist1
 character(len=500) :: msg
!arrays
 real(dp) :: ewiaf0(3,3),ewiaf1(3,3),ewloc(3,3),ifcloc(3,3)
 real(dp) :: rcart(3),rdiff(3),rsloc(3,3), srloc(3,3),vect1(3),vect2(3),vect3(3),work(3),xx(3)
! *********************************************************************
...
!scalars
 ...
 real(dp) :: fac
!arrays
 real(dp) :: omegafl, omegasr, omegalr, sumfl,sumlr,sumsr,asr
...
!Local variables-------------------------------
!scalars
 integer :: iatom,idir,imode,iphl2, ncerr
 real(dp) :: eta
!arrays
...
! ************************************************************************
 ...
 call gtdyn9(ifc%acell,ifc%atmfrc,ifc%dielt,ifc%dipdip, &
   ...
   ifc%trans,crystal%ucvol,ifc%wghatm,crystal%xred,ifc%zeff,ifc%qdrp_cart,ifc%ewald_option,eta,xmpi_comm_self,ifc%sys_dim)
...
subroutine ifc_calcnwrite_nana_terms_qpath(ifc, qpath, cryst, ncid, units)

!Arguments ------------------------------------
 class(ifc_type),intent(in) :: ifc
 integer,intent(in) :: ncid, units(:)
 type(kpath_t),intent(in) :: qpath
 type(crystal_t),intent(in) :: cryst

!Local variables-------------------------------
!scalars
 integer :: iq, nq_vers, natom3, nu
 character(len=500) :: msg
!arrays
 real(dp) :: phfreqs(3*cryst%natom), displ_cart(2,3,cryst%natom,3*cryst%natom)
 real(dp),allocatable :: qvers_red(:,:), qvers_cart(:,:)
! ************************************************************************

 natom3 = 3 * cryst%natom
 call qpath%get_versors(nq_vers, qvers_red, qvers_cart)

 do iq=1,nq_vers
   call ifc%fourq(cryst, qvers_red(:,iq), phfreqs, displ_cart, nanaqdir="reduced")
