...
! *********************************************************************
 ...
 ! gnuplot script file
  ...
  write(gpl_unt,"(a)")strcat('plot for [i=2:nbranch] "', basename(datafile), '" u 1:i every :1 with lines linetype -1')
...
! *********************************************************************
 ...
 if (my_rank == master) then
   ...
   case (1)
     call phonons_write_xmgrace(strcat(prefix, "_PHBANDS.agr"), natom, nqpts, qpath%points, phfrqs)
   case (2)
     call phonons_write_gnuplot(prefix, natom, nqpts, qpath%points, phfrqs)
   case (3)
