...
!! SOURCE
 ...
 type,public :: fftbox_plan3_t
   ...
   integer :: use_gpu_flavor = ABI_GPU_DISABLED  ! /= 0 if FFTs should be offloaded to the GPU
...
subroutine fftbox_plan3_init(plan, batch_size, dims, embed, fftalg, fftcache, use_gpu_flavor)

!Arguments ------------------------------------
!scalars
 class(fftbox_plan3_t),intent(out) :: plan
 integer,intent(in) :: batch_size, fftalg, fftcache, use_gpu_flavor
!arrays
...
! *************************************************************************
 ...
 plan%use_gpu_flavor  = use_gpu_flavor
...
subroutine fftbox_plan3_from_ngfft(plan, ngfft, batch_size, use_gpu_flavor)

!Arguments ------------------------------------
 class(fftbox_plan3_t),intent(out) :: plan
 integer,intent(in) :: ngfft(18), batch_size, use_gpu_flavor

! *************************************************************************

 call plan%init(batch_size, ngfft(1:3), ngfft(4:6), ngfft(7), ngfft(8), use_gpu_flavor)
...
#if defined HAVE_GPU_CUDA
 if (plan%use_gpu_flavor /= ABI_GPU_DISABLED) then
...
!! use_gpu_flavor=  GPU version to active (0: no GPU).
...
integer function fftbox_utests(fftalg, ndat, nthreads, use_gpu_flavor, unit) result(nfailed)

!Arguments -----------------------------------
!scalars
 integer,intent(in) :: fftalg, ndat, nthreads, use_gpu_flavor
...
! *************************************************************************
 ...
 do iset=1,SIZE(pars,DIM=2)
   ...
   ! Create the FFT plan
   call box_plan%init(ndat, pars(1,iset), pars(4,iset), fftalg, fftcache0, use_gpu_flavor)
...
!scalars
 ...
 integer :: nfftot,nproc_fft,option_ccfft,paral_kgb,use_gpu_flavor_
...
!Cuda version of fourwf
 use_gpu_flavor_=ABI_GPU_DISABLED
 if (PRESENT(use_gpu_flavor)) use_gpu_flavor_=use_gpu_flavor

 if(option==1) then
   weight_array_r(:)=weight_r
   weight_array_i(:)=weight_i
 end if
 if(use_gpu_flavor_==ABI_GPU_LEGACY) then
#if defined HAVE_GPU_CUDA
...
!  &  use_ndo,fofginb)
 ...
 else if(use_gpu_flavor_==ABI_GPU_KOKKOS) then
#if defined HAVE_GPU_CUDA
...
#endif
   call timab(840+tim_fourwf,2,tsec); return
 else if(use_gpu_flavor_==ABI_GPU_OPENMP) then
...
subroutine uplan_init(uplan, npw, nspinor, batch_size, ngfft, istwfk, kg_k, kind, use_gpu_flavor)

!Arguments ------------------------------------
!scalars
 class(uplan_t),intent(out) :: uplan
 integer,intent(in) :: npw, nspinor, batch_size, istwfk, kind, use_gpu_flavor
!arrays
 integer,intent(in) :: ngfft(18)
...
! *************************************************************************
 ...
 uplan%kind  = kind
 uplan%use_gpu_flavor  = use_gpu_flavor
 uplan%ngfft = ngfft
 ...
 if (uplan%use_gpu_flavor /= ABI_GPU_DISABLED) then
