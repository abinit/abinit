...
!!      cchi0q0_intraband,cgwf,chebfi,chiscwrt,chkdpr,chkinp,chkint_prt,chkneu
!!      chkpawovlp,clnup1,clnup2,cohsex_me,compute_kgb_indicator,compute_levels
!!      constrf,cut3d,d2frnl,datafordmft,ddb_diel,ddb_elast,ddb_internalstr
!!      ddb_io_out,ddb_piezo,debug_tools,deloc2xcart,denfgr,dfpt_cgwf
!!      dfpt_looppert,dfpt_mkrho,dfpt_nselt,dfpt_nstdy,dfpt_nstpaw,dfpt_prtene
!!      dfpt_prtph,dfpt_scfcv,dfpt_symph,dfpt_vtowfk,dfpt_wfkfermi
!!      dfptff_initberry,dfptnl_doutput,dfptnl_loop,dfptnl_mv,dielmt,dieltcel
!!      dmft_solve,dos_hdr_write,driver,dyson,echo_xc_name,eig2stern
!!      eliashberg_1d,elph2_fanddw,elphon,elpolariz,elt_ewald,entropyrec
!!      ep_fs_weights,ep_setupqpt,eph,erlxconv,evdw_wannier,exc_build_block
!!      exc_build_ham,exc_den,exc_plot,extraprho,fconv,fermi_green
!!      fermisolverec,fftprof,find_getdtset,finddistrproc,findmin,findminscf
!!      first_rec,fred2fdeloc,fresidrsp,fsumrule,gaus_dos,get_all_gkq
...
!!      gran_potrec,green_kernel,gstate,gstateimg,gw_tools,gwcompleteness
!!      gwls_polarisability,harmonic_thermo,hdr_vs_dtset,herald,hermit
!!      hubbard_one,importxyz,impurity_solve,inarray,ingeo,ingeobld,initberry
!!      initmpi_grid,initro,initwf,inkpts,inpgkk,inpspheads,instrng,intagm
!!      integrate_gamma,integrate_gamma_alt,integrate_gamma_tr
!!      integrate_gamma_tr_lova,invars1,invars1m,invars2,inwffil,ioniondist
!!      ioprof,irrzg,kpgio,kramerskronig,ks_ddiago,lapackprof,ldau_self
!!      leave_new,linemin,littlegroup_pert,littlegroup_q,lobpcgwf
!!      local_ks_green,m_abi_etsf,m_abilasi,m_anaddb_dataset,m_argparse,m_atom
!!      m_bfgs,m_bs_defs,m_bse_io,m_bz_mesh,m_cgtools,m_chi0,m_commutator_vkbr
!!      m_crystal,m_ddb,m_ddk,m_double_grid,m_dvdb,m_dynmat,m_dyson_solver
!!      m_ebands,m_energy,m_entropyDMFT,m_errors,m_esymm,m_eval_lotf
!!      m_exc_diago,m_exc_itdiago,m_exc_spectra,m_exit,m_fft,m_fft_mesh
!!      m_fft_prof,m_fftcore,m_fftw3,m_fock,m_fstab,m_geometry,m_gpu_detect
!!      m_green,m_gsphere,m_hamiltonian,m_haydock,m_hdr,m_hexc,m_hidecudarec
!!      m_hu,m_ifc,m_initcuda,m_invovl,m_io_kss,m_io_screening,m_ioarr,m_iowf
!!      m_iterators,m_kxc,m_libpaw_libxc,m_libxc_functionals,m_lotf,m_matlu
!!      m_matrix,m_melemts,m_mep,m_nctk,m_numeric_tools,m_oper,m_optic_tools
!!      m_paw_an,m_paw_dmft,m_paw_gaussfit,m_paw_ij,m_paw_io,m_paw_slater
!!      m_paw_sphharm,m_pawang,m_pawdij,m_pawfgr,m_pawfgrtab,m_pawpsp,m_pawpwij
!!      m_pawrad,m_pawrhoij,m_pawtab,m_phgamma,m_phonons,m_pimd,m_plowannier
!!      m_ppmodel,m_pptools,m_pred_lotf,m_pretty_rec,m_psps,m_psxml2ab
!!      m_ptgroups,m_qparticles,m_rec,m_rf2,m_screen,m_screening,m_self
!!      m_shirley,m_sigma,m_slk,m_vcoul,m_wfd,m_wfk,m_work_var_lotf,m_xc_vdw
!!      m_xpapi,mag_constr_e,mblktyp1,mblktyp5,memana,memorf,memory,metric
!!      mka2f,mka2fQgrid,mka2f_tr,mka2f_tr_lova,mkcore_paw,mkcore_wvl
...
!!      mpi_setup,mrgddb,mrgdv,mrggkk,mrgscr,multipoles_fftr,my_calc_wfwfg
!!      newfermie1,newkpt,newocc,newton,nlenergyrec,nonlinear,normsq_gkq,optic
!!      out1dm,out_acknowl,outelph,outgkk,outkss,outqmc,outscfcv,outvars
!!      outwant,paw_mknewh0,paw_qpscgw,pawdenpot,pawdensities,pawmkaewf
!!      pawmkrhoij,pawprt,pawpuxinit,pawuenergy,pawuj_det,pawuj_red,pawuj_utils
!!      pawxenergy,pimd_nosehoover_nvt,polcart,posdoppler,poslifetime
!!      pred_delocint,pred_isokinetic,pred_isothermal,pred_langevin,pred_nose
!!      pred_verlet,predictimg,prep_calc_ucrpa,prtefield,prteigrs,prtene
!!      prtfatbands,prtimg,prtrhomxmn,prtspgroup,prtxf,prtxfase,prtxvf
!!      psichi_renormalization,psolver_hartree,psolver_kernel,psolver_rhohxc
!!      psp10in,psp1in,psp2in,psp2lo,psp3in,psp5in,psp6in,psp8in,psp9in,pspatm
!!      pspini,pspnl_hgh_rec,pspnl_operat_rec,qmc_prep_ctqmc,randac
!!      random_stopping_power,rayleigh_ritz,read_gkk,read_plowannier
!!      recursion_nl,remove_inversion,respfn,rf2_init,rotmat,scfcge,scfcv
!!      scfeig,scfopt,scprqt,screening,setnoccmmp,setrhoijpbe0,setsymrhoij
!!      setup1,setup_bse,setup_bse_interp,setup_positron,setup_screening
!!      setup_sigma,shellstruct,sigma,smpbz,spectral_function,stress,sumrule
!!      symanal,symatm,symaxes,symcharac,symkchk,symkpt,symlatt,symplanes
!!      symspgr,tddft,testkgrid,tetrahedron,thmeig,timana,uderiv,ujdet
!!      update_e_field_vars,vdw_dftd2,vdw_dftd3,vdw_kernelgen,vtorho,vtorhorec
!!      vtorhotf,vtowfk,wfconv,wfd_mkrho,wfd_pawrhoij,wfk_analyze,wfsinp
!!      wrt_moldyn_netcdf,wrtloctens,wvl_denspot_set,wvl_descr_atoms_set_sym
!!      wvl_descr_psp_set,wvl_hpsitopsi,wvl_initro,wvl_memory,wvl_mkrho
...
!!      wvl_wfsinp_disk,wvl_wfsinp_reformat,wvl_wfsinp_scratch
!!
!! CHILDREN
