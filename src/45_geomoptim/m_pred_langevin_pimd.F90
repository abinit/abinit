...
module m_pred_langevin_pimd
  ...
  use m_abihist
  use m_pimd
  use m_pimd_langevin, only : pimd_langevin_nvt
...
contains
  ...
  subroutine pred_langevin_pimd(ab_mover,hist,itime,zDEBUG,pimd_param)
    ...
    real(dp) :: etotal
    ! Arrays
    real(dp) :: acell(3),rprimd(3,3),gprimd(3,3),gmet(3,3),rmet(3,3),strten(6)
    real(dp) :: fcart(3,ab_mover%natom),xred(3,ab_mover%natom),vel(3,ab_mover%natom)
