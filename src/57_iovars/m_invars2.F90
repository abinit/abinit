...
!  end if
   ...
   elseif(dtset%dmft_solv>=6.and.dtset%dmft_solv<=7) then
     ...
     call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'dmft_triqs_debug',tread,'INT')
     if(tread==1) dtset%dmft_triqs_debug=intarr(1)
     ...
     call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'dmft_triqs_shift_level',tread,'DPR')
     if(tread==1) dtset%dmft_triqs_shift_level=dprarr(1)
