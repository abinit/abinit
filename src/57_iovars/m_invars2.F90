...
!  Give nband explicitly for each k point and spin
   ...
   write(message, '(a,i0,3a)' )'occopt=',occopt,' not allowed.',ch10,'Action: correct your input file.'
   MSG_ERROR(message)
 end if
...
! eph variables
 ...
 call intagm(dprarr,intarr,jdtset,marr,1,string(1:lenstr),'eph_force_ftinterp',tread,'INT')
 if(tread==1) dtset%eph_force_ftinterp = intarr(1)
