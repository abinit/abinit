...
!  ffnl_lw
   ...
   ! gpu_devices
   if (dt%use_gpu_flavor/=ABI_GPU_DISABLED) then
...
!  With optcell beyond 4, one has constraints on rprimd.
   ...
   if (dt%rmm_diis /= 0) then
     ...
     ABI_CHECK(dt%use_gpu_flavor == ABI_GPU_DISABLED, "RMM-DIIS does not support GPUs.")
...
!  use_gpu_flavor
   call chkint_eq(0,0,cond_string,cond_values,ierr,'use_gpu_flavor',dt%use_gpu_flavor,4, &
   &        (/ABI_GPU_DISABLED,ABI_GPU_LEGACY,ABI_GPU_OPENMP,ABI_GPU_KOKKOS/),iout)
   if (dt%use_gpu_flavor/=ABI_GPU_DISABLED) then
     if (dt%nspinor==2) then
       write(msg,'(3a)')&
&       'Use of GPU is not allowed when nspinor==2 !',ch10,&
&       'Action: impose use_gpu_flavor=0 in your input file.'
       ABI_ERROR_NOSTOP(msg, ierr)
     end if
!    if (dt%optdriver==RUNL_GSTATE.and.mod(dt%wfoptalg,10)/=4) then
!    write(msg,'(6a)') ch10,&
!    &       ' chkinp : ERROR -',ch10,&
!    &       '  When GPU is in use (use_gpu_flavor>0), wfoptalg must be 4 or 14 !',ch10,&
...
!    end if
     ...
     if (dt%tfkinfunc>0) then
       write(msg,'(5a)')&
&       'use_gpu_flavor/=0 (use of GPU) is not allowed when tfkinfunc>0 !',ch10,&
&       'Action: suppress use_gpu_flavor from your input file',ch10,&
&       '        (GPU will be used but with another mechanism)'
     ...
     if (dt%ngfft(4)/=dt%ngfft(1).or.dt%ngfft(5)/=dt%ngfft(2).or.dt%ngfft(6)/=dt%ngfft(3)) then
       write(msg,'(3a)')&
&       'When GPU is in use (use_gpu_flavor/=0), ngfft(4:6) must be equal to ngfft(1:3) !',ch10,&
&       'Action: suppress ngfft in input file or change it.'
     ...
     write(msg,'(6a)') ch10,&
&     ' invars0: ERROR -',ch10,&
&     '   Input variable use_gpu_flavor is on but abinit hasn''t been built with GPU mode enabled !',ch10,&
&     '   Action: suppress the input variable use_gpu_flavor or re-compile ABINIT with GPU enabled.'
     call wrtout(std_out,msg)
     ierr=ierr+1
#endif
