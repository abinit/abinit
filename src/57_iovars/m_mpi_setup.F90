     ...
     if(optdriver==RUNL_GSTATE.or.optdriver==RUNL_GWLS) then
       ...
       if (mpi_enregs(idtset)%nproc_cell>0) then
         if(mpi_enregs(idtset)%paral_kgb == 1) then
           ...
           if((dtsets(idtset)%use_gpu_cuda/=ABI_GPU_DISABLED).and.(mpi_enregs(idtset)%nproc_fft/=1))then
             write(msg,'(3a,i0)') &
...
!  according to a threshold on matrix sizes
   if (dtsets(idtset)%use_gpu_cuda/=ABI_GPU_DISABLED.or.dtsets(idtset)%use_gpu_cuda==-1) then
     if (optdriver==RUNL_GSTATE.or.optdriver==RUNL_GWLS) then
