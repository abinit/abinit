module m_spin_model
  ...
  type spin_model_t
     ...
     integer :: nmatoms
   CONTAINS
     procedure :: run => spin_model_run
     procedure :: initialize=>spin_model_initialize
     procedure :: set_initial_spin => spin_model_set_initial_spin
     procedure :: finalize => spin_model_finalize
     procedure :: read_xml => spin_model_read_xml
     procedure :: make_supercell => spin_model_make_supercell
     procedure :: run_one_step => spin_model_run_one_step
     procedure :: run_time => spin_model_run_time
     procedure :: run_MvT => spin_model_run_MvT
     procedure :: run_MvH => spin_model_run_MvH
  end type spin_model_t
contains
  ! This is the main routine of all spin model.
  subroutine spin_model_run(self)
    !TODO add input as parameters.
    class(spin_model_t), intent(inout) :: self
  end subroutine spin_model_run
  
  ! initialize
  subroutine spin_model_initialize(self, xml_fname,  params)
    class(spin_model_t), intent(inout) :: self
  ...
  end subroutine spin_model_initialize

  ! finalize
  subroutine spin_model_finalize(self)
    class(spin_model_t), intent(inout) :: self
    call self%spin_primitive%finalize()
    ! TODO: finalize others
  end subroutine spin_model_finalize

  ! read xml file and set primitive cell parameters.
  subroutine spin_model_read_xml(self, xml_fname)
    class(spin_model_t), intent(inout) :: self
    character(len=40), intent(in) :: xml_fname
    call self%spin_primitive%read_xml(xml_fname)
  end subroutine spin_model_read_xml

  ! from primitive cell parameter make supercell and prepare calculator
  subroutine spin_model_make_supercell(self, sc_mat)
    class(spin_model_t), intent(inout) :: self
    integer , intent(in):: sc_mat(3, 3)
    call self%spin_primitive%make_supercell(sc_mat, self%spin_calculator)
  end subroutine spin_model_make_supercell

  subroutine spin_model_set_initial_spin(self, mode)
    class(spin_model_t), intent(inout) :: self
  ...
  end subroutine spin_model_set_initial_spin

  ! run one time step
  subroutine spin_model_run_one_step(self)
    class(spin_model_t), intent(inout) :: self
  ...
  end subroutine spin_model_run_one_step

  ! run all time step
  subroutine spin_model_run_time(self)
    class(spin_model_t), intent(inout) :: self
    real(dp) :: t
    integer :: i
    t=0.0d0
  ...
  end subroutine spin_model_run_time

  ! run spin .vs. temperature
  subroutine spin_model_run_MvT(self)
    class(spin_model_t), intent(inout) :: self
    !TODO
  end subroutine spin_model_run_MvT

  ! run spin .vs. external magnetic field.
  subroutine spin_model_run_MvH(self)
    class(spin_model_t), intent(inout) :: self
    !TODO
  end subroutine spin_model_run_MvH
