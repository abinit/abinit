...
void libxml_checkXML(char *filename,char *name_xml){  
  xmlDocPtr doc;
  xmlNodePtr cur;
...
void libxml_getDimsEffPot(char *filename,int *natom,int *ntypat, int *nph1l, int *nrpt){
  xmlDocPtr doc;
  int i,iatom,irpt,iph1l,itypat,present;
  xmlNodePtr cur,cur2;
  xmlChar *key,*uri;
  Array typat;
  ...
  while (cur != NULL) {
    ...
    if ((!xmlStrcmp(cur->name, (const  xmlChar *) "phonon"))) {
      ...
      while (cur2 != NULL) {
        if ((!xmlStrcmp(cur2->name, (const  xmlChar *) "qpoint"))) {iph1l++;}
        cur2 = cur2->next;
      }
    }
    cur = cur->next;
      ...
      while (cur2 != NULL) {
        if (iph1l<=*nph1l) {
          if ((!xmlStrcmp(cur2->name, (const  xmlChar *) "qpoint"))) {
            ...
            for(mu=0;mu<3;mu++){
              if (pch != NULL){
                qph1l[iph1l][mu]=strtod(pch,NULL);
                pch = strtok(NULL,"\t \n");
              }
            }
          }
          if ((!xmlStrcmp(cur2->name, (const  xmlChar *) "frequencies"))) {
            ...
            for(mu=0;mu<3**natom;mu++){
              if (pch != NULL){
                phfrq[iph1l][mu]=strtod(pch,NULL);
                pch = strtok(NULL,"\t \n");
              }
            }
          }
            ...
            for(ia=0;ia<*natom;ia++){
              for(mu=0;mu<3;mu++){
                for(ib=0;ib<*natom;ib++){
                  for(nu=0;nu<3;nu++){
                    if (pch != NULL){
                      dynmat[iph1l][ia][mu][ib][nu][0]=strtod(pch,NULL);
                      pch = strtok(NULL,"\t \n");
                    }
                  }
                }
...
void libxml_getValue(char *filename,char*name_key,char*result){
  xmlDocPtr doc;
  xmlNodePtr cur;
  xmlChar *key;
...
void libxml_getAttribute(char *filename,char*name_key,char*name_attributes,char*result){  
  xmlDocPtr doc;
  xmlNodePtr cur;
  xmlChar *key, *uri;
