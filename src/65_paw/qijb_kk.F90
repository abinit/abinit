...
!!
!! OUTPUT
!!  calc_qijb(2,lmn2max,natom,3) :: PAW on-site overlaps of wavefunctions at neighboring
!!                                   k points 
!!
!! SIDE EFFECTS
!!
...
!arrays
 ...
 real(dp),intent(in) :: dkvecs(3,3),expibi(2,natom,3),gprimd(3,3)
...
!    here is exp(-i b.R) for current atom: recall storage in expibi
     etb = cmplx(expibi(1,iatom,kdir),expibi(2,iatom,kdir))
...
&             *j_bessel(1:mesh_size,lbess+1)
             ...
             calc_qijb(1,klmn,iatom,kdir) = &
&             calc_qijb(1,klmn,iatom,kdir) + dreal(cterm)
             calc_qijb(2,klmn,iatom,kdir) = &
&             calc_qijb(2,klmn,iatom,kdir) + dimag(cterm)
