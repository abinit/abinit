!!****m* ABINIT/m_opernlb_gemm_ompgpu
!! NAME
!!  m_opernlb_gemm_ompgpu
!!
!! FUNCTION
!!
!! COPYRIGHT
...
!!
!! SOURCE
subroutine opernlb_gemm_ompgpu(choice,cplex,cplex_dgxdt,cplex_d2gxdt,cplex_fac,&
...
&       projs_r,projs_i,dprojs_r,dprojs_i,temp_realvec_r,temp_realvec_i,&
&       projs_recv,dprojs_recv,&
&       use_distrib)
...
!arrays
 ...
 real(dp),target,intent(out) :: vectout(:,:),svectout(:,:)
 real(dp),target,intent(in) :: d2gxdtfac(cplex,nd2gxdt,nprojs,ndat*nspinor)
 real(dp),target,intent(in) :: d2gxdtfac_sij(cplex_fac,nd2gxdt,nprojs,ndat*nspinor)
 real(dp),target,intent(inout) :: dgxdtfac(cplex,ndgxdt*nprojs,ndat*nspinor)
 real(dp),target,intent(inout) :: dgxdtfac_sij(cplex_fac,ndgxdt*nprojs,ndat*nspinor)
 real(dp),target,intent(in) :: gxfac(cplex,nprojs,ndat*nspinor)
 real(dp),target,intent(in) :: gxfac_sij(cplex_fac,nprojs,ndat*nspinor)
 real(dp),target,intent(inout) :: projs(:,:,:),projs_r(:,:,:),projs_i(:,:,:)
 real(dp),target,intent(inout) :: dprojs(:,:,:),dprojs_r(:,:,:),dprojs_i(:,:,:)
 real(dp),target,intent(out) :: projs_recv(:,:,:),dprojs_recv(:,:,:)
...
!Local variables-------------------------------
   ...
   if(cplex == 2) then
     if(.not. use_distrib) then
       if(choice==1 .or. choice==7) then
         !$OMP TARGET DATA USE_DEVICE_PTR(gxfac_sij,projs,svectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
         &    c_loc(projs), npw,&
         &    c_loc(gxfac_sij), nprojs, czero, c_loc(svectout), npw)
         !$OMP END TARGET DATA
       else if(choice==2) then
         !$OMP TARGET DATA USE_DEVICE_PTR(gxfac_sij,dprojs,svectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
         &    c_loc(dprojs), npw, &
         &    c_loc(gxfac_sij), nprojs, czero, c_loc(svectout), npw)
         !$OMP END TARGET DATA
         !$OMP TARGET DATA USE_DEVICE_PTR(dgxdtfac_sij,projs,svectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
         &    c_loc(projs), npw, &
         &    c_loc(dgxdtfac_sij), nprojs, cone, c_loc(svectout), npw)
         !$OMP END TARGET DATA
       else if(choice==3) then
         if(idir<=3) then
           !$OMP TARGET MAP(to:dgxdtfac_sij,gxfac_sij)
         ...
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
         &    c_loc(projs), npw, &
         &    c_loc(dgxdtfac_sij), nprojs, czero, c_loc(svectout), npw)
         !$OMP END TARGET DATA
         !$OMP TARGET DATA USE_DEVICE_PTR(gxfac_sij,dprojs,svectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cminusone, &
         &    c_loc(dprojs), npw, &
         &    c_loc(gxfac_sij), nprojs, cone, c_loc(svectout), npw)
         !$OMP END TARGET DATA
     ...
     !$OMP TARGET DATA USE_DEVICE_PTR(gxfac_sij,projs_r,temp_realvec_r)
     call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
     &          c_loc(projs_r), npw, &
     &          c_loc(gxfac_sij), nprojs, czero, c_loc(temp_realvec_r), npw)
     !$OMP END TARGET DATA
     !$OMP TARGET DATA USE_DEVICE_PTR(gxfac_sij,projs_i,temp_realvec_i)
     call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
     &          c_loc(projs_i), npw,&
     &          c_loc(gxfac_sij), nprojs, czero, c_loc(temp_realvec_i), npw)
     !$OMP END TARGET DATA
 ...
 if(paw_opt == 0 .or. paw_opt == 1 .or. paw_opt == 4) then
   ...
   if(cplex_fac == 2) then
     if(.not. use_distrib) then
       if(choice==1 .or. choice==7) then
         !$OMP TARGET DATA USE_DEVICE_PTR(gxfac,projs,vectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
         &        c_loc(projs), npw, &
         &        c_loc(gxfac), nprojs, czero, c_loc(vectout), npw)
         !$OMP END TARGET DATA
       else if(choice==2) then
         !$OMP TARGET DATA USE_DEVICE_PTR(gxfac,dprojs,vectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
         &    c_loc(dprojs), npw, &
         &    c_loc(gxfac), nprojs, czero, c_loc(vectout), npw)
         !$OMP END TARGET DATA
         !$OMP TARGET DATA USE_DEVICE_PTR(dgxdtfac,projs,vectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
         &    c_loc(projs), npw, &
         &    c_loc(dgxdtfac), nprojs, cone, c_loc(vectout), npw)
         !$OMP END TARGET DATA
       else if(choice==3) then
         if(idir<=3) then
           !$OMP TARGET MAP(to:dgxdtfac,gxfac)
         ...
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
         &    c_loc(projs), npw, &
         &    c_loc(dgxdtfac), nprojs, czero, c_loc(vectout), npw)
         !$OMP END TARGET DATA
         !$OMP TARGET DATA USE_DEVICE_PTR(gxfac,dprojs,vectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cminusone, &
         &    c_loc(dprojs), npw, &
         &    c_loc(gxfac), nprojs, cone, c_loc(vectout), npw)
         !$OMP END TARGET DATA
       else if(choice==5) then
         !$OMP TARGET DATA USE_DEVICE_PTR(dgxdtfac,projs,vectout)
         call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
     ...
     !$OMP TARGET DATA USE_DEVICE_PTR(gxfac,projs_r,temp_realvec_r)
     call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
     &        c_loc(projs_r), npw, &
     &        c_loc(gxfac), nprojs, czero, c_loc(temp_realvec_r), npw)
     !$OMP END TARGET DATA
     !$OMP TARGET DATA USE_DEVICE_PTR(gxfac,projs_i,temp_realvec_i)
     call abi_gpu_xgemm(cplex, 'N', 'N', npw, ndat*nspinor, nprojs, cone, &
     &        c_loc(projs_i), npw, &
     &        c_loc(gxfac), nprojs, czero, c_loc(temp_realvec_i), npw)
     !$OMP END TARGET DATA
...
end subroutine opernlb_gemm_ompgpu

end module m_opernlb_gemm_ompgpu
