     ...
     do ia=1,nincat
       do ilmn=1,nlmn
         do ii=1,cplex
           ...
           & enl(iln,itypat,ispinor+shift,min(ndat_enl,idat),iphase)*gx(ii,ilmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)
         end do
       end do
     end do
   end do
...
!  1-Enl is real
   if (cplex_enl==1) then
     if (paw_opt==2) then
       ...
       !$OMP& MAP(to:gxfac_,enl,atindx1,gx,sij,lambda) &
       !$OMP& PRIVATE(idat,ispinor) &
       !$OMP& IF(gpu_option==ABI_GPU_OPENMP)
       do idat=1,ndat
       do ispinor=1,nspinor
...
&                 gxfac_(ii,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor) + &
&                 (enl(j0lmn+jlmn,index_enl,ispinor_index,min(ndat_enl,idat),iphase)-lambda(idat) * sij(j0lmn+jlmn)) * &
&                 gx(ii,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)
             ...
             do ilmn=1,jlmn-1
               do ii=1,cplex
                 gxfac_(ii,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)=&
&                   gxfac_(ii,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor) + &
&                   (enl(j0lmn+ilmn,index_enl,ispinor_index,min(ndat_enl,idat),iphase)-lambda(idat) * sij(j0lmn+ilmn)) * &
...
&                     (enl(i0lmn+jlmn,index_enl,ispinor_index,min(ndat_enl,idat),iphase)-lambda(idat) * sij(i0lmn+jlmn)) *&
&                     gx(ii,ilmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)
                 end do
               end do
             end if
       ...
       if(ndat_enl==1) then
         ...
         do ispinor=1,nspinor
           do ia=1,nincat
             index_enl=atindx1(iatm+ia)
             !$OMP PARALLEL DO PRIVATE(jlmn)
             do jlmn=1,nlmn
               gxfac_(1:cplex,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)=&
&                  gxfac_(1:cplex,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor) + &
&                  enl_ptr2(jlmn*(jlmn-1)/2+jlmn,index_enl,ispinor+shift,1) * &
&                  gx(1:cplex,ibeg+jlmn+(ia-1)*nlmn,ispinor+(idat-1)*nspinor)
         ...
         !$OMP& PRIVATE(idat,ispinor,ispinor_index,ia,index_enl,tmp,jlmn,ilmn)
         do idat=1,ndat
         do ispinor=1,nspinor
           do ia=1,nincat
             do jlmn=1,nlmn
               ...
               tmp(1:cplex)=0
               do ilmn=1,jlmn-1
                 tmp(1:cplex)=tmp(1:cplex) + enl_ptr(jlmn*(jlmn-1)/2+ilmn,index_enl,ispinor+shift) &
&                    *gx(1:cplex,ibeg+ilmn+(ia-1)*nlmn,ispinor+(idat-1)*nspinor)
               end do
               if(jlmn<nlmn) then
                 do ilmn=jlmn+1,nlmn
                   tmp(1:cplex)=tmp(1:cplex) + enl_ptr((ilmn*(ilmn-1)/2)+jlmn,index_enl,ispinor+shift) &
&                      *gx(1:cplex,ibeg+ilmn+(ia-1)*nlmn,ispinor+(idat-1)*nspinor)
                 end do
               end if
               gxfac_(1:cplex,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)= &
&                  gxfac_(1:cplex,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)+tmp(1:cplex)
             end do
           end do
         end do
         end do
         ...
         !$OMP TARGET TEAMS DISTRIBUTE COLLAPSE(3) &
         !$OMP& MAP(to:enl_ptr2,atindx1,gx,gxfac_) &
         !$OMP& PRIVATE(idat,ispinor,ispinor_index,ia,index_enl,tmp)
         do idat=1,ndat
         do ispinor=1,nspinor
           do ia=1,nincat
             index_enl=atindx1(iatm+ia)
             ...
             do jlmn=1,nlmn
               gxfac_(1:cplex,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)=&
&                  gxfac_(1:cplex,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor) + &
&                  enl_ptr2(jlmn*(jlmn-1)/2+jlmn,index_enl,ispinor+shift,idat) * &
&                  gx(1:cplex,ibeg+jlmn+(ia-1)*nlmn,ispinor+(idat-1)*nspinor)
             end do
           end do
         end do
         ...
         !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(4) &
         !$OMP& MAP(to:enl_ptr2,atindx1,gx,gxfac_) &
         !$OMP& PRIVATE(idat,ispinor,ispinor_index,ia,index_enl,tmp,j0lmn,jlmn,ilmn)
         do idat=1,ndat
         do ispinor=1,nspinor
           do ia=1,nincat
             do jlmn=1,nlmn
               ...
               tmp(1:cplex)=0
               do ilmn=1,jlmn-1
                 tmp(1:cplex)=tmp(1:cplex) + enl_ptr2(j0lmn+ilmn,index_enl,ispinor_index,idat) &
&                    * gx(1:cplex,ibeg+ilmn+(ia-1)*nlmn,ispinor+(idat-1)*nspinor)
               end do
               if(jlmn<nlmn) then
                 do ilmn=jlmn+1,nlmn
                   i0lmn=(ilmn*(ilmn-1)/2)
                   tmp(1:cplex)=tmp(1:cplex) + enl_ptr2(i0lmn+jlmn,index_enl,ispinor_index,idat) &
&                     *gx(1:cplex,ibeg+ilmn+(ia-1)*nlmn,ispinor+(idat-1)*nspinor)
                 end do
               end if
               gxfac_(1:cplex,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)= &
...
!    2-Enl is complex  ===== D^ss'_ij=D^s's_ji^*
     ...
     if (nspinortot==1) then ! -------------> NO SPINORS
       if(paw_opt==2) then
         ...
         !$OMP& MAP(to:gxfac_,gx,gxi,atindx1,gxj,sij,enl,lambda) &
         ...
         do ia=1,nincat
           do jlmn=1,nlmn
             index_enl=atindx1(iatm+ia)
             j0lmn=jlmn*(jlmn-1)/2
             enl_(1)=enl(2*j0lmn+jlmn-1,index_enl,1,min(ndat_enl,idat),iphase)-lambda(idat)*sij(j0lmn+jlmn)
...
&                 gxfac_(2,jlmn+(ia-1)*nlmn+ibeg,1+nspinor*(idat-1))+enl_(1)*gxj(2)
             ...
             do ilmn=1,jlmn-1
               enl_(1:2)=enl(2*j0lmn+ilmn-1:2*j0lmn+ilmn,index_enl,1,min(ndat_enl,idat),iphase)
...
&                   gxfac_(2,ilmn+(ia-1)*nlmn+ibeg,1+nspinor*(idat-1))+enl_(1)*gxj(2)
             ...
             if(jlmn<nlmn) then
               do ilmn=jlmn+1,nlmn
                 i0lmn=ilmn*(ilmn-1)/2
                 enl_(1:2)=enl(2*i0lmn+jlmn-1:2*i0lmn+jlmn,index_enl,1,min(ndat_enl,idat),iphase)
...
&                     gxfac_(2,jlmn+(ia-1)*nlmn+ibeg,1+nspinor*(idat-1))+enl_(1)*gxi(2)
         ...
         !$OMP& MAP(to:gxfac_,gx,gxi,atindx1,gxj,enl) &
         !$OMP& PRIVATE(idat,ia,index_enl,jlmn,j0lmn,enl_,gxj,ilmn,i0lmn,gxi) &
         !$OMP& IF(gpu_option==ABI_GPU_OPENMP)
         do idat=1,ndat
         do ia=1,nincat
           ...
           index_enl=atindx1(iatm+ia)
             j0lmn=jlmn*(jlmn-1)/2
             enl_(1)=enl(2*j0lmn+jlmn-1,index_enl,1,min(ndat_enl,idat),iphase)
             gxj(1:cplex)=gx(1:cplex,jlmn+(ia-1)*nlmn+ibeg,1)
             gxfac_(1,jlmn+(ia-1)*nlmn+ibeg,1+nspinor*(idat-1)) = &
&               gxfac_(1,jlmn+(ia-1)*nlmn+ibeg,1+nspinor*(idat-1))+enl_(1)*gxj(1)
             if (cplex==2) then
...
!-------------------------------------------------------------------
  if (optder>=1.and.paw_opt==0) then
   ...
   !$OMP TARGET TEAMS DISTRIBUTE COLLAPSE(2) &
   !$OMP& MAP(to:dgxdtfac_,enl,atindx1,dgxdt,indlmn) &
     ...
     do ia=1,nincat
       do ilmn=1,nlmn
         do mu=1,ndgxdtfac
           ...
           &    enl(indlmn(5,ilmn,itypat),itypat,ispinor_index,min(ndat_enl,idat),iphase)&
         ...
         do ia=1,nincat
           do jlmn=1,nlmn
             ...
             do mu=1,ndgxdtfac
               ...
               &    dgxdtfac_(1:cplex,mu,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)&
               &    + enl(j0lmn+jlmn,index_enl,ispinor_index,min(ndat_enl,idat),iphase) &
             ...
             do ilmn=1,jlmn-1
               do mu=1,ndgxdtfac
                 dgxdtfac_(1:cplex,mu,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)=&
                 &    dgxdtfac_(1:cplex,mu,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)&
                 &    + enl(j0lmn+ilmn,index_enl,ispinor_index,min(ndat_enl,idat),iphase) &
                 &    * dgxdt(1:cplex,mu,ilmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)
               end do
             end do
             if(jlmn<nlmn) then
               do ilmn=jlmn+1,nlmn
                 ...
                 do mu=1,ndgxdtfac
                   dgxdtfac_(1:cplex,mu,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)=&
                   &    dgxdtfac_(1:cplex,mu,jlmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)&
                   &    + enl(i0lmn+jlmn,index_enl,ispinor_index,min(ndat_enl,idat),iphase) &
                   &    * dgxdt(1:cplex,mu,ilmn+(ia-1)*nlmn+ibeg,ispinor+(idat-1)*nspinor)
                 end do
               end do
             end if
     ...
     if (nspinortot==1) then ! -------------> NO SPINORS
       ...
       do ia=1,nincat
         do jlmn=1,nlmn
           ...
           j0lmn=jlmn*(jlmn-1)/2
           jjlmn=j0lmn+jlmn
           enl_(1)=enl(2*jjlmn-1,index_enl,1,min(ndat_enl,idat),iphase)
           ...
           do ilmn=1,jlmn-1
             ijlmn=j0lmn+ilmn
             enl_(1:2)=enl(2*ijlmn-1:2*ijlmn,index_enl,1,min(ndat_enl,idat),iphase)
