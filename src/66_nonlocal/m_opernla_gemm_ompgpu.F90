!!****m* ABINIT/m_opernla_gemm_ompgpu
!! NAME
!!  m_opernla_gemm_ompgpu
!!
!! FUNCTION
!!
!! COPYRIGHT
...
subroutine opernla_gemm_ompgpu(choice,cplex,cplex_dgxdt,cplex_d2gxdt,d2gxdt,dgxdt,gx,&
...
!arrays
 ...
 real(dp),target,intent(out) :: d2gxdt(cplex,nd2gxdt,nprojs,ndat*nspinor)
 real(dp),target,intent(out) :: dgxdt(cplex,ndgxdt*nprojs,ndat*nspinor),gx(cplex,nprojs,ndat*nspinor)
 real(dp),target,intent(inout) :: projs(:,:,:),projs_r(:,:,:),projs_i(:,:,:)
 real(dp),target,intent(inout) :: dprojs(:,:,:),dprojs_r(:,:,:),dprojs_i(:,:,:)
 real(dp),target,intent(out) :: projs_recv(:,:,:),dprojs_recv(:,:,:),temp_realvec(:)
...
!Local variables-------------------------------
 ...
 if(cplex == 2) then
   if(.not. use_distrib) then
     !$OMP TARGET DATA USE_DEVICE_PTR(gx,projs,vectin)
     call abi_gpu_xgemm(cplex, 'C', 'N', nprojs, ndat*nspinor, npw, cone, &
     &          c_loc(projs), npw,&
     &          c_loc(vectin), npw, czero, c_loc(gx), nprojs)
     !$OMP END TARGET DATA
     if(ndgxdt>0 .and. cpopt<=3) then
       !$OMP TARGET DATA USE_DEVICE_PTR(dgxdt,dprojs,vectin)
       call abi_gpu_xgemm(cplex, 'C', 'N', ndgxdt*nprojs, ndat*nspinor, npw, cone, &
       &      c_loc(dprojs), npw,&
       &      c_loc(vectin), npw, czero, c_loc(dgxdt), ndgxdt*nprojs)
       !$OMP END TARGET DATA
     end if
   else
   ...
   !$OMP TARGET DATA USE_DEVICE_PTR(gx,projs_r,temp_realvec)
   call abi_gpu_xgemm(cplex, 'T', 'N', nprojs, ndat*nspinor, npw, cone, &
   &             c_loc(projs_r), npw, &
   &             c_loc(temp_realvec), npw, czero, c_loc(gx), nprojs)
   !$OMP END TARGET DATA
   if(signs==1.and.ndgxdt>0) then
     !$OMP TARGET DATA USE_DEVICE_PTR(dgxdt,dprojs_r,temp_realvec)
     call abi_gpu_xgemm(cplex, 'T', 'N', ndgxdt*nprojs, ndat*nspinor, npw, cone, &
     &            c_loc(dprojs_r), npw, &
     &            c_loc(temp_realvec), npw, czero, c_loc(dgxdt), ndgxdt*nprojs)
     !$OMP END TARGET DATA
   end if
   ...
   !$OMP TARGET DATA USE_DEVICE_PTR(gx,projs_i,temp_realvec)
   call abi_gpu_xgemm(cplex, 'T', 'N', nprojs, ndat*nspinor, npw, cone, &
            c_loc(projs_i), npw, &
            c_loc(temp_realvec), npw, cone , c_loc(gx), nprojs)
   !$OMP END TARGET DATA
   ...
   if(signs==1.and.ndgxdt>0) then
     !$OMP TARGET DATA USE_DEVICE_PTR(dgxdt,dprojs_i,temp_realvec)
     call abi_gpu_xgemm(cplex, 'T', 'N', ndgxdt*nprojs, ndat*nspinor, npw, cone, &
               c_loc(dprojs_i), npw, &
               c_loc(temp_realvec), npw, cone , c_loc(dgxdt), ndgxdt*nprojs)
     !$OMP END TARGET DATA
...
end subroutine opernla_gemm_ompgpu
end module m_opernla_gemm_ompgpu
