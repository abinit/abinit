...
!scalars
 ...
 integer, intent(in) :: ntypat,optforces,optres,pwind_alloc,usecprj,usevxctau,with_vectornd
...
!arrays
 ...
 real(dp), intent(inout) :: vectornd(with_vectornd*nfftf,dtset%nspden,3),vtrial(nfftf,dtset%nspden)
 ...
 real(dp), intent(inout),optional :: vxctau(nfftf,dtset%nspden,4*usevxctau)
               ...
               if((paw_dmft%band_in(iband).and.paw_dmft%band_in(iband1)).or. &
                & ((paw_dmft%dmft_solv == 6 .or. paw_dmft%dmft_solv == 7).and.iband==iband1)) then
                 ...
                 ekindmft2=ekindmft2  -  dtset%wtk(ikpt)*paw_dmft%occnd(2,iband,iband1,ikpt,isppol)*&
                   eknk_nd(2,iband,iband1,ikpt,isppol)
...
!  Compute density residual (if required) and its squared norm
   if (iscf>=0) then
     ...
     if (optres==1) then
       ...
       ! LB-03/2026:
       ! With threads, a noise accumulates in nvresid after each SCF cycle,
       ! resulting in different densities/potentials for different MPI processes,
       ! whereas they should be equals.
       ! This slowly worsens the SCF cycle, leading to wrong results after many iterations.
       ! So here we compute the mean of nvresid over all the MPI processes to reduce the noise.
