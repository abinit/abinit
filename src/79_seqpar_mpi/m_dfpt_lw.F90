...
!If GGA xc first calculate the Cartesian q-gradient of the xc kernel
 ...
 do iq1grad=1,nq1grad
   ...
   do iatpert=1,natpert
     ...
     !Calculate the gradient of the Hartree potential generated by the first order atomic displacement density
...
!==== Initialize response functions files and handlers ====
 ...
 do iatpert=1,natpert
   ...
   call wfk_open_read(wfk_t_atdis(iatpert),fiwfatdis,formeig1,dtset%iomode,forunit, xmpi_comm_self)
 end do
...
& dtset%tphysel,dtset%tsmear,dtset%occopt,occ_rbz,wtk_rbz, dtset%cellcharge(1), dtset%kptopt, &
...
!      this term)
       call dfpt_isdqfr(atindx,cg,dtset,frwfdq_k,gs_hamkq,gsqcut,icg,ikpt,&
