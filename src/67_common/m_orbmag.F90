...
subroutine make_onsite_l(atindx1,cprj,dtset,idir,mcprj,mpi_enreg,nband_k,onsite_l,pawrad,pawtab)
  ...
  !Local variables -------------------------
  !scalars
  integer :: icprj,ierr,ikpt,isppol,my_nspinor,ncpgr,nproc,spaceComm
...
subroutine mpi_ctocprjb(atindx1,cg,cprj_kb_k,dtorbmag,dtset,gmet,gprimd,&
  ...
  real(dp),intent(in) :: cg(2,mcg),gmet(3,3),gprimd(3,3),rmet(3,3),rprimd(3,3),xred(3,dtset%natom)
  type(pawtab_type),intent(in) :: pawtab(dtset%ntypat*psps%usepaw)
  type(pawcprj_type),intent(inout) :: cprj_kb_k(dtset%mkmem,6,0:4,dtset%natom,dtorbmag%nspinor*dtset%mband)
           ...
           do gdir=1, 3
              ...
              do gfor=1, 2
                 ...
                 do iband = 1, nband_k
                    ...
                    call pawcprj_put(atindx1,cwaveprj,cprj_kb_k(ikpt,bdx,gdxstor,:,:),dtset%natom,&
                         & iband,0,ikpt,0,1,nband_k,1,dtset%natom,1,nband_k,dimlmn,dtset%nspinor,dtset%nsppol,0)
...
subroutine mpi_orbmag(atindx1,cg,cprj,dtset,dtorbmag,kg,&
 ...
 ABI_ALLOCATE(eeig,(nband_k,dtset%mkmem))
 call mpi_eeig(cg,dtset,eeig,mcg,mcprj,mpi_enreg,nband_k,nfftf,paw_ij,pawfgr,pawtab,psps,rprimd,&
     & vhartr,vpsp,vxc,usecprj,xred)
...
subroutine mpi_eeig(cg,dtset,eeig,mcg,mcprj,mpi_enreg,nband_k,nfftf,paw_ij,pawfgr,pawtab,psps,rprimd,&
     & vhartr,vpsp,vxc,usecprj,xred)
 ...
 real(dp),intent(in) :: cg(2,mcg),rprimd(3,3)
 real(dp),intent(in) :: vhartr(nfftf),vpsp(nfftf),vxc(nfftf,dtset%nspden),xred(3,dtset%natom)
 real(dp),intent(out) :: eeig(nband_k,dtset%mkmem)
 type(paw_ij_type),intent(inout) :: paw_ij(dtset%natom*psps%usepaw)
 type(pawfgr_type),intent(in) :: pawfgr
 type(pawtab_type),intent(in) :: pawtab(dtset%ntypat*psps%usepaw)
 ...
 !Local variables -------------------------
 !scalars
 integer :: isppol,ngfft1,ngfft2,ngfft3,ngfft4,ngfft5,ngfft6
 type(gs_hamiltonian_type) :: gs_hamk

 !arrays
 real(dp) :: rhodum(1)
 real(dp),allocatable :: cgrvtrial(:,:),vlocal(:,:,:,:),vtrial(:,:)
