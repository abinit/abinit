...
subroutine make_S1trace_k_n(atindx,cprj_k,dtset,ENK,iband,nband_occ,pawtab,S1trace)

  !Arguments ------------------------------------
  !scalars
  integer,intent(in) :: iband,nband_occ
  real(dp),intent(in) :: ENK
  type(dataset_type),intent(in) :: dtset

  !arrays
  integer,intent(in) :: atindx(dtset%natom)
  complex(dpc),intent(out) :: S1trace(3)
...
subroutine make_rhorij1_k_n(atindx,cprj_k,dtset,iband,nband_occ,&
  ...
  !arrays
  integer,intent(in) :: atindx(dtset%natom)
  complex(dpc),intent(out) :: rhorij1(3)
  type(pawcprj_type),intent(in) :: cprj_k(dtset%natom,nband_occ)
    ...
    do epsabg = 1, -1, -2
      ...
      do iat=1,dtset%natom
        ...
        if (paw_ij(iat)%lmn2_size .NE. pawtab(itypat)%lmn2_size ) then
          ABI_BUG('lmn2_size mismatch in make_rhorij1_k_n')
        end if
        do ilmn=1,pawtab(itypat)%lmn_size
          do jlmn=1,pawtab(itypat)%lmn_size
            klmn=MATPACK(jlmn,ilmn)
            ...
            cpk=cmplx(cprj_k(iatom,iband)%dcp(1,gdir,jlmn),cprj_k(iatom,iband)%dcp(2,gdir,jlmn),KIND=dpc)
            if (cplex_dij) then
               cdij=cmplx(paw_ij(iat)%dij(2*klmn-1,1),paw_ij(iat)%dij(2*klmn,1),KIND=dpc)
...
!! This routine computes the orbital magnetization and Berry curvature based on input 
!! wavefunctions and DDK wavefuntions. It is based on using the modern theory of
!! orbital magnetism directly. It is assumed that only completely filled bands are present.
!!
!! COPYRIGHT
!! Copyright (C) 2003-2021 ABINIT  group
!! This file is distributed under the terms of the
...
subroutine orbmag_modmag(cg,cg1,cprj,dtset,gsqcut,kg,mcg,mcg1,mcprj,mpi_enreg,&
    & nfftf,ngfftf,npwarr,occ,paw_ij,pawang,pawfgr,pawrad,pawtab,psps,rprimd,vtrial,&
    & xred,ylm,ylmgr)
 ...
 integer,parameter :: om1=1,om2=2,om3=3,om4=4,om5=5,berrycurve_pw=6,berrycurve_qij=7,berrycurve_dij=8
 ...
 real(dp),allocatable :: vectornd(:,:),vectornd_pac(:,:,:,:,:),vlocal(:,:,:,:)
 real(dp),allocatable :: ylm_k(:,:),ylmgr_k(:,:,:)
 complex(dpc) :: bckn(3,3),ddijkn(3),dqijkn(3),olkn(3),omdp(3),ompw(3)
 complex(dpc),allocatable :: dldij(:,:,:,:),dlrij(:,:,:,:)
 ...
 ! compute first order Dij terms d/dk(left) Dij and bare d/dk(left) term for Berry
 lmn_size_max = 0
 do itypat=1,dtset%ntypat
 ...
 ABI_MALLOC(dlrij,(dtset%ntypat,lmn_size_max,lmn_size_max,3))
 call make_dldij(dldij,dlrij,dtset,gprimd,lmn_size_max,pawang,pawrad,pawtab,rhoij)
 ...
 do ikpt = 1, dtset%nkpt
   ...
   do nn = 1, nband_k
     ...
     call berrycurve_k_n(atindx,bckn,cg_k,cg1_k,cprj_k,cprj1_k,dlrij,dtset,&
       & gprimd,nn,lmn_size_max,mcgk,nband_k,nattyp,npw_k,pawtab)
 ...
 do nn = 1, nband_k
   ...
   orbmag_terms(1:3,om5,nn) =  ucvol*MATMUL(gprimd,orbmag_terms(1:3,om5,nn))
 ...
 call orbmag_modmag_output(dtset,local_fermie,nband_k,nterms,orbmag_terms,orbmag_trace)
...
subroutine orbmag_modmag_pw_k_n(atindx,cg_k,cg1_k,cprj_k,cprj1_k,dimlmn,dtset,Enk,&
 ...
 do adir = 1, 3
   ...
   call pawcprj_get(atindx,cwaveprj1,cprj1_k(:,:,adir),dtset%natom,iband,0,ikpt,0,isppol,dtset%mband,&
        &           dtset%mkmem,dtset%natom,1,nband_k,my_nspinor,dtset%nsppol,0)
   call getghc(cpopt,cwavef,cwaveprj,ghc,gsc,gs_hamk,gvnlc,lambda,mpi_enreg,ndat,&
        &           prtvol,sij_opt,tim_getghc,type_calc)
     ...
     do iat=1,natom
       ...
       do ilmn=1,pawtab(itypat)%lmn_size
         do jlmn=1,pawtab(itypat)%lmn_size
           ...
           ! orbmag_modmag_pw_k_n
           omdp(adir) = omdp(adir) + half*j_dpc*epsabg*(dij+qij)*&
...
!! paw_dmft code in orbmag_modmag
!! Direct questions and comments to J Zwanziger
!!
!! PARENTS
!!
...
!!
!! FUNCTION
!! Compute Dij term arising from d/dk (left) (Dij)
!!
!! COPYRIGHT
!! Copyright (C) 2003-2020 ABINIT  group
!! This file is distributed under the terms of the
...
subroutine make_dldij(dldij,dlrij,dtset,gprimd,lmn_size_max,pawang,pawrad,pawtab,rhoij)
 ...
 complex(dpc),intent(out) :: dlrij(dtset%ntypat,lmn_size_max,lmn_size_max,3)
 type(pawrad_type),intent(in) :: pawrad(dtset%ntypat)
 ...
 ! bare <phi|(r-R)_alpha|phi> term for Berry
 call make_dlrij(dlrij,dtset,gntselect,gprimd,l_max,lmn_size_max,pawrad,pawtab,realgnt)
...
!!
!! FUNCTION
!! Compute ij term arising from <d\phi_i/dk|\phi_j> needed for Berry
!!
!! COPYRIGHT
!! Copyright (C) 2003-2020 ABINIT  group
!! This file is distributed under the terms of the
...
subroutine make_dlrij(dlrij,dtset,gntselect,gprimd,l_max,lmn_size_max,pawrad,pawtab,realgnt)
 ...
 ! so adir 2 (y) gets mapped to ilm 2
 ! so adir 3 (z) gets mapped to ilm 3
 ! constant of proportionality is \sqrt(4\pi/3)
 ...
 do itypat=1,dtset%ntypat
   ...
   do ijlmn=1,pawtab(itypat)%lmn2_size
     ...
     do adir = 1, 3
       ...
       do imesh = 2, mesh_size
         ff(imesh) = (pawtab(itypat)%phiphj(imesh,ijln) - pawtab(itypat)%tphitphj(imesh,ijln)) * &
     ...
     ! convert from cartesian axes to reduced coords
     dlij_red(1:3) = MATMUL(TRANSPOSE(gprimd),dlij_cart(1:3))
     dlrij(itypat,ilmn,jlmn,1:3) = dlij_red(1:3)
     dlrij(itypat,jlmn,ilmn,1:3) = dlij_red(1:3)
...
!!
!! FUNCTION
!! Compute Dij term arising from <d\phi_i/dk|v_H[n_Zc]|\phi_j>
!!
!! COPYRIGHT
!! Copyright (C) 2003-2020 ABINIT  group
!! This file is distributed under the terms of the
...
subroutine make_dldij_vhnzc(dldij_vhnzc,dtset,gntselect,gprimd,&
 ...
 do itypat=1,dtset%ntypat
   ...
   if( (pawtab(itypat)%has_vhnzc .NE. 2) .OR. &
     & (pawtab(itypat)%has_vhtnzc .NE. 2) ) then 
     ABI_ERROR("vhnzc or vhtnzc not defined in pawtab!") 
   end if
...
!!****f* ABINIT/make_dldij_kinetic
!! NAME
!! make_dldij_kinetic
!!
!! FUNCTION
!! Compute Dij term arising from <d\phi_i/dk|-1/2\nabla^2|\phi_j>
!!
!! COPYRIGHT
!! Copyright (C) 2003-2020 ABINIT  group
!! This file is distributed under the terms of the
...
subroutine make_dldij_kinetic(dldij_kinetic,dtset,gntselect,gprimd,&
 ...
 real(dp) :: c1,intg,rr,rr2,the_gnt

 !arrays
 integer,dimension(3) :: idirindx = (/4,2,3/)
 complex(dpc) :: dkij_cart(3),dkij_red(3)
 real(dp),allocatable :: ff(:),tujp(:),tujpp(:),ujp(:),ujpp(:)
 ...
 do itypat=1,dtset%ntypat
   ...
   ABI_MALLOC(ff,(mesh_size))
   ABI_MALLOC(ujp,(mesh_size))
   ABI_MALLOC(ujpp,(mesh_size))
   ABI_MALLOC(tujp,(mesh_size))
   ABI_MALLOC(tujpp,(mesh_size))
   ...
   do jlmn=1,pawtab(itypat)%lmn_size
     ...
     call nderiv_gen(ujp,pawtab(itypat)%phi(:,jln),pawrad(itypat),ujpp)
     call nderiv_gen(tujp,pawtab(itypat)%tphi(:,jln),pawrad(itypat),tujpp)
     ...
     do ilmn=1,pawtab(itypat)%lmn_size
       ...
       do adir = 1, 3
         ...
         do imesh = 2, mesh_size
           rr = pawrad(itypat)%rad(imesh)
           rr2 = rr*rr
           ff(imesh) = pawtab(itypat)%phi(imesh,iln)*ujpp(imesh) 
           ff(imesh) = ff(imesh) - pawtab(itypat)%tphi(imesh,iln)*tujpp(imesh)
           ff(imesh) = ff(imesh) - jl*(jl+one)*(&
             & pawtab(itypat)%phi(imesh,iln)*pawtab(itypat)%phi(imesh,jln) - &
             & pawtab(itypat)%tphi(imesh,iln)*pawtab(itypat)%tphi(imesh,jln))/rr2
           ff(imesh) = ff(imesh)*rr
         end do
         call pawrad_deducer0(ff,mesh_size,pawrad(itypat))
         call simp_gen(intg,ff,pawrad(itypat))

         dkij_cart(adir) = -half*j_dpc*c1*the_gnt*intg 

       end do ! end loop over adir

       ! convert from cartesian axes to reduced coords
       dkij_red(1:3) = MATMUL(TRANSPOSE(gprimd),dkij_cart(1:3))
       dldij_kinetic(itypat,ilmn,jlmn,1:3) = dkij_red(1:3)
...
subroutine orbmag_modmag_ddij_k_n(atindx,cprj_k,cprj1_k,ddijkn,dldij,dtset,gprimd,&
 ...
 ! in abinit, exp(i k.r) is used not exp(i 2\pi k.r) so the following
 ! term arises to properly normalize the derivatives (there are two in this expression
 ! one for each wavefunction derivative) 
 c2=1.0d0/(two_pi*two_pi)
     ...
     do iat=1,dtset%natom
       ...
       do ilmn=1,pawtab(itypat)%lmn_size
         do jlmn=1,pawtab(itypat)%lmn_size
           ...
           dqijterm = dqijterm + j_dpc*epsabg*(cpi*dlrij(itypat,ilmn,jlmn,bdir)*(dup_g+udp_g) + &
...
subroutine berrycurve_k_n(atindx,bckn,cg_k,cg1_k,cprj_k,cprj1_k,&
    & dlrij,dtset,gprimd,iband,lmn_size_max,mcgk,nband_k,nattyp,npw_k,pawtab)

 !Arguments ------------------------------------
 !scalars
 integer,intent(in) :: iband,lmn_size_max,mcgk,nband_k,npw_k
 type(dataset_type),intent(in) :: dtset
 ...
 !arrays
 real(dp),allocatable :: cwaveb(:,:),cwaveg(:,:)

 !-----------------------------------------------------------------------

 ABI_MALLOC(cwaveb,(2,npw_k))
 ...
 do adir = 1, 3
   ...
   do epsabg = 1, -1, -2
     ...
     cwaveb = cg1_k(:,(iband-1)*npw_k+1:iband*npw_k,bdir)
     cwaveg = cg1_k(:,(iband-1)*npw_k+1:iband*npw_k,gdir)

     dotr =  DOT_PRODUCT(cwaveb(1,:),cwaveg(1,:)) + DOT_PRODUCT(cwaveb(2,:),cwaveg(2,:))
       ...
       do ilmn=1,pawtab(itypat)%lmn_size
         do jlmn=1,pawtab(itypat)%lmn_size
           ...
           qijterm = qijterm + j_dpc*epsabg*&
             & (cdup_b+cudp_b)*pawtab(itypat)%sij(klmn)*(dup_g+udp_g)

           dijterm = dijterm + j_dpc*epsabg*(cpi*dlrij(itypat,ilmn,jlmn,bdir)*(dup_g+udp_g) + &
             & (cdup_b+cudp_b)*conjg(dlrij(itypat,ilmn,jlmn,gdir))*cpj)
...
subroutine orbmag_gipaw_output(dtset,nband_k,nterms,orbmag_terms,orbmag_trace,bc_tensor)
 ...
 write(message,'(a,a)')' Orbital magnetization computed with DFPT derivative wavefunctions ',ch10
 call wrtout(ab_out,message,'COLL')

 write(message,'(a)')' Orbital magnetization, Cartesian directions : '
 call wrtout(ab_out,message,'COLL')
 ...
 if(dtset%orbmag .GE. 22) then
   write(message,'(a)')ch10
   call wrtout(ab_out,message,'COLL')
   write(message,'(a)')' Orbital magnetization, Term-by-term breakdown : '
   call wrtout(ab_out,message,'COLL')
 ...
 if(abs(dtset%orbmag) .EQ. 23) then
   write(message,'(a)')ch10
   call wrtout(ab_out,message,'COLL')
   write(message,'(a)')' Orbital magnetization, Term-by-term breakdown for each band : '
   call wrtout(ab_out,message,'COLL')
   do iband = 1, nband_k
     write(message,'(a)')ch10
     call wrtout(ab_out,message,'COLL')
...
subroutine orbmag_modmag_output(dtset,fermie,nband_k,nterms,orbmag_terms,orbmag_trace)
 ...
 if(abs(dtset%orbmag) .GE. 2) then
   ...
   call wrtout(ab_out,message,'COLL')
   write(message,'(a,3es16.8)') ' d<u|p>/dk dD/dk <u|p> : ',(orbmag_trace(adir,om5),adir=1,3)
   call wrtout(ab_out,message,'COLL')
 ...
 if(abs(dtset%orbmag) .EQ. 3) then
   ...
   do iband = 1, nband_k
     ...
     call wrtout(ab_out,message,'COLL')
     write(message,'(a,3es16.8)') ' d<u|p>/dk dD/dk <u|p> : ',(orbmag_terms(adir,om5,iband),adir=1,3)
     call wrtout(ab_out,message,'COLL')
...
end subroutine orbmag_modmag_output
!!***
...
!!
!! FUNCTION
!! return boolean list of real space points in unit cell, indexed by atom in cell
!! true if point is within PAW sphere of the given atom
!!
!! COPYRIGHT
!! Copyright (C) 2003-2020 ABINIT  group
!! This file is distributed under the terms of the
...
subroutine pawsphere(dtset,in_pawsphere,nfft1,nfft2,nfft3,pawtab,rmet,xred)
 ...
 logical,intent(out) :: in_pawsphere(nfft1,nfft2,nfft3,dtset%natom)
 type(pawtab_type),intent(in) :: pawtab(dtset%ntypat)

 !Local variables -------------------------
 !scalars
 integer :: i1,i2,i3,iat,ic1,ic2,ic3,itypat
 real(dp) :: rdiffl,rdiffl_min

 !arrays
 real(dp) :: rcell(3),rdiff(3),xatom(3)
 ...
 do i1 = 1, nfft1
   rcell(1) = REAL(i1-1,KIND=dp)/nfft1
   do i2 = 1, nfft2
     rcell(2) = REAL(i2-1,KIND=dp)/nfft2
     do i3 = 1, nfft3
       rcell(3) = REAL(i3-1,KIND=dp)/nfft3
       do iat = 1, dtset%natom
         itypat = dtset%typat(iat)
         rdiffl_min = 1.0D6
         do ic1 = -1, 1
           xatom(1) = xred(1,iat) + ic1
           do ic2 = -1, 1
             xatom(2) = xred(2,iat) + ic2
             do ic3 = -1, 1
               xatom(3) = xred(3,iat) + ic3
               rdiff = xatom - rcell
               rdiffl = SQRT(DOT_PRODUCT(rdiff,MATMUL(rmet,rdiff)))
               IF (rdiffl .LT. rdiffl_min) rdiffl_min = rdiffl
             end do
           end do
         end do
         IF (rdiffl_min .LE. pawtab(itypat)%rpaw) in_pawsphere(i1,i2,i3,iat) = .TRUE.
       end do ! end loop over atoms
     end do ! end loop over nfft3
   end do ! end loop over nfft2
 end do ! end loop over nfft1

end subroutine pawsphere
!!***
...
subroutine testprj(atindx,atindx1,cg,cprj,dtset,gsqcut,kg,mcg,mcprj,mpi_enreg,&
 ...
 !Local variables -------------------------
 !scalars
 integer :: fourwf_cplex,fourwf_ndat,fourwf_option
 integer :: i1,i2,i3,i4,i5,i6,iat,iatom,iband,itypat,icg,icprj,ikg,ikpt,il,ilm,ilmn,iln
 ...
 type(pawcprj_type),allocatable :: cprj_k(:,:),my_cprj_k(:,:),my_pprj_k(:,:)
 ...
 ABI_MALLOC(dimlmn,(dtset%natom))
 call pawcprj_getdim(dimlmn,dtset%natom,nattyp,dtset%ntypat,dtset%typat,pawtab,'O')
 ABI_MALLOC(my_cprj_k,(dtset%natom,nband_k))
 call pawcprj_alloc(my_cprj_k,ncpgr,dimlmn)
 ABI_MALLOC(cprj_k,(dtset%natom,nband_k))
 call pawcprj_alloc(cprj_k,ncpgr,dimlmn)
 ABI_MALLOC(my_pprj_k,(dtset%natom,nband_k))
 call pawcprj_alloc(my_pprj_k,ncpgr,dimlmn)
 ...
 do ikpt = 1, dtset%nkpt
   ...
   call pawcprj_get(atindx,cprj_k,cprj,dtset%natom,1,icprj,ikpt,0,isppol,dtset%mband,&
     & dtset%mkmem,dtset%natom,nband_k,nband_k,my_nspinor,dtset%nsppol,0)
   call pawcprj_set_zero(my_cprj_k)
   ...
   ! make all spherical bessel functions at this k pt
   max_l_size = 0
   max_mesh_size = 0
   do itypat = 1, dtset%ntypat
     if (pawtab(itypat)%mesh_size .GT. max_mesh_size) max_mesh_size = pawtab(itypat)%mesh_size
     l_max = (pawtab(itypat)%l_size+1)/2
     if(l_max .GT. max_l_size) max_l_size = l_max
   end do
   ABI_MALLOC(sphharm,(max_mesh_size,max_l_size,npw_k,dtset%ntypat))
   ABI_MALLOC(sbf8_sphharm,(l_max))
   do itypat = 1, dtset%ntypat
     do ipw = 1, npw_k
       kpgl=sqrt(DOT_PRODUCT(kpg_k(ipw,:),MATMUL(gmet,kpg_k(ipw,:))))
       do imesh = 1, pawtab(itypat)%mesh_size
         rr = pawrad(itypat)%rad(imesh)
         jarg = two_pi*kpgl*rr
         call sbf8(l_max,jarg,sbf8_sphharm)
         sphharm(imesh,1:l_max,ipw,itypat) = sbf8_sphharm(1:l_max)
       end do
     end do
   end do
   ABI_FREE(sbf8_sphharm)
   ...
   do iat=1,dtset%natom
     iatom = atindx(iat)
     itypat = dtset%typat(iat)
     if (pawtab(itypat)%has_tproj .NE. 2) then
       ABI_ERROR("tproj not available in routine testprj")
     end if
     mesh_size=pawtab(itypat)%mesh_size
     ABI_MALLOC(ff,(mesh_size))
     ...
     do ilmn=1,pawtab(itypat)%lmn_size
       il  = pawtab(itypat)%indlmn(1,ilmn)
       c2 = c1*ipowl(mod(il,4))
       ilm = pawtab(itypat)%indlmn(4,ilmn)
       iln = pawtab(itypat)%indlmn(5,ilmn)

       do ipw = 1, npw_k
         cph = cmplx(ph3d(1,ipw,iatom),ph3d(2,ipw,iatom))

         ff(2:mesh_size) = pawrad(itypat)%rad(2:mesh_size)*pawtab(itypat)%tproj(2:mesh_size,iln)*&
           & sphharm(2:mesh_size,il+1,ipw,itypat)
         call pawrad_deducer0(ff,mesh_size,pawrad(itypat))
         call simp_gen(intcprj,ff,pawrad(itypat))

         ff(2:mesh_size) = pawrad(itypat)%rad(2:mesh_size)*pawtab(itypat)%tphi(2:mesh_size,iln)*&
           & sphharm(2:mesh_size,il+1,ipw,itypat)
         call pawrad_deducer0(ff,mesh_size,pawrad(itypat))
         call simp_gen(intpprj,ff,pawrad(itypat))

         ! now make cprj and pprj for this kpt, one band at a time
         do iband = 1, nband_k
           ccg = cmplx(cg(1,icg+(iband-1)*npw_k+ipw),cg(2,icg+(iband-1)*npw_k+ipw))
           c3 = c2*ylm_k(ipw,ilm)*intcprj*ccg*cph
           my_cprj_k(iatom,iband)%cp(1,ilmn) = my_cprj_k(iatom,iband)%cp(1,ilmn) + real(c3)
           my_cprj_k(iatom,iband)%cp(2,ilmn) = my_cprj_k(iatom,iband)%cp(2,ilmn) + aimag(c3)
           c3 = c2*ylm_k(ipw,ilm)*intpprj*ccg*cph
           my_pprj_k(iatom,iband)%cp(1,ilmn) = my_pprj_k(iatom,iband)%cp(1,ilmn) + real(c3)
           my_pprj_k(iatom,iband)%cp(2,ilmn) = my_pprj_k(iatom,iband)%cp(2,ilmn) + aimag(c3)
       ...
       do iband = 1, nband_k
         pp_k(1,iatom,iband) = pp_k(1,iatom,iband) + &
           & my_pprj_k(iatom,iband)%cp(1,ilmn)*my_cprj_k(iatom,iband)%cp(1,ilmn) + &
           & my_pprj_k(iatom,iband)%cp(2,ilmn)*my_cprj_k(iatom,iband)%cp(2,ilmn) 
         pp_k(2,iatom,iband) = pp_k(2,iatom,iband) + &
           & my_pprj_k(iatom,iband)%cp(1,ilmn)*my_cprj_k(iatom,iband)%cp(2,ilmn) - &
           & my_pprj_k(iatom,iband)%cp(2,ilmn)*my_cprj_k(iatom,iband)%cp(1,ilmn) 
       end do
...
!!****f* ABINIT/orbmag_magsym
!! NAME
!! orbmag_ddk
!!
!! FUNCTION
!! This routine computes the orbital magnetization and Berry curvature based on input 
!! wavefunctions and DDK wavefuntions. It is based on using magnetic translation symmetry
!! to compute the first order change in E with B. It is assumed that only completely 
...
subroutine orbmag_magsym(cg,cg1,cprj,dtset,gsqcut,kg,mcg,mcg1,mcprj,mpi_enreg,&
 ...
 integer :: l_size,l_max,lmn_size_max
 integer :: me,mcgk,my_nspinor,nband_k,ncpgr,ncpgr1,ndat,ngfft1,ngfft2,ngfft3,ngfft4
 integer :: ngfft5,ngfft6,ngnt,nn,nnp,nkpg,npw_k
 integer :: nonlop_choice,nonlop_cpopt,nonlop_nnlout,nonlop_pawopt,nonlop_signs,nonlop_tim
 integer :: nproc,nterms,projbd_scprod_io,projbd_tim,projbd_useoverlap,spaceComm,with_vectornd
 integer,parameter :: cci=1,vvii=2,vvia=3,vvib=4,rho0h1=5,rho0s1=6,lrb=7,a0an=8,berrycurve_pw=9
 integer,parameter :: berrycurve_qij=10,berrycurve_dij=11
 real(dp) :: arg,dbi,dbr,dgi,dgr,doti,dotr,dub_dsg_i,dug_dsb_i
 real(dp) :: ecut_eff,Enk,lambda,local_fermie,trnrm,ucvol
 complex(dpc) :: dbc,dgc
 logical :: has_nucdip
 ...
 real(dp),allocatable :: ylm_k(:,:),ylmgr_k(:,:,:)
 complex(dpc) :: bckn(3,3),obkn(3),olkn(3),rhorij1(3),S1trace(3)
 complex(dpc),allocatable :: dlrij(:,:,:,:)
 ...
 ikg = 0
 icprj = 0
 nterms = 11 ! various contributing terms in orbmag and berrycurve
 ! 1 orbmag CC
 ! 2 orbmag VV II
 ! 3 orbmag VV I+III part a
 ! 4 orbmag VV I+III part b 
 ...
 do ikpt = 1, dtset%nkpt
   ...
   do nn = 1, nband_k
     ...
     do adir = 1, 3
       ...
       call pawcprj_put(atindx,cwaveprj1,cprj1_k(:,:,adir),dtset%natom,&
         & nn,0,ikpt,0,isppol,nband_k,dtset%mkmem,&
         & dtset%natom,1,nband_k,dimlmn,dtset%nspinor,dtset%nsppol,0,&
         & mpicomm=mpi_enreg%comm_kpt,proc_distrb=mpi_enreg%proc_distrb)
   ...
   do nn = 1, nband_k
     ...
     call make_S1trace_k_n(atindx,cprj_k,dtset,Enk,nn,nband_k,pawtab,S1trace)
     orbmag_terms(1:3,rho0s1,nn) = orbmag_terms(1:3,rho0s1,nn) - real(S1trace(1:3))*trnrm
     !  
     ! 6 Tr[\rho^0 H^1] contribution:
     ! -i/2 eps_abg <u|dp/db>D_ij^0<dp/dg|u>
     call make_rhorij1_k_n(atindx,cprj_k,dtset,nn,nband_k,paw_ij,pawtab,rhorij1)
     orbmag_terms(1:3,rho0h1,nn) = orbmag_terms(1:3,rho0h1,nn) + real(rhorij1(1:3))*trnrm

     ! 7 onsite A_0.p = 1/2 L_R.B contribution
     call make_onsite_l_k_n(atindx,cprj_k,dtset,nn,nband_k,olkn,pawrad,pawtab)
     orbmag_terms(1:3,lrb,nn) = orbmag_terms(1:3,lrb,nn) + real(olkn(1:3))*trnrm

     ! 8 onsite A0.An contiribution
     call make_onsite_bm_k_n(atindx,cprj_k,dtset,nn,nband_k,obkn,pawang,pawrad,pawtab)
     orbmag_terms(1:3,a0an,nn) = orbmag_terms(1:3,a0an,nn) + real(obkn(1:3))*trnrm
 ...
 do nn = 1, nband_k
   ...
   orbmag_terms(1:3,berrycurve_qij,nn) =  ucvol*MATMUL(gprimd,orbmag_terms(1:3,berrycurve_qij,nn))
   orbmag_terms(1:3,berrycurve_dij,nn) =  ucvol*MATMUL(gprimd,orbmag_terms(1:3,berrycurve_dij,nn))
   orbmag_terms(1:3,rho0h1,nn) =  ucvol*MATMUL(gprimd,orbmag_terms(1:3,rho0h1,nn))
   orbmag_terms(1:3,rho0s1,nn) =  ucvol*MATMUL(gprimd,orbmag_terms(1:3,rho0s1,nn))
 end do
 ...
 call orbmag_magsym_output(dtset,local_fermie,nband_k,nterms,orbmag_terms,orbmag_trace)
...
!!****f* ABINIT/orbmag_magsym_output
!! NAME
!! orbmag_magsym_output
!!
!! FUNCTION
!! This routine outputs orbmag terms tailored for magsym ddk routine
!!
!! COPYRIGHT
!! Copyright (C) 2003-2021 ABINIT  group
!! This file is distributed under the terms of the
...
subroutine orbmag_magsym_output(dtset,fermie,nband_k,nterms,orbmag_terms,orbmag_trace)
 ...
 !arrays
 real(dp) :: berry_bb(3,nband_k),berry_total(3),orbmag_bb(3,2,nband_k),orbmag_total(3)
 ...
 if(abs(dtset%orbmag) .GE. 12) then
   write(message,'(a)')ch10
   call wrtout(ab_out,message,'COLL')
   write(message,'(a)')' Orbital magnetic moment, Term-by-term breakdown : '
   call wrtout(ab_out,message,'COLL')
...
end subroutine orbmag_magsym_output
!!***
