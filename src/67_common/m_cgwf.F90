...
!Local variables-------------------------------
 ...
 integer :: use_vnlx,useoverlap,wfopta10
...
!if PAW, and no Fock, no need to compute Vnl contributions
 use_vnlx=0; if (gs_hamk%usepaw==0 .or. associated(gs_hamk%fockcommon)) use_vnlx=1
 if (gen_eigenpb.and.(use_vnlx==1)) then
...
&   nspinor,subham,subovl,subvnlx,use_subovl,use_vnlx,me_g0)
...
&     mpicomm=spaceComm_distrb,proc_distrb=mpi_enreg%proc_distrb)
 ...
 if(prtvol==-level)then
   isubh=1
   if (use_vnlx==1) write(message,'(a)') ' cgwf : isubh  subham(isubh:isubh+1)  subvnlx(isubh:isubh+1)'
   if (use_vnlx==0) write(message,'(a)') ' cgwf : isubh  subham(isubh:isubh+1)'
   do iband=1,nband
     do ii=1,iband
       if (use_vnlx==1) then
...
!!  use_subovl=1 if the overlap matrix is not identity in WFs subspace
!!  use_vnlx= 1 if <C band,k|H|C band_prime,k> has to be computed
...
!!  subvnlx(nband_k*(nband_k+1)*use_vnlx)=non-local Hamiltonian (if NCPP)  plus Fock ACE operator (if usefock_ACE)
!!   expressed in the WFs subspace
!!
!! PARENTS
!!      cgwf
...
!scalars
 ...
 integer,intent(in) :: nbdblock,npw_k,nspinor,use_subovl,use_vnlx,me_g0
...
!arrays
 ...
 real(dp),intent(inout) :: subvnlx(nband_k*(nband_k+1)*use_vnlx)
...
!      Use the time-reversal symmetry, but should not double-count G=0
       if(istwf_k==2 .and. me_g0==1) then
         ...
         if (use_vnlx==1) cvcre=half*cg(1,1+iwavef)*gvnlxc(1,1)
         ipw1=2
       else
         chcre=zero; ipw1=1
         if (use_vnlx==1) cvcre=zero
       end if
       if (use_vnlx==0) then
         do isp=1,nspinor
