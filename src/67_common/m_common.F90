...
&                                           blocksize,nblocks,nfft_blocks,warn_on_fail,disable_output)
   ...
   logical,intent(in),optional  :: warn_on_fail,disable_output
   ...
   logical  :: print_and_exit,l_warn_on_fail,l_disable_output,fixed_blocksize,fixed_fourwf_blocks
...
! *********************************************************************
   ...
   l_disable_output=.false.;if(present(disable_output)) l_disable_output=disable_output
#ifdef HAVE_GPU
   ...
   if(blocksize > 0 .and. nfft_blocks > 0) then
     ...
     if(blocksize > 0) then
       ...
       blocksize=1
       if(.not. l_disable_output) write(std_out,*) "Setting GEMM nonlop block number...", new_line('A')
     end if
     ...
     if(nfft_blocks > 0) then
       ...
       nfft_blocks=1
       if(.not. l_disable_output) write(std_out,*) "Setting FFT blocks number...", new_line('A')
     end if
   end if
   ...
   if(.not. l_disable_output .or. sum_other_mem > free_mem .or. sum_mem > free_mem) then
