...
! have the cursor (cf: pair_list_look) moving forward of one element.
    subroutine pair_list_next(pl) bind(C, name="pair_list_next")
      use m_type_pair_list
      type(pair_list),intent(in) :: pl
...
! and reset the pair_list variable (it can be reused as an empty list) 
    subroutine pair_list_free(pl) bind(C, name="pair_list_free")
      use m_type_pair_list
...
! private
    ...
    subroutine pair_list_get_(pl, key, type_code, i, r, s, len, len_s) bind(C, name="pair_list_get_")
      use m_type_pair_list
      type(pair_list) :: pl
      character(kind=c_char) :: key(*), s(*)
      integer(kind=c_int) :: i, type_code, len, len_s
      real(kind=c_double) :: r
    end subroutine pair_list_get_

    subroutine pair_list_look_(pl, key, type_code, i, r, s, len, len_s) bind(C, name="pair_list_look_")
      use m_type_pair_list
      type(pair_list) :: pl
      integer(kind=c_int) :: type_code, i, len, len_s
      character(kind=c_char) :: key(len), s(len_s)
      real(kind=c_double) :: r
    end subroutine pair_list_look_
...
! type_code is -2 if the list is empty (neither i nor r are setted)
  subroutine pair_list_get(pl, key, type_code, i, r, s)
    ...
    call pair_list_get_(pl, trim(key), type_code, i, r, s, len_trim(key), len(s))
...
! type_code is -2 if the cursor is null (list is empty or end have been reached)
  subroutine pair_list_look(pl, key, type_code, i, r, s)
    ...
    real(kind=c_double),intent(out) :: r
    call pair_list_look_(pl, key, type_code, i, r, s, len(key), len(s))
  end subroutine pair_list_look

! pair_list_init
! arg:
!   type(pair_list): pl
! init a pair_list to zero, preventing random surprises
  subroutine pair_list_init(pl)
    type(pair_list),intent(out) :: pl
    pl%first = C_NULL_PTR
    pl%cursor = C_NULL_PTR
    pl%length = 0
  end subroutine pair_list_init
...
! provided). Nothing happen if none of them are provided.
  subroutine pair_list_set(pl, key, i, r, s)
    ...
    if(present(i)) then
      call pair_list_seti(pl, trim(key), i, len_trim(key))
    elseif(present(r)) then
        call pair_list_setr(pl, trim(key), r, len_trim(key))
    elseif(present(s)) then
        call pair_list_sets(pl, trim(key), s, len_trim(key), len_trim(s))
    end if
