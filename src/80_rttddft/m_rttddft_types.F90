...
! ***********************************************************************
 ...
 call first_setup(tdks,codvsn,dtfil,dtset,ecut_eff,gmet,mpi_enreg,pawrad,pawtab, &
                & psps,psp_gencond,rmet)
...
! call setup2(dtset,npwtot,start,wvl%wfs,xred)
 ...
 call second_setup(tdks,dtset,gmet,mpi_enreg,pawang,pawrad,pawtab,psps, &
                 & psp_gencond,rmet)
...
!! gmet(3,3) <real(dp)> = geometric tensor in reciprocal space 
!! mpi_enreg <MPI_type> = MPI-parallelisation information
!! pawrad(ntypat*usepaw) <type(pawrad_type)> = paw radial mesh and related data
!! pawtab(ntypat*usepaw) <type(pawtab_type)> = paw tabulated starting data
!! psps <type(pseudopotential_type)> = variables related to pseudopotentials
!! psp_gencond <integer> = store conditions for generating psp
!! rmet(3,3) <real(dp)> = geometric tensor in direct space 
!!
!! OUTPUT
!!
!! SIDE EFFECTS
...
!!
!! SOURCE
subroutine first_setup(tdks,codvsn,dtfil,dtset,ecut_eff,gmet,mpi_enreg,pawrad,pawtab,psps,psp_gencond,rmet)
...
! ***********************************************************************
 ...
 call setup1(dtset%acell_orig,bantot,dtset,ecutdg_eff,ecut_eff,gmet,tdks%gprimd, &
           & gsqcut_eff,gsqcutc_eff,ngfftf,ngfft,dtset%nkpt,dtset%nsppol,response, &
           & rmet,dtset%rprim_orig,tdks%rprimd,tdks%ucvol,psps%usepaw)

 !Set up the basis sphere of planewaves
 ABI_MALLOC(tdks%npwarr,(dtset%nkpt))
 ABI_MALLOC(tdks%kg,(3,dtset%mpw*dtset%mkmem))
 call kpgio(ecut_eff,dtset%exchn2n3d,gmet,dtset%istwfk,tdks%kg,dtset%kptns, &
 ...
 call pspini(dtset,dtfil,ecore,psp_gencond,gsqcutc_eff,gsqcut_eff,pawrad,pawtab,psps,tdks%rprimd,comm_mpi=comm_psp)
 ...
 case(0,1)
   tdks%energies%e_corepsp   = ecore / tdks%ucvol
 ...
 call tdks%hdr%update(bantot,tdks%hdr%etot,tdks%hdr%fermie,tdks%hdr%fermih, &
...
!!
!! SOURCE
subroutine second_setup(tdks, dtset, gmet, mpi_enreg, pawang, pawrad, pawtab, psps, psp_gencond, rmet)
...
! ***********************************************************************
 ...
 if(psps%usepaw==1) then
   ...
   call chkpawovlp(dtset%natom,psps%ntypat,dtset%pawovlp,pawtab,rmet, &
...
&      (dtset%positron/=0.and.forces_needed==2)) then
   ...
   call nhatgrid(tdks%atindx1,gmet,my_natom,dtset%natom,&
