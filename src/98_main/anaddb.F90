...
program anaddb
        ...
        real(dp):: wminmax(2)
        real(dp), allocatable:: dcdq(:,:,:,:,:),d2cdq(:,:,:,:,:,:),d2cart(:,:), dchide(:,:,:), lst(:)
        real(dp), allocatable:: dchidt(:,:,:,:), displ(:), eigval(:,:)
        real(dp), allocatable:: eigvec(:,:,:,:,:), fact_oscstr(:,:,:), instrain(:,:)
        real(dp), allocatable:: gred(:,:), phi1(:,:,:,:,:),phi2(:,:,:,:,:),phfrq(:)
        ...
        ABI_MALLOC(phi2, (3,natom,3,3,3))      
        ABI_MALLOC(dcdq, (3,natom,3,natom,3))      
        ABI_MALLOC(d2cdq, (3,natom,3,natom,3,3))      
        dcdq=zero  
        if (inp%asr ==6 .and. inp%flexoflag == 1) then
           write(msg, '(a, a)' )' Rotational invariance: will read IFCs derivatives from flexo DDB',ch10
           call wrtout(units, msg)
           call ddb_phi1(ddb,ddb_lw,ddb_hdr%ddb_version, crystal, filnam(3), phi1,phi2)
           asrq0 = ddb%get_asrq0(inp%asr,inp%rfmeth,crystal,inp%dim_msr,comm,d2dq=-phi1)
        else
           if (inp%asr ==6) then
             write(msg, '(a, a)' )' Rotational invariance: no flexo DDB available, will use real-space IFCs moments',ch10
             call wrtout(units, msg)
           end if
        ...
        !epsinf(1,1)=13.8599!*6/5
        !epsinf(2,2)=24.7808!*6/5
        !epsinf(3,3)=7.3997!*6/5
        ...
        !**********************************************************************
        if (inp%asr==6) then
                if (inp%flexoflag ==1) then
                        call mkphbs(Ifc, crystal, inp, ddb, asrq0, filnam(8), comm,dcdq=dcdq,d2cdq=d2cdq,phi1=phi1,phi2=phi2)
                else
                        call mkphbs(Ifc, crystal, inp, ddb, asrq0, filnam(8), comm,dcdq=dcdq)
                end if
        else
        ...
        if ((inp%dieflag /= 0 .and. inp%dieflag /= 2) .or. inp%nph2l /= 0 .or. inp%nlflag == 1) then
                ...
                if (inp%ifcflag == 1) then
                        ...
                        call gtdyn9(ddb%acell, Ifc%atmfrc, epsinf, Ifc%dipdip, &
                                ...
                                Ifc%asr,dipquad = Ifc%dipquad, quadquad = Ifc%quadquad)
                        if (inp%asr==6) then
                                if (inp%flexoflag ==1) then
                                        call asrq0%apply(natom, mpert, msize, d2cart, qphon, Crystal, dcdq,-phi1,-phi2,d2cdq)
     else 
       call asrq0%apply(natom, mpert, msize, d2cart, qphon, Crystal, dcdq)
     end if
   end if
    ...
    call asrq0%apply(natom, mpert, msize, d2cart,qphon,Crystal)
