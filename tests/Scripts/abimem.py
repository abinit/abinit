...
"""
    ...
    p_leaks = subparsers.add_parser('leaks', parents=[paths_selector_parser], help="Find possible memory leaks.")
    # Subparser for small
    p_small = subparsers.add_parser('small', parents=[paths_selector_parser], help="Find small allocations.")
    # Subparser for intens
    p_intensive = subparsers.add_parser('intens', parents=[paths_selector_parser], help="Find routines with intensive allocations.")
    # Subparser for peaks command.
    p_peaks = subparsers.add_parser('peaks', parents=[paths_selector_parser], help="Find memory peaks.")
    # Subparser for weird command.
    p_weird = subparsers.add_parser('weird', parents=[paths_selector_parser], help="Find weird allocations. i.e. pointers <= 0")
    # Subparser for zerosized command.
    p_zerosized = subparsers.add_parser('zerosized', parents=[paths_selector_parser], help="Find zero-sized allocations.")
    # Subparser for plot command.
    p_plot = subparsers.add_parser('plot', parents=[paths_selector_parser], help="Plot data.")
