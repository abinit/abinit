  ...
  if test "${sd_hdf5_enable}" = "auto" -o "${sd_hdf5_enable}" = "yes"; then
    ...
    if test "${sd_hdf5_ok}" = "yes"; then
      ...
      if test "${sd_hdf5_mpi_ok}" = "yes"; then
        ...
        sd_hdf5_enable="no"
        sd_hdf5_cppflags=""
        sd_hdf5_cflags="-I ${ac_abs_top_builddir}/fallbacks/install_fb/${abi_fc_vendor}/${abi_fc_version}/hdf5/${abi_fb_hdf5_version}}/include"
        sd_hdf5_cxxflags=""
        sd_hdf5_fcflags=""
        sd_hdf5_ldflags=""
        sd_hdf5_libs="-L${ac_abs_top_builddir}/fallbacks/install_fb/${abi_fc_vendor}/${abi_fc_version}/hdf5/${abi_fb_hdf5_version}/lib ${sd_hdf5_libs_def}"
	test "${sd_hdf5_enable_fc}" = "yes" && \
          sd_hdf5_fcflags="${sd_hdf5_fcflags_def} -I ${ac_abs_top_builddir}/fallbacks/install_fb/${abi_fc_vendor}/${abi_fc_version}/hdf5/${abi_fb_hdf5_version}}/include"
...
AC_DEFUN([_SD_HDF5_CHECK_COMPILERS], [
  ...
  if test "${sd_hdf5_h5cc}" != ""; then
    ...
    #These extralibs are useless and sometimes not found
    #AC_MSG_CHECKING([for HDF5 extra dependencies])
    #sd_hdf5_libs_extra=`${sd_hdf5_h5cc} -showconfig | grep 'Extra libraries: ' | sed -e 's/.*Extra libraries: //g' -e 's/m\;dl\;//g' -e 's/m\;dl//g' -e 's/Threads::Threads/-lpthread/g' -e 's/;/ /g'`
