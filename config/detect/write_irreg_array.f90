program write_irreg_array
  ...
  if ( myrank == 0 ) map(1:nb) = (/0,2/)
  if ( myrank == 1 ) map(1:nb) = (/1,3/)
  call mpi_type_create_indexed_block(nb, 1, map, mpi_double_precision, &
&   filetype, ierr)
  call printerr(ierr, "mpi_type_create_indexed_block")
