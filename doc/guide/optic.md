...
generated by Abinit (actually matrix elements of the dH/dk operator), and
...
Abinit program, four different files, corresponding to the physical system that you want to study:
  ...
  * Three files containing the matrix elements of the dH/dk operators of the unperturbed wavefunctions, 
...
in such calculations. Still, the dH/dk will be made available in such files.
...
All the other ones are neglected. This has two important consequences for the preliminary runs:
  ...
  * One does not need the full change of Bloch wavefunctions with respect to d/dk, but 
    only the matrix elements of dH/dk between the wavefunctions of this range of energy
...
Contains the following 3 data sets
  ...
  * 7) Column 1 - energy(eV), column 2 - absorption coeff (in m-1) = omega Im(eps) / c n(eps)

In the header of the file you can find information about the calculation. 

In case [[nsppol]]==2, the spin-decomposition of items 1 and 2 above is provided in columns 3-4 for the imaginary part, and 3-5 for the real part (the spin-independent contribution delta(ab) is mentioned as well).
