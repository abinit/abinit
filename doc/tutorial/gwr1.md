...
!!! important
    ...
    To solve the problem, simply set [[ngfft]] explicitly in the input file using the value used in the GS part.


To accelerate the computation and reduce the memory requirements, we truncate the PW basis set using
[[ecutwfn]] = 10 < [[ecut] = 16.
[[gwr_boxcutmin]] is set to 1.0 in order to accelerate the calculation and reduce memory requirements.
...
the converged values of [[nband]], [[ecuteps]], [[ecutsigx]], [[ecutwfn]] and [[gwr_boxcutmin]],
...
The first parameter we check for convergence is the cutoff energy [[ecutwfn]] used to build
...
robot.plot_qpgaps_convergence_new(x="ecutwfn", y="qpz0_dirgaps",
                                  abs_conv=abs_conv,
