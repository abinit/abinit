...
The examples are for Abinit v8 and some option names should be changed to be compatible
...
if we are using a precompiled library installed with your package manager.
...
For each external library, the build system provides specialized options to pass 
flags to the compiler and the linker.
Let's use *libxc* as example. In this case, we have:
...
and the let *configure* automatically detect the other options.
...
!!! important
    ...
    Also note that one can use shell variables and reuse the output of external tools using
...
Other examples of configuration files for clusters can be found in the
