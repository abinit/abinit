...
The true library should be written in an Oriented Object way to allow to swith during runtime between different libraries, or accelerators.
...
Even if you created a `xg_t` object to allocate memory, you cannot work on `xg_t` but rather on its own block that you can access with `xg`%self`.
`xg%self` is of type `xgBlock_t` and representis the full array you allocated.

To work on a subarray, you wil need a `xgBlock_t` object, and build it with the `xg_setBlock` function
```fortran
type(xg_t)::xg1
type(xgBlock_t)::xgblockA
...
You can achieve this operation with whas is called *mapping* and *reverse mapping*.
From `Fortran90` array to `xgBlock_t` it is  *mapping*
On a regular cpu, this should be free, no allocations, no memory transfert, nothing.
But keep in mind that if you `xgBlock_t` represent an array on a GPU or other coprocessor unit, this will allocate memory and do transferts !
...
This is exactly the opposite operation as the maping.
```fortran
double precision :: array(2,10)
type(xgBlock_t) :: xgblockA
...
call xgBlock_reverseMap(xgblockA,array,rows,cols)
```
Be carefull ! If you work with complex, the array(2,10) has 10 complex numbers, only **one** row of complex numbers, not 2 !!!  
Example:
...
Please refer to blas documentation at [Netlib documentation] (http://www.netlib.org/lapack/explore-html/dir_8ff1c41005a6034efef54f08b5a85d05.html)
## POTRF
## TRSM
## GEMM
## ADD
...
Missing arguments are not needed with this library.
Please refer to blas documentation at [Netlib documentation] (http://www.netlib.org/lapack/explore-html/df/d9a/group__complex16_h_eeigen.html)
## HEEV
## HEEVD
## HPEV
## HPEVD
...
ScaLAPACK
---------
**WARNING : ** At this day (2018/10), none of scalapack implementation is thread safe.
...
-----------------
## Array Shift
This function is the same as the standard fortan `cshift` function.
...
All those 4 variables are optinal.

## Colwise $$Y-aX$$
The colwise operation done here is done column by columns.
It is a userfull function to compute residus for eigen problems.
Let suppose you solve HX=eX or HX=eBX, then calling this function will compute HX-eX or HX-eBX for each eigen vector of the matrix X.
```fortran
...
*Note:* this function is very similar to the BLAS `saxpy` function by the last one is not use because the minus operation would require an array copy that I want to avoid.
## Set to 0
...
## Get average
Compute the average of the fulle block
```fortran
