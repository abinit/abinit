...
#
# Then one should update the present script to put the current year (at present, valid for going from 2022 to 2023 !) !
#
...
# sed -iTMP 's!2022 ABINIT!2023 ABINIT!' */*.py */*/*.py */*/*/*.py
# followed by
...
#sed -e 's&\(([cC]) ....-20\)20\( ABINIT\)&\121\2&' $file > $file.tmp
 sed -e 's&\(([cC]) ....-20\)22\( ABINIT\)&\123\2&' $file > $file.tmp
 rm $file
...
#sed -e 's&\(([cC]) 20\)20\( ABINIT\)&\120-2021\2&' $file.tmp > $file
 sed -e 's&\(([cC]) 20\)22\( ABINIT\)&\122-2023\2&' $file.tmp > $file
 rm $file.tmp
