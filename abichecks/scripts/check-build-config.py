...
def main():
    ...
    for dep in cnf_dep.sections():
        env_config += [
            "%s_%s" % (dep.upper(), item) for item in ["FCFLAGS", "LDFLAGS", "LIBS"]
        ]
        langs = cnf_dep.get(dep, "languages").split()
        if "c" in langs:
            env_config += [
                "%s_%s" % (dep.upper(), item) for item in ["CPPFLAGS", "CFLAGS"]
            ]
        if "c++" in langs:
    ...
    if nerr > 0:
        sys.stderr.write(
            "%s: reporting environment and option mismatches\n\n"
            % (os.path.basename(sys.argv[0]))
        )
        sys.stderr.write(
            "X: D=Documentation / I=Instance / R=Removed / U=Undefined\n\n"
        )
