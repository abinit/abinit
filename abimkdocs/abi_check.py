...
def main():
  ...
  with open(path+"abinit_vars.yml") as f:
    variables = yaml.load(f)
  with open(path+"characteristics.yml") as f:
    characteristics = yaml.load(f)
  with open(path+"varfiles.yml") as f:
    list_varfiles = yaml.load(f)
    varfile_names=[]
    for varfile in list_varfiles:
      varfile_names.append(varfile.name)
  with open(path_topics+"list_of_topics.yml") as f:
    topics = yaml.load(f)
    for i,item in enumerate(topics) :
       topics[i]=item.strip()
  with open(path_topics+"list_relevances.yml") as f:
  ...
  for var in variables:
    ...
    if var.topics is None:
      ...
      for topic_name_relevance in topics_name_relevance:
        ...
        if name_relevance[0].strip() not in topics:
          ...
          retcode += 1
        if name_relevance[1].strip() not in relevance_names:
    ...
    if var.varfile is None:
      ...
      retcode += 1
    elif not isinstance(var.varfile, str) or var.varfile not in varfile_names:
